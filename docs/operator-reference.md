...
| Own-fork continuation resolution | After an own-fork continuation passes its escalation end date, a non-`None` outcome selected by the child pool overrides the carried non-decision result; otherwise the normal resolution remains in effect. | [`EscalationGameCalculations.getQuestionResolution`](../solidity/contracts/peripherals/EscalationGameCalculations.sol), [`SecurityPoolForker.getQuestionOutcome`](../solidity/contracts/peripherals/SecurityPoolForker.sol) |
