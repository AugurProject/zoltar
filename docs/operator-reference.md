...
| Strict leading resolution | After the unresolved-cost check and the all-zero `Invalid` fallback, a strict `Invalid`, `Yes`, or `No` lead returns that outcome. Valid local deposits prevent tied maxima below non-decision by reducing the accepted amount by `1 wei`, or reverting if that adjusted amount becomes invalid; continuation snapshots reject preserved positive tied maxima below `nonDecisionThreshold`. Ties that already sit at `nonDecisionThreshold`, or all-zero preserved balances, remain valid carried states. | [`EscalationGameCalculations.getQuestionResolution`](../solidity/contracts/peripherals/EscalationGameCalculations.sol), [`EscalationGameCalculations._getStrictLeaderOrNone`](../solidity/contracts/peripherals/EscalationGameCalculations.sol), [`EscalationGameCalculations._getAcceptedDepositAmount`](../solidity/contracts/peripherals/EscalationGameCalculations.sol), [`EscalationGameCarry.initializeForkCarrySnapshotWithResolutionBalances`](../solidity/contracts/peripherals/EscalationGameCarry.sol) |
...
| Local carry batching | Local unresolved export work is paged in batches of at most `MAX_UNRESOLVED_EXPORT_REFS = 64` refs per vault. | [`EscalationGameEscrow._exportVaultUnresolvedDepositBatchDetailed`](../solidity/contracts/peripherals/EscalationGameEscrow.sol) |
...
| Continuation balance snapshot | `getForkCarrySnapshot` returns the inherited carry totals, and child initialization uses those totals for both the live `Invalid`/`Yes`/`No` continuation balances and carry totals. Forked escrow separately tracks source principal and arriving child REP backing. | [`SecurityPoolForkerBase._initializeChildForkedEscalationGameIfNeeded`](../solidity/contracts/peripherals/SecurityPoolForkerBase.sol), [`EscalationGameCarry.getForkCarrySnapshot`](../solidity/contracts/peripherals/EscalationGameCarry.sol), [`EscalationGameCarry.initializeForkCarrySnapshotWithResolutionBalances`](../solidity/contracts/peripherals/EscalationGameCarry.sol), [`EscalationGameEscrow.recordForkedEscrowForOutcome`](../solidity/contracts/peripherals/EscalationGameEscrow.sol) |
| Fork-wide escalation migration | `migrateRepToZoltar` explicitly registers outcomes in the child destination set, while `createChildUniverse` also registers an outcome as it deploys that child pool. `migrateVaultWithUnresolvedEscalation` has no child selector: it exports each parent vault batch once, persists that batch, and advances a destination cursor by one registered child per transaction. The next parent batch cannot be exported until the current batch reaches every fixed destination. External forks reproduce the same REP amount in every registered child; own forks convert and cap child REP independently per outcome, so backing can differ while source principal and carry stay identical. New destinations through either registration route are rejected after escalation migration starts or the deadline. Ordinary vault ownership still selects one child through `migrateVault`. | [`SecurityPoolForker.migrateRepToZoltar`](../solidity/contracts/peripherals/SecurityPoolForker.sol), [`SecurityPoolForkerVaultMigrationBase._getOrDeployChildPool`](../solidity/contracts/peripherals/SecurityPoolForkerVaultMigrationBase.sol), [`EscalationGameForker.migrateVaultWithUnresolvedEscalation`](../solidity/contracts/peripherals/EscalationGameForker.sol), [`EscalationGameForker._migratePendingEscalationBatch`](../solidity/contracts/peripherals/EscalationGameForker.sol), [`SecurityPoolForkerVaultMigrationBase._capOwnForkEscalationChildRep`](../solidity/contracts/peripherals/SecurityPoolForkerVaultMigrationBase.sol), [`Zoltar.splitMigrationRep`](../solidity/contracts/Zoltar.sol) |
| Unresolved escalation migration permissions | During either fork's migration window, only the affected vault may migrate its unresolved carry. After an external-fork deadline, any caller may name an affected vault and fund that carry in every registered child. Own-fork migration remains vault-authorized. Late carry funding never migrates ordinary vault ownership. | [`EscalationGameForker.migrateVaultWithUnresolvedEscalation`](../solidity/contracts/peripherals/EscalationGameForker.sol), [`SecurityPoolForker.migrateVault`](../solidity/contracts/peripherals/SecurityPoolForker.sol) |
...
| Child-pool deployment window | New outcome selection closes after eight weeks and when escalation migration starts, whether registration is requested through `migrateRepToZoltar` or child deployment through `createChildUniverse`. Destination registration is stateful even when the current migration amount is zero; zero REP does not permit late registration. A destination registered before escalation migration starts is already fixed, so paged unresolved-carry recovery may still materialize that registered continuation after the deadline. | [SecurityPoolForker.sol](../solidity/contracts/peripherals/SecurityPoolForker.sol), [SecurityPoolForkerVaultMigrationBase.sol](../solidity/contracts/peripherals/SecurityPoolForkerVaultMigrationBase.sol), [`EscalationGameForker._migratePendingEscalationBatchToChild`](../solidity/contracts/peripherals/EscalationGameForker.sol), [SecurityPoolUtils.sol](../solidity/contracts/peripherals/SecurityPoolUtils.sol) |
