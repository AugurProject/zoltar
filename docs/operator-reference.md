...
| Auction withdrawal dust | Funded auctions carry uniform-price division dust through `clearingRemainder`; underfunded auctions carry pro-rata division dust through `underfundedRemainder`. Both reconcile bid-level withdrawals to finalized aggregate REP. | [UniformPriceDualCapBatchAuction.sol](../solidity/contracts/peripherals/UniformPriceDualCapBatchAuction.sol) |
...
| Price round | One accepted REP/ETH price inside the coordinator, plus the operation volume that price is allowed to authorize before another report is required. | [SecurityPoolOracleCoordinator.sol](../solidity/contracts/peripherals/SecurityPoolOracleCoordinator.sol) |
| Price-round budget | The remaining ETH-notional operation volume available in the current price round. Withdrawals, liquidations, and allowance increases spend this shared cap so one OpenOracle report cannot authorize unlimited exposure. | [SecurityPoolOracleCoordinator.sol](../solidity/contracts/peripherals/SecurityPoolOracleCoordinator.sol) |
| ETH-notional | Operation exposure expressed in ETH value, used so REP movement, debt movement, and allowance increases can be compared against the same price-round budget. | [SecurityPoolOracleCoordinator.sol](../solidity/contracts/peripherals/SecurityPoolOracleCoordinator.sol) |
| Immediate execution | If a price is still valid and the operation fits the remaining price-round budget, the operation executes immediately and the caller's ETH is refunded. | [SecurityPoolOracleCoordinator.sol](../solidity/contracts/peripherals/SecurityPoolOracleCoordinator.sol) |
