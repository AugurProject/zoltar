...
| Strict leading resolution | A strict `Invalid`, `Yes`, or `No` lead returns that outcome. If no outcome strictly leads and the game is not otherwise unresolved, the helper returns `None` so tied leaders remain unresolved. | [`EscalationGameCalculations.getQuestionResolution`](../solidity/contracts/peripherals/EscalationGameCalculations.sol), [`EscalationGameCalculations._getStrictLeaderOrNone`](../solidity/contracts/peripherals/EscalationGameCalculations.sol) |
