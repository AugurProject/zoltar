...
Augur Placeholder is a prediction-market and oracle-security protocol built on top of [Zoltar](../solidity/contracts/Zoltar.sol). Zoltar supplies forkable universes, question registration, outcome encoding, and REP migration across disputed branches of reality. Augur Placeholder adds question-specific [security pools](../solidity/contracts/peripherals/SecurityPool.sol), ETH-collateralized complete sets, escalation-driven local resolution, and a fork-recovery path that migrates economic state into child universes and, when needed, restores missing collateral through a truth auction.
...
## 2. Zoltar: Base Oracle Substrate

Zoltar is the base layer. It does not implement the entire market system in this repository. Its role is narrower and more fundamental: represent disputed outcomes as branchable universes and let REP holders migrate into those branches. The core contracts in this layer are [Zoltar](../solidity/contracts/Zoltar.sol), [ZoltarQuestionData](../solidity/contracts/ZoltarQuestionData.sol), [ReputationToken](../solidity/contracts/ReputationToken.sol), and [ScalarOutcomes](../solidity/contracts/ScalarOutcomes.sol).

What Zoltar does not do is equally important. Zoltar does not create security pools, price REP against ETH, run the local Placeholder escalation lifecycle, or perform the truth auction that repairs missing collateral after a fork. Those are all Augur Placeholder features layered on top of Zoltar.
...
Universe `0` is the genesis universe. Its REP token is an external genesis token configured in `Constants.GENESIS_REPUTATION_TOKEN`. Child universes are identified deterministically as `uint248(keccak256(abi.encode(parentUniverseId, outcomeIndex)))`.

### Fork thresholds and burn behavior
...
Under the current constants, initiating a fork requires burning REP equal to 5% of the universe's total theoretical supply. The initiator loses part of that amount permanently. Specifically, 20% of the fork-triggering deposit is burned, while the remaining 80% becomes the initiator's internal migration balance for the forked universe.

If `B` is the permanently burned portion and `M` is the initiator's migration balance created by the fork:

- `B = F / 5`
- `M = F - B = 4F / 5`

Genesis REP cannot be burned natively, so the contract transfers it to the configured burn address. Child-universe REP is minted and burned directly by `Zoltar` through `ReputationToken`.

### Child universes and REP migration
...
### Questions and outcome encoding
...
Categorical questions store sorted outcome labels. The current implementation requires the labels to be strictly ordered by hash, which prevents equivalent sets from being registered in different orders.

Scalar questions store no categorical labels. Instead, they define a tick range and display range. `ScalarOutcomes` converts the encoded payout numerators into human-readable values.

At a high level, if `p` is the payout assigned to the upper scalar leg and `T` is `numTicks`, the displayed scalar value is interpolated linearly across the configured range:
...
This distinction matters because malformed answers are rejected during child-universe REP migration and share migration, while `Invalid` remains a valid branch and a valid final outcome.

Zoltar therefore provides the base mechanism for branching disputed reality into child universes and migrating reputation capital across those branches.

## 3. Augur Placeholder Architecture

Augur Placeholder is the system described by the peripheral contracts. It extends Zoltar with an underwriting and collateral system around individual questions. The main contracts in this layer are [SecurityPool](../solidity/contracts/peripherals/SecurityPool.sol), [EscalationGame](../solidity/contracts/peripherals/EscalationGame.sol), [SecurityPoolForker](../solidity/contracts/peripherals/SecurityPoolForker.sol), [UniformPriceDualCapBatchAuction](../solidity/contracts/peripherals/UniformPriceDualCapBatchAuction.sol), [PriceOracleManagerAndOperatorQueuer](../solidity/contracts/peripherals/PriceOracleManagerAndOperatorQueuer.sol), and [ShareToken](../solidity/contracts/peripherals/tokens/ShareToken.sol).
...
The escalation game becomes available after the question end time. It is started lazily on first use and currently uses a hard-coded initial bond parameter in `SecurityPool.depositToEscalationGame`.
...
## 8. Forks and Migration Across Layers

Fork handling spans both layers and must be described in two parts.

### Part A: Zoltar fork

At the Zoltar layer:

- a universe forks through `forkUniverse`
- the forking question is recorded
- child universes exist per outcome index
- REP holders move REP into an internal migration balance
- `splitMigrationRep` mints child-universe REP into the selected branches

This step branches reputation capital.

### Part B: Augur Placeholder migration

At the Augur Placeholder layer, the parent `SecurityPool` and its economic state must also move.
...
This separation is important. REP migration is a Zoltar primitive. Vault migration, collateral transfer, and child-pool initialization are Augur Placeholder primitives built on top of it.
...
6. If the auction is underfunded, the branch still continues, but the white paper should treat this as an important degraded mode because collateral repair was incomplete.
...
8. If the escalation game reaches non-decision, a Zoltar fork path can be triggered. If the fork was caused by the pool's own escalation game, `forkZoltarWithOwnEscalationGame` is the direct bridge from Placeholder back into Zoltar.
...
- Truth-auction and oracle parameters are intentionally provisional in the current code.
