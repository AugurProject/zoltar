...
const presetDefinitions: Record<string, ToolPreset[]> = {
...
function toolInputs(tool: HTMLDetailsElement): ToolInput[] {
	return Array.from(tool.querySelectorAll(toolInputSelector)).filter((input): input is ToolInput => input instanceof HTMLInputElement || input instanceof HTMLSelectElement)
}

function dispatchInput(input: ToolInput): void {
...
function toolState(tool: HTMLDetailsElement): ToolState {
	return Object.fromEntries(
		toolInputs(tool)
			.map((input): readonly [string, string] | undefined => {
				const key = inputKey(input)
...
function scenarioUrl(tool: HTMLDetailsElement): string {
...
function applyLinkedScenario(): void {
	if (linkedScenarioHandled) return
	const url = new URL(document.baseURI)
	...
	if (toolIsUnavailable(tool)) return
	linkedScenarioHandled = true
