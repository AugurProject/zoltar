...
For escalation, append each `LocalDepositAppended` leaf under its stable deposit index and retain the live event-derived MMR peaks and leaves for every game. The leaf's depositor is the immutable claim owner. On `ClaimDeposit` with `transferredRep = false`, decrement that depositor's raw local backing by `originalDepositAmount` when `truthAuctionRepBefore == 0`; otherwise decrement by the ceiling of `originalDepositAmount × truthAuctionRepBefore / truthAuctionRepRemaining`. Other unconsumed deposits by the same depositor remain backed. There are no claim-owner transitions, owner slots, portfolio registries, or participant caps.

When `SecurityPoolForkSnapshot` records unresolved escalation, preserve the proof/carry roots, counts, peaks, and leaves under `escalationSnapshotId`. `ForkCarryCheckpoint` installs that immutable proof state, aggregate backing, and retention checkpoint in the child. It copies no leaves, bundles, or owners. A winning inherited settlement supplies its carry proof, derives the immutable payout address from the committed depositor, consumes the stable index through the nullifier tree, and pays once. The fixed 64-peak MMR frontier is logarithmic commitment state supporting up to `2^64 - 1` leaves, not a 64-participant cap.
