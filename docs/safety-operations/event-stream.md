...
| Escalation claim ownership and auction retention | `EscalationClaimMoved`, `TruthAuctionHaircutApplied` |
...
| Escalation continuation | `ForkCarryCheckpoint`, `PayoutClaimCheckpointImported`, `ForkContinuationResumed`, `InheritedThresholdTie`, `CarryDepositConsumed` |
...
| `SecurityPoolFactory.DeploySecurityPool` | `securityPool indexed`, `truthAuction`, `priceOracleManagerAndOperatorQueuer`, `shareToken`, `parent indexed`, `universeId indexed`, `questionId`, `statoblastSecurityMultiplierBps`, `initialReportPriorityFeeWeiPerGas`, `currentRetentionRate`, `completeSetCollateralAmount` | Discovers the pool and all of its constructed relationships, including the immutable initial-report priority fee inherited by child pools in the lineage. |
...
| `EscalationGame.EscalationClaimMoved` | `fromVault indexed`, `toVault indexed`, `numerator`, `denominator` | Moves the stated fraction of every bounded claim bundle from the target vault to the liquidator vault. The underlying REP remains in the game; reducers update current claim ownership, not token balances. |
...
| `EscalationGame.PayoutClaimCheckpointImported` | `sourceGame indexed`, `startIndex`, `endIndex`, `sourceBundleCount` | Replaces the imported checkpoint cursor with `endIndex` after copying the half-open bundle range `[startIndex, endIndex)`. Reducers require contiguous progress from the same source and treat completion as `endIndex == sourceBundleCount`. |
| `EscalationGame.ForkContinuationResumed` | `resumedAt` | Starts post-fork time only after aggregate backing and the payout-claim checkpoint are complete. Reducers retain the haircut's rebased elapsed position separately; the deadline is `max(rebasedCurveEnd, resumedAt + 3 days)`. |
...
For escalation, append each `LocalDepositAppended` leaf under its stable deposit index and retain the live event-derived MMR peaks and leaves for every game. Track one bounded claim bundle per stable bundle identity, which aggregates deposits for its depositor and creating game: initialize its depositor with `CLAIM_SHARE_SCALE = 10^18` shares and add later deposits from that depositor only while it still owns every share. Ownership changes only through `EscalationClaimMoved`; there is no voluntary transfer surface. A partial move uses `fromOwnerShares * numerator / denominator` with Solidity integer division and cannot allocate the eighth live owner slot, which is reserved for a later full close. A bundle whose result is zero shares remains with the source and contributes no liquidation claim credit; other bundles still move. A full close replaces the source owner in place, or merges into an existing destination slot, so fragmented ownership cannot veto liquidation. On `ClaimDeposit` with `transferredRep = false`, preserve the bundle key and ownership metadata and decrement its raw payout backing by `originalDepositAmount` when `truthAuctionRepBefore == 0`; otherwise decrement by `⌈originalDepositAmount × truthAuctionRepBefore / truthAuctionRepRemaining⌉`. Other unconsumed deposits represented by the same bundle remain backed. Process the globally capped 64 bundle keys, each source vault's at most 64 registered keys, and the eight fixed owner slots per key.

When `SecurityPoolForkSnapshot` records unresolved escalation, preserve the proof/carry roots, counts, peaks, and leaves under `escalationSnapshotId`. `ForkCarryCheckpoint` installs that immutable proof state in the child. A root game's nonzero `ForkedEscrowRecorded` record registers or increases its child-local payout bundle. In a continuation with a nonzero carry source, the same event updates local fork escrow without allocating another child-local payout key; the durable payout identity arrives through checkpoint import. Payout ownership uses a separate consolidated checkpoint: clone the frozen source registry in batches of at most eight bundle keys, apply each `PayoutClaimCheckpointImported(sourceGame, startIndex, endIndex, sourceBundleCount)` slice, and do not accept `ForkContinuationResumed` until `endIndex == sourceBundleCount`. The global source count cannot exceed 64, so at most eight calls finish the checkpoint; auction finalization may produce the first import event itself after the child becomes operational. Every child then contains the complete current payout registry, so liquidation and settlement read only that child and never walk fork ancestry.

`TruthAuctionHaircutApplied` replaces the game's effective REP basis, rebases elapsed curve position, and updates its cumulative normalized binary retention mantissa and exponent. A carried proof compares the current checkpoint directly with the checkpoint of the game that created its stable identity and applies that single ratio to both proof amount and cumulative amount. Renormalization preserves significant bits across arbitrary recursive depth; the final REP-base-unit result may floor to zero, but the lineage checkpoint itself does not. Recursive forks therefore cannot make old haircuts disappear or make liquidation run out of ancestry slots. Independently floor event-derived outcome balances, aggregate `totalEscrowedRep`, and aggregate `forkCarryEscrowedRep`; never reconstruct an aggregate by summing independently rounded bundle views. A later `LocalDepositAppended` after resume is converted to raw claim units with ceiling division by the game's local auction ratio so it does not inherit earlier losses. The haircut does not reverse an already triggered own fork. `ForkContinuationResumed(resumedAt)` starts time later and sets the deadline to `max(rebasedCurveEnd, resumedAt + 3 days)`. `NonDecisionReached` records the `Local` lifecycle state and timestamp. `InheritedThresholdTie(sourceGame indexed)` records inherited state without fabricating a local timestamp; accept it only after a `ForkCarryCheckpoint` from the same child emitter, and require its indexed `sourceGame` to equal that checkpoint's source. Apply each `CarryDepositConsumed` by its explicit reason and resulting roots and totals. Reconstruct and maintain peaks from events; never read them from contract storage.
