...
| `EscalationGame.ForkContinuationResumed` | `resumedAt` | Starts post-fork time only after aggregate backing is complete. Reducers retain the haircut's rebased elapsed position separately; the deadline is `max(rebasedCurveEnd, resumedAt + 3 days)`. |
...
For escalation, append each `LocalDepositAppended` leaf under its stable deposit index and retain the live event-derived MMR peaks and leaves for every game. Track each deposit's bounded claim bundle separately: initialize its depositor with `CLAIM_SHARE_SCALE = 10^18` shares and add later deposits from that depositor to the same bundle principal only while that depositor still owns every share. Once liquidation splits or fully moves the bundle, another same-game deposit from that depositor reverts; otherwise fresh principal would be captured by owners of the earlier claim. For `EscalationClaimMoved(fromVault, toVault, numerator, denominator)`, visit each of the source vault's at most eight live bundles. A full close (`numerator = denominator`) moves every remaining source share exactly; otherwise move `fromOwnerShares * numerator / denominator` with Solidity integer division. If that value is zero for any live bundle, the entire transaction reverts before emitting the move event, because aggregate principal and liabilities must not move without the matching payout share. Each later successful move uses the already-rounded current shares. Add moved shares to the destination's existing slot or one of its bounded empty slots. Ownership changes only through this liquidation event; there is no voluntary transfer surface. When `SecurityPoolForkSnapshot` records an unresolved escalation, resolve its source game from the pool relationship and `EscalationRepDrainedAtFork`, then preserve an immutable copy of only the current proof/carry roots, counts, peaks, and leaves under `escalationSnapshotId`. Retain the immediate live source-game pointer separately. Later source consumption changes only live carry, while liquidation processes the current game and at most seven immediate-source ancestors, applying `EscalationClaimMoved` to all eight registries without altering the immutable proof snapshot. An eighth source link requiring a ninth registry reverts rather than producing partial ownership. On `ForkCarryCheckpoint`, select that historical carry version by `snapshotId`, verify its source game and each checkpoint count/root, then clone the frozen peaks and leaves into the child before applying child-local deposits. The checkpoint fixes the roots, counts, unresolved totals, and resolution balances written to that child. A continuation-local stable deposit index encodes its exact source registry; a root deposit retains the legacy small index and resolves by walking the bounded immediate-source chain. Settlement therefore follows the correct bounded current owners even after recursive continuation. `TruthAuctionHaircutApplied` replaces the game's effective REP basis and rebased elapsed curve position. Independently floor the event-derived outcome balances, aggregate `totalEscrowedRep`, and aggregate `forkCarryEscrowedRep`; raw bundle principal and claim shares do not change, while effective bundle views apply `repRemaining / repBefore` with their own floor. Never reconstruct an aggregate by summing independently rounded bundle views. When a carried proof crosses more than one continuation, apply each descendant auction ratio in the bounded immediate-source chain. Stop before the encoded source game for a continuation-local leaf, because that deposit arrived after its source auction; an unencoded root leaf receives every later continuation haircut. Apply the same cumulative ratio to the proof amount and cumulative amount so its reward position and the aggregate effective backing stay on one scale. A later `LocalDepositAppended` after resume represents newly accepted effective REP: convert it to raw bundle units with ceiling division by the stored retention ratio, so it does not suffer the historical auction loss. The haircut does not reverse an already triggered own fork. `ForkContinuationResumed(resumedAt)` starts time later and sets the deadline to `max(rebasedCurveEnd, resumedAt + 3 days)`. `NonDecisionReached` records the `Local` lifecycle state and its timestamp. `InheritedThresholdTie(sourceGame indexed)` records the inherited state without creating a local timestamp; accept it only after a `ForkCarryCheckpoint` from the same child emitter and require its indexed `sourceGame` to equal that checkpoint's source. Apply each `CarryDepositConsumed` by its explicit reason and resulting roots/totals. Reconstruct and maintain peaks from events; never read them from contract storage.
