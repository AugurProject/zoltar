...
| Continuation withdrawal | Anyone may relay a batch of winning carried proofs for one claim bundle after a child continuation resolves. A proof authenticates the original deposit and consumes its leaf once; its stable identity selects the creating game's normalized retention checkpoint, while payout follows the current owner shares in the child's consolidated bounded checkpoint. One mantissa/exponent ratio applies every intervening auction haircut to both proof amount and cumulative reward position without walking ancestors, and excludes losses before a continuation-local deposit was created. Inherited losing outcomes retire in constant-size work when the result is final and require no proof transaction; locally created losing deposits retain ordinary settlement. Child initialization freezes checkpoint metadata but performs no owner import. Before resume, anyone may advance import through the operational child pool in batches of at most eight bundle keys. Keeping those batches out of deployment, migration, and auction finalization prevents claim count from exhausting their gas budget. Liquidation then processes only the current game's at-most-64 keys per vault, so recursive depth is never an operator liveness limit. | [`SecurityPool.withdrawForkedEscalationDeposits`](../../solidity/contracts/peripherals/SecurityPool.sol), [`EscalationGameSettlement.withdrawDeposit`](../../solidity/contracts/peripherals/EscalationGameSettlement.sol), [`EscalationGameCarry._getEffectiveInheritedUnresolvedTotal`](../../solidity/contracts/peripherals/EscalationGameCarry.sol) |
...
| Liquidation snapshot | A staged liquidation becomes stale if target ownership or allowance changes in either direction. The coordinator's distance guard uses the queue-time free REP and locally attributed escalation REP; after target equality checks pass, the pool re-evaluates safety from the live REP-per-ownership rate, balances, and current-game escrow. A partial liquidation quotes free REP at a 5% bonus before ownership-unit flooring; a full or dust-promoted close transfers all remaining target ownership. Escalation claims move proportionally and target fees remain with the target. Target-assigned rescue collateral invalidates the quote, but an unsolicited pool-wide REP transfer does not: otherwise one atomic unit would buy a repeatable liquidation veto. A donation large enough to restore live health still stops execution through the safety check. | [OpenOraclePriceCoordinator.sol](../../solidity/contracts/peripherals/OpenOraclePriceCoordinator.sol), [SecurityPool.sol](../../solidity/contracts/peripherals/SecurityPool.sol) |
...
| `EscalationGameDepositDelegate`, `EscalationGameClaimDelegate`, `EscalationGameForker`, `SecurityPoolForkerVaultMigrationDelegate`, and `SecurityPoolLiquidationDelegate` | Their public methods are delegatecall implementations. The deposit delegate implements `recordDeposit`, `recordForkedEscrowForOutcome`, `applyTruthAuctionHaircut`, funding-gated `resumeFromFork`, `consumeEscrowedRepForOwner`, `consumeUnresolvedRepForClaimOwners`, `creditClaimOwners`, and `creditExternalClaimOwners`. The shared claim delegate handles bounded checkpoint import, retention reads, and liquidation-only claim movement. The forker delegates implement escalation and vault migration. The pool helper implements bonus-priced REP liquidation, proportional claim movement, fee isolation, and permissionless bounded continuation progress. Direct calls operate on isolated or uninitialized helper storage and normally revert; they are not protocol actions. | Accept resulting state and logs only when the recognized game, pool, or forker executes the matching wrapper through `delegatecall`. State changes and events then occur in that recognized contract's context. Canonical liquidation enters through `SecurityPool.performLiquidation`; continuation progress enters through `SecurityPool.resumeForkedEscalationGame`. |
...
These external pure previews use REP base units, wei-denominated allowance, REP-per-ETH price scaled by `PRICE_PRECISION`, and basis-point multipliers. They have no lifecycle, caller, freshness, or canonical-pool authority; only a recognized pool or coordinator transaction can apply their result.

| Function | Exact boundary behavior | Conceptual owner |
| --- | --- | --- |
| `calculateFeeAccrual(collateral, retentionRate, timeDelta, indexRemainder, eligibleAllowance, feesOwedRemainder)` | Applies fixed-point retention over elapsed time, carries the global index remainder, and returns whole credited fees plus the next fee remainder. Canonical callers avoid zero eligible allowance before this division. | [Fee accrual](../whitepapers/statoblast-whitepaper.html#pools) |
| `calculateVaultFee(allowance, feeIndexDelta, remainder)` | Floors the vault's whole fee credit and returns the remaining fixed-point numerator for its next checkpoint. | [Fee accrual](../whitepapers/statoblast-whitepaper.html#pools) |
| `calculateBundledLiquidationTransfer(targetOwnership, targetAllowance, requestedDebt, repEthPrice, currentTotalRep, currentDenominator)` | Returns zero when ownership or allowance is zero; caps debt at allowance; ceilings the partial 5%-bonus REP quote, then floors quote-to-ownership and ownership-to-reported-REP conversions. Debt or REP dust promotes the transfer to a full close, which moves the exact remaining ownership. | [Bonus-priced liquidation](../protocol-design/liquidation.html#punitive-liquidation) |
| `isVaultHealthy(freeRep, escalationRep, securityBondAllowance, repEthPrice, poolSecurityMultiplierBps)` | Ordinary associated-REP equality is healthy. Positive allowance additionally requires strict free-REP migration coverage, so migration equality is unhealthy; zero allowance bypasses that second comparison. | [Two health conditions](../protocol-design/liquidation.html#rule) |
| `isLiquidationBeyondMinPriceDistance(freeRep, escalationRep, securityBondAllowance, poolSecurityMultiplierBps, currentPrice, minPriceDistanceBps)` | Zero configured distance returns true. Otherwise zero allowance or price returns false; the function floors both price thresholds, selects the lower one, requires price strictly above it, and then applies the BPS distance floor. | [OpenOracle liquidation guard](../protocol-design/open-oracle-integration.html#eq-openoracle-contract-liquidation-guard) |
| `calculateRetentionRate(completeSetCollateralAmount, securityBondAllowance)` | Zero allowance returns the maximum retention rate. Otherwise retention decreases linearly through the 80% utilization dip and remains at the minimum rate above it. | [Retention rate](../whitepapers/statoblast-whitepaper.html#pools) |
...
| Escalation game composition | [`EscalationGame.sol`](../../solidity/contracts/peripherals/EscalationGame.sol), [`EscalationGameCalculations.sol`](../../solidity/contracts/peripherals/EscalationGameCalculations.sol), [`EscalationGameCarry.sol`](../../solidity/contracts/peripherals/EscalationGameCarry.sol), [`EscalationGameClaimDelegate.sol`](../../solidity/contracts/peripherals/EscalationGameClaimDelegate.sol), [`EscalationGameDepositDelegate.sol`](../../solidity/contracts/peripherals/EscalationGameDepositDelegate.sol), [`EscalationGameEscrow.sol`](../../solidity/contracts/peripherals/EscalationGameEscrow.sol), [`EscalationGameSettlement.sol`](../../solidity/contracts/peripherals/EscalationGameSettlement.sol), [`EscalationGameState.sol`](../../solidity/contracts/peripherals/EscalationGameState.sol), [`EscalationGameStorage.sol`](../../solidity/contracts/peripherals/EscalationGameStorage.sol) | Splits the game across immutable ownership and storage, claim and deposit delegation, calculations, continuation proofs, escrow, and settlement without exposing those internal modules as separate canonical games. |
