...
| Continuation withdrawal | Anyone may relay a batch of winning carried proofs for one claim bundle after a child continuation resolves. A proof authenticates the original deposit and consumes its leaf once; its stable identity selects the creating game's normalized retention checkpoint, while payout follows the current child's consolidated bounded owner checkpoint. One mantissa/exponent ratio applies every intervening auction haircut to both proof amount and cumulative reward position without walking ancestors, and excludes losses before a continuation-local deposit was created. Inherited losing outcomes retire in constant-size work when the result is final and require no proof transaction; locally created losing deposits retain ordinary settlement. Before resume, anyone may advance payout-checkpoint import through the operational child pool in batches of at most eight bundle keys. Liquidation then processes only the current game's at-most-64 keys per vault, so recursive depth is never an operator liveness limit. | [`SecurityPool.withdrawForkedEscalationDeposits`](../../solidity/contracts/peripherals/SecurityPool.sol), [`EscalationGameSettlement.withdrawDeposit`](../../solidity/contracts/peripherals/EscalationGameSettlement.sol), [`EscalationGameCarry._getEffectiveInheritedUnresolvedTotal`](../../solidity/contracts/peripherals/EscalationGameCarry.sol) |
...
| Liquidation snapshot | A staged liquidation becomes stale if target ownership or allowance changes in either direction, and pool execution rejects a changed REP-per-ownership rate using a full-width cross-product comparison. The coordinator's distance guard uses the exact queue-time free REP and locally attributed escalation REP; after equality checks pass, the pool re-evaluates safety from live balances and current-game escrow before moving the quoted proportional bundle. Exact invalidation prevents a keeper from turning a small unsafe quote into an option on later rescue collateral, while unrelated deposits that preserve the exact rate do not invalidate it. | [OpenOraclePriceCoordinator.sol](../../solidity/contracts/peripherals/OpenOraclePriceCoordinator.sol), [SecurityPool.sol](../../solidity/contracts/peripherals/SecurityPool.sol) |
