...
| Continuation withdrawal | Anyone may relay a batch of winning carried proofs for one claim bundle after a child continuation resolves. A proof authenticates the original deposit and consumes its leaf once; a packed continuation-local index identifies the exact creating registry, while a legacy unencoded root index follows the bounded immediate-source chain. Payout follows that registry's bounded current owners; only a legacy source without an owner registry falls back to the original depositor. Recursive settlement also walks that bounded source chain to apply every auction haircut that occurred after the proof entered carry. It excludes the encoded creating game's earlier auction for a continuation-local proof, so new post-resume REP is not charged retroactively. The proof amount and cumulative reward position use the same cumulative ratio. Inherited losing outcomes retire in constant-size work when the result is final and require no proof transaction; locally created losing deposits retain ordinary settlement. Liquidation can process the current game plus seven ancestors; a ninth required registry makes the already staged coordinator operation fail and be consumed, so operators must resolve or avoid unsupported deeper claim lineages before queueing. | [`SecurityPool.withdrawForkedEscalationDeposits`](../../solidity/contracts/peripherals/SecurityPool.sol), [`EscalationGameSettlement.withdrawDeposit`](../../solidity/contracts/peripherals/EscalationGameSettlement.sol), [`EscalationGameCarry._getEffectiveInheritedUnresolvedTotal`](../../solidity/contracts/peripherals/EscalationGameCarry.sol) |
...
| Liquidation snapshot | A staged liquidation becomes stale if target ownership decreases or target allowance changes. Increasing ownership does not invalidate it. The coordinator's distance guard uses queue-time free REP and queue-time locally attributed escalation REP. The pool then tests the target with that snapshotted free REP plus live current-game escrow, while the transferred bundle uses current ownership, current pool totals, and current claim owners. A later REP top-up therefore does not guarantee rescue and can move in a full close. The proportional bundle may leave the target unsafe after a partial slice, but the caller must remain healthy and both resulting positions must respect minimum REP and allowance floors. | [OpenOraclePriceCoordinator.sol](../../solidity/contracts/peripherals/OpenOraclePriceCoordinator.sol), [SecurityPool.sol](../../solidity/contracts/peripherals/SecurityPool.sol) |
...
| `EscalationGameDepositDelegate`, `EscalationGameForker`, `SecurityPoolForkerVaultMigrationDelegate`, and `SecurityPoolLiquidationDelegate` | Their public methods are delegatecall implementations. The game delegate implements `recordDeposit`, `recordForkedEscrowForOutcome`, `moveEscalationClaim`, `applyTruthAuctionHaircut`, `resumeFromFork`, `consumeEscrowedRepForOwner`, `consumeUnresolvedRepForClaimOwners`, `creditClaimOwners`, and `creditExternalClaimOwners`; guarded game wrappers retain caller authority. The other delegates implement `claimForkedEscalationDeposits` and `migrateVaultWithUnresolvedEscalation`; `createChildUniverse`, `migrateVault`, `ensureChildPoolRepSplit`, and `finalizeTruthAuctionRepair`; and `performBundledLiquidation`, respectively. Each pool constructs its own immutable liquidation helper. Direct calls operate on a helper's isolated or uninitialized storage and normally revert; they are not protocol actions. | Accept resulting state and logs only when the recognized game, pool, or forker executes the matching wrapper through `delegatecall`. State changes and events then occur in that recognized contract's context. Canonical liquidation enters through `SecurityPool.performLiquidation`. |
