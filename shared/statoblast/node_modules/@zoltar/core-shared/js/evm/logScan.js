...
function walkErrorCauses(error, visit) {
    const seen = new Set();
    let current = error;
...
export function logRangeLimitError(error) {
    return walkErrorCauses(error, current => {
        if (!('message' in current) || typeof current.message !== 'string')
            return false;
        const message = current.message.toLowerCase();
        if (message.includes('fromblock exceeds toblock'))
            return false;
        if (message.includes('http 413') || message.includes('range is too large'))
            return true;
        if (message.includes('response exceeds') || message.includes('response too large'))
            return true;
...
export async function fetchLogsWithAdaptiveRanges(fromBlock, toBlock, maximumRange, fetchRange) {
    if (maximumRange < 1n)
