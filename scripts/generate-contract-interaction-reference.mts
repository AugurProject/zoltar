...
const eventSourceByName: Record<string, string> = {
	...
	PriceCandidateFinalized: 'solidity/contracts/peripherals/OpenOraclePriceCoordinator.sol',
	PriceCandidateStaged: 'solidity/contracts/peripherals/OpenOraclePriceCoordinator.sol',
	PriceConsumed: 'solidity/contracts/peripherals/OpenOraclePriceCoordinator.sol',
...
const contractReferences: ContractReference[] = [
	...
	},
	{
		...
		interactions: [
			...
			},
			{
				...
				caller: 'Vault owner',
				effect: "Deploys the local game on the first deposit, removes enough vault ownership, and escrows accepted REP on the selected outcome. Escrowed REP enters the slashable resolution game rather than the vault owner's wallet.",
				declarations: [{ name: 'depositToEscalationGame' }],
				preconditions: 'Question end has passed; pool operational in an unforked universe and not awaiting continuation; when live security-bond allowance is nonzero, timestamp-only `isPriceValid()` is true; outcome and amount accepted; remaining vault and pool backing stay solvent.',
			...
			},
			{
				...
				effect: "Executes the corresponding solvency-sensitive vault operation after the coordinator has enforced the accepted report's single-use exposure capacity.",
				declarations: [{ name: 'performWithdrawRep' }, { name: 'performLiquidation' }, { name: 'performSetSecurityBondsAllowance' }],
				preconditions: 'Operational, unresolved pool in an unforked universe; coordinator price is usable and its report security is unconsumed; operation-specific solvency and snapshot checks pass.',
	...
	},
	{
		...
		interactions: [
			...
			},
			{
				...
				effect: 'Withdraws finalized bid settlements, converts purchased REP into child-pool ownership, and independently credits the bid positional allowance share. A winning dust bid can receive positive allowance when its REP share rounds to zero.',
	...
	},
	{
		...
		purpose: 'Obtains an economically validated REP-per-ETH price and gates withdrawal, allowance, and liquidation operations behind its single-use report security.',
		readSurface:
			'Use timestamp-only `isPriceValid`, operation-facing `isPriceUsable`, `minimumToken1Report`, request-cost getters, candidate fields, accepted WETH and REP exposure, pending report fields, `getPendingOperationSlot`, active-operation pagination, and pending-settlement IDs to reconstruct oracle and operation state.',
		...
		interactions: [
			{
				...
				effect: 'Records the operation, executes immediately against unused accepted report security, or gives the first queued operation the single pending settlement slot and opens a report when required.',
				declarations: [{ name: 'requestPriceIfNeededAndStageOperation' }],
				preconditions:
					"Unresolved pool; valid target and nonzero amount except zero allowance; timeout from 1 second through 5 minutes. Bounty, buffered funding for at least the inclusion-floor-based WETH minimum and coordinator-derived REP side, and approvals are required only when this call opens a new report; the caller may request a larger initial WETH amount. Later operations remain staged and cannot share the first operation's report-security budget.",
			...
			},
			{
				...
				preconditions:
					'`isPriceUsable()` is false; no pending report or candidate; nonzero proposed REP/ETH price, ETH bounty, and funding and approvals for at least the inclusion-floor-based WETH minimum plus matching REP. A timestamp-valid price whose operation capacity was consumed is replaceable. Zero requested WETH uses the minimum; a larger request voluntarily increases the initial report.',
				signals: '`PriceRequested` and `CoordinatorStateCheckpoint`',
			},
			{
				call: '`executeStagedOperation(operationId)`',
				caller: 'Anyone',
				effect: 'Consumes and attempts one active staged operation whose WETH- or REP-denominated exposure fits the accepted report correction-profit capacity. A successful positive-exposure operation consumes both capacities, preventing cached-price reuse.',
				declarations: [{ name: 'executeStagedOperation' }],
				preconditions: 'Operation exists and `isPriceUsable()` is true; lifecycle or exposure failures are emitted rather than retried.',
				signals: '`ExecutedStagedOperation`, `PriceConsumed` after success, and `CoordinatorStateCheckpoint`',
			},
			{
				call: '`recoverSettledPendingReport()`',
				caller: 'Anyone',
				effect: 'Recovers a settled report whose callback failed and stages the same economically unvalidated candidate as the normal callback path.',
				...
				signals: '`PriceCandidateStaged`, `PendingReportRecovered`, and `CoordinatorStateCheckpoint`',
			},
			{
				call: '`openOracleCallback(...)`',
				caller: 'Configured `OpenOracle` only',
				effect: 'Authenticates the finalized OpenOracle game, returns reporter balances, clears pending-report state, and stages a `SettledPriceCandidate`; it does not activate a price or execute an operation.',
				declarations: [{ name: 'openOracleCallback' }],
				preconditions: 'Callback report, amounts, and settlement timestamp match the pending report and stored finalized game.',
				signals: '`PriceCandidateStaged` and `CoordinatorStateCheckpoint`',
			},
			{
				call: '`finalizeSettledPrice(opportunityHeaders, firstClosedHeader)`',
				caller: 'Anyone able to submit recent canonical execution headers',
				effect: 'Authenticates the final consecutive dispute-opportunity blocks and first closed block with `blockhash`, models the highest required inclusion cost, and either activates bounded WETH/REP operation capacity or rejects the candidate. Malformed proofs revert without consuming the candidate.',
				declarations: [{ name: 'finalizeSettledPrice' }],
				preconditions:
					'A candidate is pending; headers cover the configured final opportunity-block count and next block, remain inside the recent blockhash window, satisfy dispute timing and block gas-capacity checks, and produce sufficient correction profit under the configured total-gas-price, priority-fee, and absolute-premium floors.',
				signals: '`PriceReported` or `PriceReportRejected`, `PriceCandidateFinalized`, and `CoordinatorStateCheckpoint`',
			},
			{
				call: '`rejectExpiredPriceCandidate()`',
				caller: 'Anyone',
				effect: 'Rejects an unfinalized candidate and pays its finalizer reward once proof authentication or the price-validity window has expired.',
				declarations: [{ name: 'rejectExpiredPriceCandidate' }],
				preconditions: 'A candidate is pending and either its configured blockhash proof window or five-minute price-validity window has expired.',
				signals: '`PriceReportRejected` and `CoordinatorStateCheckpoint`',
			},
			{
	...
	},
	{
		...
		interactions: [
			...
			},
			{
				...
				effect: 'Returns refunds, purchased REP, and a companion pro-rata allocation for the selected beneficiary bids so the forker can credit pool ownership and allowance. Withdrawal-time allocation assigns division dust from deterministic cumulative ETH positions, making each payout independent of claim order.',
