...
const contractReferences: ContractReference[] = [
	...
	},
	{
		...
		securityBoundary:
			'Price-sensitive withdrawal, allowance, and liquidation calls depend on [A16 timely inclusion](./security-model.html#assumption-a16), [A22 external token behavior](./security-model.html#assumption-a22), [A19 observable correctable price](./security-model.html#assumption-a19), and [A06 lifecycle executors](./security-model.html#assumption-a06). User-initiated pool calls additionally depend on [A29 account authority](./security-model.html#assumption-a29).',
		readDeclarations: [
		...
		interactions: [
			...
			},
			{
				...
				preconditions:
					"Operational and unforked; `isEscalationResolved()` is false; not awaiting continuation; positive ETH converts to at least one complete-set unit; vault-assigned, fee-eligible bond capacity covers the new collateral (the pool's resulting total collateral, not merely this deposit); under [A23 asset-recipient compatibility](./security-model.html#assumption-a23), a contract trader accepts `onERC1155BatchReceived`.",
	...
	},
	{
		...
		securityBoundary:
			'Report and staged-operation liveness depends on [A16 timely inclusion](./security-model.html#assumption-a16), [A17 available arbitrager](./security-model.html#assumption-a17), [A18 independent corrector](./security-model.html#assumption-a18), [A19 observable correctable price](./security-model.html#assumption-a19), and [A06 lifecycle executors](./security-model.html#assumption-a06). An official-client proposal made when `lastPrice` is zero and the client must request an offchain market quote additionally depends on [A20 representative client quotes](./security-model.html#assumption-a20); proposals copied from a nonzero cached price do not.',
		readDeclarations: [
			{ name: 'isPriceValid' },
	...
	},
	{
		...
		interactions: [
			...
			},
			{
				...
				preconditions: 'Caller is authorized; `account` is nonzero; `amount` is positive; under [A23 asset-recipient compatibility](./security-model.html#assumption-a23), a contract account accepts `onERC1155BatchReceived`.',
...
The external boundaries most directly affecting these interactions are [A11 practical migration](./security-model.html#assumption-a11), [A16 timely inclusion](./security-model.html#assumption-a16), [A22 external token behavior](./security-model.html#assumption-a22), [A24 verified deployments](./security-model.html#assumption-a24), [A06 lifecycle executors](./security-model.html#assumption-a06), [A21 chain data and proof availability](./security-model.html#assumption-a21), [A27 Ethereum execution](./security-model.html#assumption-a27), [A28 cryptographic security](./security-model.html#assumption-a28), [A23 asset-recipient compatibility](./security-model.html#assumption-a23), [A25 client, RPC, and wallet integrity](./security-model.html#assumption-a25), and [A29 account authority](./security-model.html#assumption-a29).
