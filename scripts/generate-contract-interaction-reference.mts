...
const expectedProductionSoliditySourceFingerprint = 'f2128388d2ac3bdd0ae6b83a2b85c807500710d1f981085322f6f92dd2a38927'
...
const referencedEventAbiFingerprint = '5460fde219ab46fb9f87e20f4b95d8a7d48da5751856ba815b5f495ba2c5251b'
...
const entrypointSignaturesBySource: Record<string, Record<string, string[]>> = {
	...
	'solidity/contracts/peripherals/OpenOraclePriceCoordinator.sol': {
		...
		stageAndRequestOperationBounty: ['external(uint256,address,address,OperationType,address,uint256,uint256,uint256,uint256)'],
	},
	'solidity/contracts/peripherals/OpenOracleOperationBountyBoard.sol': {
		...
		deploy: ['external(OpenOraclePriceCoordinator,ReputationToken,IWeth9,bytes32)'],
		initialize: ['external(OpenOraclePriceCoordinator,ReputationToken,IWeth9)'],
		postOperationBounty: ['external(OperationType,address,uint256,uint256,address,uint256,uint256,uint256,uint256)'],
		recordOperationResult: ['external(uint256,bool)'],
...
const stateChangingAbiFingerprintBySource: Record<string, string> = {
	...
	'solidity/contracts/peripherals/OpenOraclePriceCoordinator.sol': '2cf0bda2841046d295f4ba60a5a6d70393c5f678b1ab0453f97bb8a28e408820',
...
const readDeclarationExclusionsBySource: Record<string, string[]> = {
	...
	'solidity/contracts/peripherals/OpenOracleOperationBountyBoard.sol': ['storedGame'],
...
const contractReferences: ContractReference[] = [
	...
	},
	{
		...
		readAbiFingerprint: '41f3d91e7c93867012ef0372c603c6305208524cf8df9149073c23b9f1bd43cc',
		readSurface:
		...
		interactions: [
			...
			{
				call: '`stageAndRequestOperationBounty(bountyId, sponsor, creator, operation, targetVault, ...)`',
				caller: 'The coordinator’s configured `operationBountyBoard` only',
				effect: 'Stages a self-receiving creator operation while assigning initial-report token funding, report sponsorship, and unused ETH refunds to the accepting operator. The operation result is returned to the board before the coordinator emits its execution event.',
				declarations: [{ name: 'stageAndRequestOperationBounty' }],
				preconditions: 'No nested bounty staging call is active; the creator and target satisfy the normal operation route; and a stale price has room in the four-operation pending settlement batch.',
				signals: '`StagedOperationQueued`, possibly `PriceRequested`, then `ExecutedStagedOperation`; authoritative `CoordinatorStateCheckpoint` records',
			},
			{
			...
			},
			{
				...
				effect: 'Binds the coordinator-local operation bounty board once.',
				declarations: [{ name: 'setOperationBountyBoard' }],
				preconditions: 'Board has deployed code and no board was previously installed.',
	...
	{
		compiledAbiFingerprint: '46b37ce4b96591befccadb0f39f90495131153c75924d48fd58686fb3282215a',
		...
		interactions: [
			{
				...
				signals: 'No event; the factory completes initialization before returning the proxy.',
			},
			{
				call: '`postOperationBounty(operation, targetVault, amount, validForSeconds, rewardToken, rewardAmount, acceptanceDeadline, minimumInitialAttoWeth, maximumInitialAttoWeth)`',
			...
			},
			{
				...
				signals: '`OperationBountyAccepted`; coordinator staging and reporting events',
			},
			{
				call: '`claimOperationBounty(bountyId)`',
				caller: 'Assigned operator only',
				effect: 'Marks the bounty paid and transfers its escrowed reward to the operator.',
				declarations: [{ name: 'claimOperationBounty' }],
				preconditions: 'Bounty remains assigned and its recorded execution status is succeeded.',
			...
			{
				call: '`recordOperationResult(bountyId, success)`',
				caller: 'Bound `OpenOraclePriceCoordinator` only',
				effect: 'Records the assigned bounty as succeeded or failed so its escrow can be claimed or refunded.',
				declarations: [{ name: 'recordOperationResult' }],
				preconditions: 'Bounty remains assigned.',
				signals: 'The coordinator emits `ExecutedStagedOperation` with the same result and any failure detail.',
			},
			{
				call: '`deploy(coordinator, reputationToken, weth, salt)`',
				caller: 'Owning price-coordinator factory only',
				effect: 'Lazily deploys one shared board implementation, then deterministically deploys and initializes a minimal proxy for the coordinator and token pair.',
				declarations: [{ name: 'deploy' }],
				preconditions: 'Caller equals the factory `owner`; the derived CREATE2 address is unused.',
				signals: 'No dedicated event; the coordinator subsequently emits `OperationBountyBoardSet`.',
			},
		],
	},
	{
