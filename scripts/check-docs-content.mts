...
assert.match(openOracleIntegration, /Candidate finalization converts the settled[\s\S]{0,100}amounts supplied through the callback/)
assert.doesNotMatch(openOracleIntegration, /coordinator converts the callback amounts/)
assert.match(whitepaper, /<h2>11\. Parameter Sources<\/h2>/)
assert.match(whitepaper, /open-oracle-integration\.html#parameters/)
assert.match(whitepaper, /auction-design\.html#lifecycle/)
...
assert.match(operatorReference, /auction-design\.html#clearing/)
...
assert.match(invariantsHtml, /<code>ORA-01<\/code>[\s\S]{0,900}cannot stage a settled-price candidate/)
assert.match(invariantsHtml, /<code>ORA-02<\/code>[\s\S]{0,1200}<code>finalizeSettledPrice<\/code>/)
assert.match(invariantsHtml, /<code>ORA-03<\/code>[\s\S]{0,700}<code>isPriceValid\(\)<\/code> is a timestamp-only check[\s\S]{0,700}remains timestamp-valid[\s\S]{0,300}<code>ORA-09<\/code>/)
assert.doesNotMatch(invariantsHtml, /four minutes and 59 seconds ago remains usable/)
assert.doesNotMatch(whitepaper, /staged callback queue/)
...
assert.match(protocolTerms, /A delayed action that cannot run until the coordinator has an accepted REP\/ETH price with sufficient unconsumed native capacity\./)
assert.match(protocolTerms, /isPriceValid: 'The coordinator timestamp-only check/)
assert.match(protocolTerms, /isPriceUsable: 'The coordinator check requiring both timestamp-only price validity and nonzero accepted WETH or REP operation capacity\.'/)
