...
assert.match(whitepaper, /After the migration window ends, each undercollateralized child pool may run its own <a href="\.\/auction-design\.html#lifecycle">truth auction<\/a>[\s\S]{0,160}as much repair ETH as demand supports/)
assert.doesNotMatch(whitepaper, /class="subtitle"[\s\S]{0,180}\bcensorship-resistant\b/)
assert.match(zoltarWhitepaper, /any address able to commit the threshold REP[\s\S]{0,120}any eligible ended global question/)
assert.match(zoltarWhitepaper, /applications that choose to continue from that forked universe/)
assert.match(zoltarWhitepaper, /data-rounding="integer-flooring"/)
assert.match(zoltarWhitepaper, /≈5% of theoretical REP supply[\s\S]{0,900}aria-label="approximately 20 percent uncredited haircut"[\s\S]{0,420}aria-label="approximately 80 percent migration balance"/)
...
assert.match(whitepaper, /A Zoltar fork first freezes or redirects fork-guarded parent flows[\s\S]{0,180}Statoblast fork handling must then be initiated separately/)
assert.match(whitepaper, /pool's own local non-decision or an unrelated external Zoltar fork/)
