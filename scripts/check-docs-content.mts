...
assert.match(whitepaper, /Every selected continuation receives the complete parent game snapshot\.[\s\S]{0,320}complete aggregate backing at 1:1 once/)
...
assert.match(operatorReference, /\| Aggregate escalation backing \| An external fork locks the drained game's complete REP[\s\S]*?one-for-one[\s\S]*?own fork reserves the complete drained escalation-game REP at 1:1/)
...
assert.match(whitepaper, /1:1 rule applies to fork conversion and aggregate backing, not\s+to the reward formula/)
...
assert.match(whitepaper, /escalationChildRepAtFork = escalationRepToFork; vaultRepAtFork = auctionableRepAtFork - escalationRepToFork; selectedChildEscalationBacking = escalationRepToFork/)
