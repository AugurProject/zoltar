...
function assertContractInteractionDistinctions(): void {
	...
	assert.match(securityPool, /function redeemShares\(\)[\s\S]*shareTokenSupply = remainingWinningShareSupply/)
	assert.match(securityPoolForker, /securityPool\.setTotalShares\(securityPool\.shareToken\(\)\.maximumOutcomeSupply\(securityPool\.universeId\(\)\)\)/)
	assert.match(contractInteractionReference, /redeemCompleteSet\(completeSetAmount\)[\s\S]*completeSetAmount \* completeSetCollateralAmount \/ shareTokenSupply[\s\S]*truth-auction start seeds it from `maximumOutcomeSupply`[\s\S]*each later `redeemShares` replaces it with the remaining winning-outcome supply/)
	assert.doesNotMatch(contractInteractionReference, /redeemCompleteSet\(completeSetAmount\)[\s\S]*largest live outcome supply/)
	assert.match(whitepaperPlaceholder, /payout uses the pool's current stored\s*<code>shareTokenSupply<\/code> denominator[\s\S]*truth-auction setup\s*seeds that value from the maximum outcome supply[\s\S]*winning-share\s*redemption replaces it with the remaining winning supply/)
	...
	assert.match(contractInteractionReference, /canonical source pool that is `Operational` or `PoolForked`[\s\S]*One missing target may be deployed lazily only for a single-target call; bulk migration requires every child pool to exist already/)
	assert.match(contractInteractionReference, /If the source pool is still operational, first initiates its pool fork/)
	...
	assert.match(shareToken, /_burn\(msg\.sender, fromId, fromIdBalance\);[\s\S]*_mint\(msg\.sender, toId, fromIdBalance\);[\s\S]*emit Migrate\(msg\.sender, fromId, toId, fromIdBalance\)/)
	...
	assert.match(contractInteractionReference, /migrate\(fromId, targetOutcomeIndexes\)[\s\S]*contract holder accepts `onERC1155Received` for every target mint[\s\S]*one ERC-1155 source-burn `TransferSingle`, followed by one mint `TransferSingle` and `Migrate` per target on successful callbacks/)
