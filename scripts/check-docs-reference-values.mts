...
function assertStartHereTimelines(): void {
	...
	assert.match(startHere, /whitepaper_placeholder\.html#fig-placeholder-recursive-continuation/)
...
function assertContractInteractionDistinctions(): void {
	...
	assert.match(contractInteractionReference, /Withdrawal-time allocation carries division dust through `clearingRemainder`/)
	...
	assert.match(truthAuction, /clearingRemainder = numerator % (?:clearingPriceLocal|totalEth)/)
