...
function assertContractInteractionDistinctions(): void {
	...
	assert.match(priceCoordinator, /function requestPrice\([\s\S]*if \(excess > 0\) \{[\s\S]*payable\(msg\.sender\)\.call\{\s*value:\s*excess\s*\}\(''\)[\s\S]*require\(sent, ETH_REFUND_FAILED\)/)
	assert.match(priceCoordinator, /function requestPriceIfNeededAndStageOperation\([\s\S]*if \(refund > 0\) \{[\s\S]*payable\(reportSponsor\)\.call\{\s*value:\s*refund\s*\}\(''\)[\s\S]*require\(sent, ETH_REFUND_FAILED\)/)
	...
	assert.match(priceCoordinator, /require\(\s*reportSponsor == pendingReportSponsor,\s*'Only the pending report sponsor can queue more operations until settlement'/)
	...
	assert.match(priceCoordinator, /_initialReportPriorityFeeAttoEthPerGas <= maximumInitialReportPriorityFeeAttoEthPerGas, INVALID_ORACLE_CONFIGURATION/)
