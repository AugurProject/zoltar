...
function assertInvariantCatalogLifecycleBoundaries(): void {
	...
	assert.match(vaultEntry, /append-only[\s\S]*contains each address at most once[\s\S]*full exit do not reorder or remove an entry[\s\S]*Consumers read current vault and escalation state/)
...
function assertContractInteractionDistinctions(): void {
	...
	assert.match(invariantsHtml, /id="vault-03"[\s\S]*vault registry is append-only[\s\S]*<code>_registerVault<\/code>/)
	...
	assert.match(operatorReference, /getVaults\(startIndex, count\).*append-only vault registry in newest-created-first order/)
