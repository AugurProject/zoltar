...
function assertMigrationSecurityCoverageCommitmentDocs(): void {
	...
	assert.match(contractInteractionReference, /exactly `max\(1 REP, live universe theoretical REP supply \/ 10,000,000\)`[\s\S]*zero configured vault REP floor[\s\S]*live universe theoretical REP supply \/ 100,000[\s\S]*nonzero constructor value is the exact override[\s\S]*security-bond debt floor defaults to 1 ETH/)
	assert.doesNotMatch(contractInteractionReference, /max\(1 REP, configured floor, theoretical REP supply \/ 10,000,000\)/)
	assert.match(operatorReference, /effective escalation deposit is exactly `max\(1 REP, live universe theoretical REP supply \/ 10,000,000\)`/)
...
function assertContractInteractionDistinctions(): void {
	...
	assert.match(contractInteractionReference, /`QuestionCreated` logs for discovery and categorical presentation metadata[\s\S]*`protocolQuestions`[\s\S]*`questions`[\s\S]*scalar metadata and validation/)
	assert.match(contractInteractionReference, /`QuestionData` encodes `startTime` and `endTime` as `uint48` and `numTicks` as `uint120`[\s\S]*determine the `getQuestionId` and `createQuestion` selectors/)
	assert.match(contractInteractionReference, /Categorical questions require at most `uint32\.max`[\s\S]*`numTicks`, both display bounds, and `answerUnit` must be zero or empty/)
	...
	assert.match(invariantsHtml, /id="esc-13"[\s\S]*max\(1 REP, live universe theoretical REP supply \/ 10,000,000\)[\s\S]*clamps the live start bond to[\s\S]*nonDecisionThresholdAttoRep - 1/)
	...
	assert.match(contractInteractionReference, /`forkBurnDivisor >= 5`[\s\S]*uncredited fork haircut at 20%/)
