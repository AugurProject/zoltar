...
function assertContractInteractionDistinctions(): void {
	...
	const sepoliaAllocationAmounts = [...sepoliaRepAllocations.matchAll(/amount: parseUnits\('([0-9]+)', 18\)/g)].map(match => BigInt(match[1] ?? ''))
	assert.deepStrictEqual(sepoliaAllocationAmounts, [3_000_000n, 3_000_000n, 3_000_000n])
	assert.match(operatorReference, /three listed holders each receive 3 million REP, for a 9 million REP starting supply/)
