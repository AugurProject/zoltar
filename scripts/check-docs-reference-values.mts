...
function assertContractInteractionDistinctions(): void {
	...
	assert.match(contractInteractionReference, /selected child can be created or loaded, remains in `ForkMigration`, has a continuation game whose immutable `securityPool\(\)` binding equals that child, and is inside the eight-week claim window/)
	...
	assert.match(securityPoolForkerVaultMigrationBase, /childEscalationGame = child\.escalationGame\(\);[\s\S]*childEscalationGame\.securityPool\(\)[\s\S]*'Child game pool'[\s\S]*_ensureChildEscalationBacking\(parent, outcomeIndex, child, childEscalationGame\)/)
	assert.match(escalationGameForker, /\(ISecurityPool child, EscalationGame childEscalationGame\) = _getOrDeployChildPool\([\s\S]*childEscalationGame\.recordForkedEscrowForOutcome\([\s\S]*childEscalationGame\.exportForkedEscrowByOutcome\(vault, vault\)/)
	for (const childPoolInteractionRow of [createChildUniverseRow, migrateVaultRow, migrateVaultWithUnresolvedEscalationRow]) {
		assert.match(childPoolInteractionRow, /nonzero escalation game[\s\S]*immutable `securityPool\(\)` binding[\s\S]*equal(?:s| to) (?:that|the selected) child/)
	}
	assert.match(claimForkedEscalationDepositsRow, /continuation game whose immutable `securityPool\(\)` binding equals that child/)
