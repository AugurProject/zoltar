...
test('chaos is a first-class package and follows bot-shared and Solidity dependencies', () => {
	expect(classifyCiChange(['bots/chaos/src/operator.ts']).packageMatrix.map(entry => entry.package)).toEqual(['chaos'])
	expect(classifyCiChange(['bots/shared/src/ethereum.ts']).packageMatrix.map(entry => entry.package)).toContain('chaos')
	expect(classifyCiChange(['solidity/contracts/Zoltar.sol']).packageMatrix.map(entry => entry.package)).toContain('chaos')
})
