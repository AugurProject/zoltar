...
for (const path of new TextDecoder().decode(sourceFilesResult.stdout).trim().split('\n')) {
	...
	if ((path.startsWith('docs/') || /^ui\/(?:coreShared|zoltar|statoblast)\/ts\/copy\//.test(path)) && ambiguousSettlementCollateralTerminology.test(source)) failures.push(`${path}: uses generic open-interest or parent collateral instead of settlement collateral`)
	if (/^ui\/(?:coreShared|zoltar|statoblast)\/ts\//.test(path) && uiEscrowAccountingAlias.test(source)) failures.push(`${path}: uses escrow mechanics terminology for the dispute-staked REP accounting state`)
	if (/^ui\/(?:coreShared|zoltar|statoblast)\/ts\//.test(path) && uiDirectRepTruthAuctionClaimAlias.test(source)) failures.push(`${path}: describes truth-auction REP backing units as direct child-pool REP`)
	...
	if (/^ui\/(?:coreShared|zoltar|statoblast)\/ts\//.test(path) && uiLegacyDisputeStakeCopy.test(source)) failures.push(`${path}: uses stake or escrow copy instead of dispute-staked REP`)
