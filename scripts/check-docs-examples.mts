...
async function checkReaderRuntimeStates(): Promise<void> {
	...
	try {
		...
		const deploymentDecoderLink = window.document.querySelector('.reader-nav-document[data-navigation-document-path="deployment-status.html"] a[data-document-fragment="deployment-mask-decoder"]')
		...
		const callbackLink = window.document.querySelector('.reader-nav-document[data-navigation-document-path="open-oracle-integration.html"] a[data-document-fragment="callback-rejection-and-recovery"]')
		...
		assert.equal(window.location.hash, '#doc-open-oracle-integration--callback-rejection-and-recovery', 'callback navigation must preserve its section fragment in reader history')
	} finally {
		window.close()
	}
