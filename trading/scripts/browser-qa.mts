...
const scenarios = [
	...
	},
	{
		...
		height: 844,
		path: '/?demo=1&scenario=baseline#/market',
		assertExpression: `(${poolInternalsHiddenAssertion}) && document.querySelector('.wallet-summary__address--compact')?.getBoundingClientRect().width > 0 && document.querySelector('.trade-summary')?.textContent?.includes('You receive') === true && document.querySelector('.pool-mechanics')?.textContent?.includes('Conditional YES price70.0%') === true && [...document.querySelectorAll('.segmented button, .brand, .eyebrow[href], details summary, .security-pool-link')].every(control => control.getBoundingClientRect().height >= 44) && document.documentElement.scrollWidth <= document.documentElement.clientWidth`,
	},
