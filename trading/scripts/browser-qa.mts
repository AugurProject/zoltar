...
const scenarios = [
	...
	{ name: 'market-desktop', width: 1440, height: 900, path: '/?demo=1&scenario=baseline#/market' },
	{
		...
		assertExpression: `[...document.querySelectorAll('.segmented button')].every(control => control.getBoundingClientRect().height >= 44)`,
	},
	...
	{ name: 'pair-missing-market-mobile', width: 390, height: 844, path: '/?demo=1&scenario=missing-pair#/market', scrollY: 500 },
	{
		...
		assertExpression: `document.body.textContent?.includes('Conditional price available after pair initialization') === true && document.body.textContent?.includes('Initialize this pair in Liquidity') === true && document.body.textContent?.includes('Conditional YES price 0.0%') !== true`,
	},
	{
		...
		height: 844,
		path: '/?demo=1&scenario=uninitialized-pair#/market',
		assertExpression: `document.body.textContent?.includes('Conditional price available after pair initialization') === true && document.body.textContent?.includes('Initialize this pair in Liquidity') === true`,
		scrollY: 500,
	},
