...
async function simulateSettlementWithExpiryParameters(
	...
	parameters: Readonly<{ amount?: bigint; deadline?: bigint; validityMinutes?: bigint; slippageBps?: bigint; sourceOutcome?: ShareOutcome; targetOutcomeIndex?: bigint }> = {},
) {
...
export async function simulateSettlement(
	...
	account: Address,
	operation: SettlementOperation,
	parameters: Readonly<{ amount?: bigint; validityMinutes?: bigint; slippageBps?: bigint; sourceOutcome?: ShareOutcome; targetOutcomeIndex?: bigint }> = {},
) {
	if (operation === 'redeem-complete-set') {
	...
	if (operation === 'migrate-shares') {
		const sourceOutcome = parameters.sourceOutcome
		const targetOutcomeIndex = parameters.targetOutcomeIndex
		return await simulateSettlementWithExpiryParameters(client, configuration, market, account, operation, { ...(sourceOutcome === undefined ? {} : { sourceOutcome }), ...(targetOutcomeIndex === undefined ? {} : { targetOutcomeIndex }) })
	}
...
export async function submitFreshSettlement(client: WalletClient, configuration: DeploymentConfiguration, account: Address, quote: Awaited<ReturnType<typeof simulateSettlement>>, guardedWrite: GuardedWalletWrite): Promise<Hash> {
	await requireQuoteBlock(client, quote)
	let parameters: Readonly<{ amount?: bigint; deadline?: bigint; validityMinutes?: bigint; slippageBps?: bigint; sourceOutcome?: ShareOutcome; targetOutcomeIndex?: bigint }> = {}
