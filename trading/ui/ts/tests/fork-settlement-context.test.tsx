...
import { LiveSettlementControls } from '../features/LiveTrading.tsx'
import type { DeploymentConfiguration } from '../protocol/config.ts'
import type { ForkMigrationContext } from '../protocol/forks.ts'
...
describe('live fork settlement context', () => {
	...
	test('retries failed fork metadata and refreshes branches after confirmed migration', async () => {
		...
		const target = document.querySelector('input[aria-label="Red fork branch"]')
		if (!(target instanceof HTMLInputElement)) throw new Error('Missing categorical fork target')
