...
function LiveSettlementControls({
	...
	async function submitCurrent() {
		...
		try {
			...
			broadcastHash = await submitFreshSettlement(
				walletClient,
				configuration,
				account,
				selectedQuote,
	...
	return (
		<div class='operation-block'>
			...
			{!(state === 'error' && error !== undefined) && (balanceState !== 'error' || state === 'confirmed' || state === 'approval-confirmed') && receiptWarning === undefined ? (
	...
	return (
		<div class='operation-block' aria-busy={balanceState === 'loading'}>
			...
			{!(mode === 'exit' && balances?.approved === false) && quote === undefined ? (
				<button class='primary-action' aria-busy={state === 'simulating'} disabled={closed || balanceState !== 'ready' || balances === undefined || parsedInput === undefined || parsedInput === 0n || exceedsInsurance || state === 'simulating' || workflowLocked} onClick={simulate}>
