...
const files = { '/': 'index.html', '/index.html': 'index.html', '/app.js': 'app.js', '/app.js.map': 'app.js.map', '/app.css': 'app.css', '/core-deployments.json': 'core-deployments.json', '/deployment.json': 'deployment.json' } as const
...
export function serveTradingAsset(request: Request) {
	...
	return new Response(Bun.file(path.join(root, file)), { headers: { ...securityHeaders, 'cache-control': file === 'core-deployments.json' || file === 'deployment.json' || file === 'index.html' ? 'no-store' : 'no-cache' } })
}
