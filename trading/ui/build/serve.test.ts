...
describe('trading UI asset server', () => {
	test('protects responses and prevents stale deployment configuration', () => {
		const deployment = serveTradingAsset(new Request('http://localhost:12346/deployment.json'))
		...
		const missing = serveTradingAsset(new Request('http://localhost:12346/missing'))
