...
Without `TRADING_UI_DEPLOYMENT`, the build writes `deployment.json` as `null` and writes `core-deployments.json` from the root mainnet and Sepolia deployment manifests, adding each network's default public RPC URL. The live setup screen defaults to a supported chain and its default RPC; the advanced configuration section accepts an immutable fee from 0 to 9999 basis points and an optional HTTPS or loopback HTTP RPC override. It computes the trading factory and router through the core deployment's canonical CREATE2 proxy, shows each contract's deployment status, verifies or deploys each contract through the wallet, and stores the completed configuration under `zoltar.trading.deployment.v1` in browser local storage.
