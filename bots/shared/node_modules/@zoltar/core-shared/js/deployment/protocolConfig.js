...
export const MAINNET_PROTOCOL_CONFIG = {
    forkBurnDivisor: 5n,
...
function parseConfigBigInt(value, field) {
    if (value === undefined)
        return undefined;
    ...
    if (typeof value === 'number') {
        if (!Number.isInteger(value))
            throw new Error(`Protocol config ${field} must be an integer`);
        return BigInt(value);
    }
...
function getEnvironmentProtocolConfigOverrides() {
...
export function validateProtocolConfig(config) {
    ...
    if (forkThresholdDivisor === undefined)
        throw new Error('Protocol config forkThresholdDivisor is required');
    if (forkBurnDivisor === undefined)
        throw new Error('Protocol config forkBurnDivisor is required');
    if (minimumSecurityBondDebtAttoEth === undefined)
        throw new Error('Protocol config minimumSecurityBondDebtAttoEth is required');
    if (minimumVaultRepDepositAttoRep === undefined)
        throw new Error('Protocol config minimumVaultRepDepositAttoRep is required');
    if (forkThresholdDivisor <= 1n)
        throw new Error('Protocol config forkThresholdDivisor must be greater than 1');
    if (forkBurnDivisor < 5n)
        throw new Error('Protocol config forkBurnDivisor must be at least 5');
    if (minimumSecurityBondDebtAttoEth <= 0n)
        throw new Error('Protocol config minimumSecurityBondDebtAttoEth must be positive');
    if (minimumVaultRepDepositAttoRep < 0n)
        throw new Error('Protocol config minimumVaultRepDepositAttoRep cannot be negative');
    return {
        forkBurnDivisor,
...
export function getProtocolConfig(overrides = {}) {
...
function collectProtocolConfigOverrideSources(overrides) {
...
export function assertMainnetProtocolConfigFrozen(overrides = {}) {
    for (const { config, source } of collectProtocolConfigOverrideSources(overrides)) {
        for (const field of Object.keys(MAINNET_PROTOCOL_CONFIG)) {
            const overrideValue = parseConfigBigInt(config[field], field);
            if (overrideValue === undefined)
                continue;
            if (overrideValue === MAINNET_PROTOCOL_CONFIG[field])
                continue;
...
export function getMainnetProtocolConfig(overrides = {}) {
