...
export function rpcConnectivityFields({ independentQuorum = false, submissionLimit, statusId }: { independentQuorum?: boolean; submissionLimit?: number; statusId: 'network-status' | 'connectivity-status' }) {
 ...
 </div><div class="form-actions"><span id="${statusId}" class="action-status muted" role="status" aria-live="polite">Every endpoint is checked against the selected chain before it is accepted.</span><button class="button" type="submit">Check &amp; apply network</button></div>`
}
