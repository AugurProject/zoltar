...
test('presents unavailable deployments and liquidity as informative status', () => {
	...
	expect(marketAvailabilityPresentation({ kind: 'no-execution-pools', chainId: 1 })?.title).toBe('No execution pools')
	...
	expect(botStatusLabels({ paused: false, status: 'error', mode: 'dry-run', marketAvailability: { kind: 'no-execution-pools', chainId: 1 } }).status).toBe('Error')
})
