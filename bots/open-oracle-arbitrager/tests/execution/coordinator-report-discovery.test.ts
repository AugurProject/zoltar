	...
	test('loads current report state at one block without querying event history', async () => {
		...
		const inner = multicallProvider(multicall3, ({ blockTag, data, to }) => {
			...
			if (decoded.functionName === 'disputeHistory') {
				return encodeAbiParameters([{ type: 'uint128' }, { type: 'uint128' }, { type: 'uint128' }, { type: 'uint48' }], [1_400n, 2_300n, 15n, 95n])
			}
		...
		expect(methods).toEqual(['eth_call', 'eth_call', 'eth_call'])
		expect(contractReads).toEqual(['pendingReportId', 'pendingReportId', 'storedGame', 'storedHelper', 'disputeHistory'])
