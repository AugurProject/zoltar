...
import { positionRiskLimitMismatch, projectedLifecycleGasReserveWethAttoEth } from '#core/safety-controls'
import {
	calculateFee,
...
export async function loadBalances(client: ReadClient, wallet: WriteClient | undefined, config: Configuration, pools: readonly Pool[], tokens: readonly Address[]) {
	...
	const [ethAttoEth, wethAttoEth, repAttoRep] = await Promise.all([
	...
	const raw = { ethAttoEth, repAttoRep, tokens: tokenBalances, wethAttoEth }
	let repValueWethAttoEth: bigint | undefined
	if (repAttoRep === 0n) repValueWethAttoEth = 0n
	else {
