...
export async function processPositionLifecycle(client: ReadClient, readClients: readonly ReadClient[], wallet: WriteClient, config: Configuration, position: PositionRecord, blockNumber: bigint, reportPath: ActiveReport | undefined, persistPosition: (position: PositionRecord) => Promise<void>, track: TrackTransaction) {
	...
	if (!currentReporter) {
		...
		const allowance = credit.token === 'token1' ? balancesBefore.internalAllowanceWethAttoEth : balancesBefore.internalAllowanceToken
		...
		const allowanceMismatch = lifecycleAllowanceMismatch({ token1: balancesBefore.internalAllowanceWethAttoEth, token2: balancesBefore.internalAllowanceToken }, { token1: expectedWethAttoEth, token2: expectedToken })
