...
import { validateConnectivitySettings, validateIndependentReadRpcUrls, type ConnectivitySettings } from '#monitoring/connectivity'
import { decimalWeth, parseDecimalWeth, updateStrategyFromRequest, type MutableStrategy } from '#state/operator-state'
import { defaultRpcUrl, networkConfiguration, parseNetworkName, type NetworkConfiguration } from '#config/network'
import { DEFAULT_RISK_LIMITS, type RiskLimits } from '#core/safety-controls'
...
const defaultStateDirectory = resolve(import.meta.dir, '..', '..', '.state')
...
Execution is off by default. See README.md.`)
}
...
export async function loadConfiguration(): Promise<Configuration> {
	...
	return {
		...
		historyFile: resolve(option('history-file') ?? resolve(defaultStateDirectory, `history-${networkName}.jsonl`)),
		...
		privateKey,
		positionFile: resolve(option('position-file') ?? resolve(defaultStateDirectory, `positions-${networkName}.json`)),
		quorumRpcUrls,
		riskLimits,
