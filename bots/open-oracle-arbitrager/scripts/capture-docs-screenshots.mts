...
async function captureScreenshots(chromium: string, origin: string, outputDirectory: string) {
	...
	try {
		...
		for (const [name, section] of [
			...
			const fragment = section === undefined ? 'overview' : section === 'operations' ? 'operations' : section === 'token-market-title' ? 'markets' : section === 'network-connectivity' || section === 'settings' || section === 'deployment-configuration' || section === 'create2-form' || section === 'complete-configuration' ? 'settings' : 'overview'
