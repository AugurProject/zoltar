...
The example relay hosts are deliberately unusable. A relay must report the configured chain and prove the configured signer's authenticated `eth_sendPrivateTransaction` path. The capability request carries the same non-broadcastable invalid-signature envelope and uses the exact production `x-flashbots-signature` algorithm; authentication rejection never counts as capability evidence. A transaction-specific parser rejection is sufficient. The [official Flashbots Sepolia relay](https://docs.flashbots.net/flashbots-auction/advanced/rpc-endpoint) instead returns the exact generic `-32600: incorrect request`, so that response counts only when the same endpoint also distinguishes a validly authenticated high-entropy unsupported method, accepts authenticated cancellation lookup for the impossible envelope's never-admitted hash as `tx not found`, and rejects the identical unauthenticated cancellation as `signature is required`. Every response, identifier, code, message, and HTTP disposition in that fallback is checked exactly and bounded; generic parameter, signature, gateway, or write-policy errors fail closed. `minimumBundleRelaySuccesses` is the number of distinct relay origins that must pass preflight and accept the transaction; multiple successful paths on one origin consume one threshold slot. Relay URLs must use HTTPS or loopback HTTP and cannot embed credentials, query parameters, or fragments.
