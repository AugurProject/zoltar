...
`/healthz` is unauthenticated process liveness and intentionally ignores pause/readiness state. `/readyz` and `/metrics` require dashboard authentication. Readiness scan age is bounded by three lifecycle polls plus two configured maximum block intervals; it does not inherit the random scheduler delay. Readiness fails for intentional or safety pause, a stale/incomplete scan, unhealthy quorum, a missing live signer, inventory below the full ETH/REP principal, reserve, and gas envelope, unresolved transaction/workflow recovery, pending/blocked/failed lifecycle work, a scheduled automatic retry, an unrepresented lifecycle-presence guard, an active runtime error, or an unreadable runtime snapshot. Metrics expose every named readiness check plus workflow, pending-transaction, obligation, retry, presence, pause, and scan-age gauges. Compose checks liveness so a deliberate safety stop does not create a restart loop; alerting should scrape authenticated readiness/metrics separately.
