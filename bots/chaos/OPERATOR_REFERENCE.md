...
The example relay hosts are deliberately unusable. A relay must report the configured chain and prove authenticated `eth_sendPrivateTransaction` support. `minimumBundleRelaySuccesses` is the number of distinct relay origins that must pass preflight and accept the transaction; multiple successful paths on one origin consume one threshold slot. Relay URLs must use HTTPS or loopback HTTP and cannot embed credentials, query parameters, or fragments.
