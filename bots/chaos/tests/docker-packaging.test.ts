...
describe('chaos Docker packaging', () => {
	test('provides a location-independent Windows launcher', async () => {
		...
		expect(commands).toContain('docker compose stop || goto failed')
		expect(commands.indexOf('docker compose stop || goto failed')).toBeGreaterThan(commands.indexOf('if /I "%~1"=="doctor" goto doctor'))
		expect(commands.indexOf('docker compose stop || goto failed')).toBeLessThan(commands.indexOf('docker compose build || goto failed'))
