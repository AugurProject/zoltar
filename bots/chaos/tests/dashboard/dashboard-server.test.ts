...
describe('chaos dashboard server', () => {
	test('serves five protected routes with hardened no-store responses', async () => {
		...
		expect(readiness.headers.get('content-type')).toBe('application/json; charset=utf-8')
		expect(readiness.headers.get('content-security-policy')).toContain("object-src 'none'")
		expect(readiness.headers.get('cross-origin-resource-policy')).toBe('same-origin')
		expect(readiness.headers.get('x-frame-options')).toBe('DENY')
