		...
		try {
			for (const viewport of [
				...
				for (const mode of ['keyless', 'read-only', 'signer'] as const) {
					...
					const expectedEthDisplay = mode === 'keyless' ? '—' : '1.000000000000000000'
					const expectedBadge = { keyless: 'Signer missing', 'read-only': 'Read-only — signer not loaded', signer: 'Signer ready' }[mode]
					const ready = `document.getElementById('balance-eth')?.textContent === ${JSON.stringify(expectedEthDisplay)} && document.getElementById('signer-badge')?.textContent === ${JSON.stringify(expectedBadge)}`
					for (let attempt = 0; attempt < 200; attempt += 1) {
