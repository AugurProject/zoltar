...
import { IMMUTABLE_TOPOLOGY_SEGMENT_BYTES, immutableTopologySidecarDirectory, loadImmutableTopologyCache, saveImmutableTopologyCache, type CanonicalImmutableTopologyCache, type ImmutableTopologyIdentity } from '../../src/monitoring/topology-cache.ts'
...
describe('immutable topology sidecar', () => {
	test('round-trips more than ten thousand immutable records through checksummed fixed-size segments', async () => {
		...
		const segments = generationEntries.filter(name => name.startsWith('segment-'))
