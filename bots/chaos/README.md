...
At this keyless, dry stage, `doctor` validates the configuration, network, deployment, and any durable state already present. Because no signer is configured, it does **not** validate signer inventory or funding and does **not** acquire a signer-scoped lock. A pass means the keyless bootstrap is coherent; it is not signer-aware live readiness. The full stopped preflight for a saved signer comes in step 6.
