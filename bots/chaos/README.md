...
When no drain is active for an operated old profile, the launcher derives the retirement destination from the configured private key and automatically requests retirement without terminal input. It then starts the old pinned bot with the default retirement policies: recover assets to the signer wallet and unwrap WETH, without unmatched-share exit, claim migration, or automatic exit after completion. These defaults replace any policies retained from a cancelled drain. If a drain is already active, the launcher resumes it with its saved policies.

The old configuration must already permit unpaused live execution with a configured signer and readers satisfying its configured `rpcQuorum`; the launcher does not turn execution on. Retirement completion uses that same quorum: `rpcQuorum: 1` permits a single trusted reader, while `rpcQuorum: 2` requires agreement from two independent readers. Completion must still be finalized and canonical on the configured chain. The launcher checks retirement every minute and switches to the current addresses only after verifying completion, using a new unused state file and preserving the old one.
