...
After resume, `/readyz` returns HTTP 200 only while the bot is ready and idle. Its named `submission` check proves recent `eth_sendRawTransaction` dispatch or current-signer-authenticated private-relay threshold evidence using a fixed transaction envelope whose impossible signature cannot enter a mempool. A private relay's transaction-parser rejection counts only when the identical body without authentication is explicitly rejected for missing authentication, a valid disposable-canary identity reaches the same parser response as the configured signer, and that same claimed canary with a valid signature from a different canary is rejected for invalid authentication. These controls hold the body and claimed canary constant while proving header presence and cryptographic identity binding before parsing. Flashbots' generic `incorrect request` compatibility profile is trusted only for the official relay matching the selected mainnet or Sepolia chain, or for a loopback test relay, and must pass exact configured-signer method-dispatch and harmless never-submitted cancellation controls; it is known-provider compatibility evidence, not an independent cryptographic negative proof. A generic gateway error is never enough. The runtime refreshes that evidence periodically and repeats the same preflight immediately before signing or resubmission. If due work starts immediately, the `recovery` check intentionally returns 503 while a transaction, workflow, lifecycle obligation, or automatic retry is active. Match that named blocker to the work shown in Overview and Activity & recovery; do not interrupt healthy in-flight work merely to restore 200. Pause for any other failed readiness check or an unexplained recovery blocker.
