...
Before starting the long-running process, run `bun run doctor` while every operator for this state and signer is stopped. The doctor acquires the same state and live-signer locks as the operator, fully parses the durable journal and its referenced protocol-index generation, authenticates the carry-proof journal and immutable-topology store when present, and validates deployment-profile, signer, index, and companion-store identity before any network probe. It is read-only with respect to configuration, durable state, companion stores, and the chain; its only local write is process-lock owner metadata, which it releases before exit. It never signs or submits a transaction. It authenticates all eight deployment roots and their configured graph (including both trading roots), requires every reader's finalized checkpoint to be no more than 96 blocks behind the quorum anchor, proves each finalized-tag hash, rechecks that the checkpoint did not regress or change, verifies a stable common finalized block identity, exercises the configured initial log span using bounded adaptive chunks, checks configured private-relay capabilities with intentionally invalid requests, and reports signer inventory and canonical topology. A configured signer must hold ETH for the reserve, one maximum operation principal, and one maximum gas budget, plus canonical REP for the reserve and one maximum REP principal; either shortfall makes the command exit nonzero. Live mode also fails without a signer. A passing doctor is a launch prerequisite, not a replacement for completing the paused dry-run indexes and reviewing exact operation eligibility in the dashboard.
...
The dashboard is restricted to loopback authority and never starts without authentication. Set `ZOLTAR_BOT_DASHBOARD_PASSWORD` to at least 16 characters before every local run; the commands above generate an owner-only 48-character secret that can be reused across restarts. Only `/healthz` stays unauthenticated. It is process liveness only and deliberately remains healthy while the operator is intentionally paused, so Compose does not restart a safe paused process. Authenticated `/readyz` returns HTTP 200 only when configuration, RPC quorum, poll-bounded canonical-scan freshness, signer/inventory, lifecycle presence, recovery, readable runtime state, and the absence of an active runtime error are ready for scheduled work. Authenticated `/metrics` exposes the overall result, every named readiness check—including the runtime-error check—workflow/transaction/obligation counts, retry and presence blockers, pauses, and scan age as bounded gauges. This is mandatory even on loopback because another local process or user must not be able to change limits, load a signer, or resume the funded bot. For remote administration, keep the bot loopback-only, use an access-controlled encrypted SSH tunnel, and open the forwarded service at `http://127.0.0.1:4193`. Basic authentication alone does not protect a plaintext remote hop.
...
Keep `runtime.execute: false`, then resume from the dashboard. The committed templates start with `strategy.selectableOperationAllowlist: []`, so this first resume should exercise scanning and lifecycle handling without scheduling random novelty. Confirm that no random operation is selected, then pause before creating the canary.

Choose at least one exact low-risk selectable definition ID from the Operation catalog, such as `open-oracle.weth.wrap`. In Settings, turn off **Allow every selectable operation** and enter that ID; add only a few low-principal IDs at a time. Resume dry-run and observe multiple randomized selections, confirming that every selection belongs to the explicit canary set. Dry-run records the anchored operation selection and summary without signing, submitting, or running prerequisites. It does not retain the full plan, calldata, or anchor for replay, or claim that the same workflow will remain executable later. Pause again before changing live controls and inspect the durable countdown. Its timestamp keeps aging while paused, so a countdown that is already due can run immediately on the next resume. An empty array means lifecycle-only mode; `null` means every otherwise-eligible selectable definition and should be used only after the staged set has behaved as expected. This control never suppresses lifecycle discovery or obligations, and removing an ID does not abandon a durable workflow that already started.
...
A pending nonce blocks novel work until its canonical outcome is known. Signed bytes are persisted before the first broadcast, and automatic resubmission uses the exact same bytes only after repeating durable preflights. Every receipt must be individually reported finalized by the required independent-reader quorum, retain a quorum-agreed canonical receipt-block identity throughout verification, and pass operation-specific semantic evidence. Readers may expose adjacent finalized heights; the bot does not mistake ordinary checkpoint propagation skew for a reorganization.

When consensus-finalized recovery proves that a lifecycle transaction reverted, or proves an exact nonce cancellation, the bot records one automatic-retry failure slot. Successful prerequisite recoveries, continuation entries, unsigned preflight changes, and connectivity outages do not consume that budget. Random novelty remains blocked during the retry window, but another independently actionable urgent lifecycle obligation can still run. The first two finalized failures retry after 60 and 120 seconds respectively; the third finalized failure exhausts the three-attempt limit and requires paused operator review. Activity & recovery shows the number of finalized attempts already failed and the next attempt time. An exhausted item stays failed until the operator uses the audited retry or abandonment path; semantic-evidence failures are never retried automatically.
...
Restore the complete unit only while the bot is stopped. Restore ownership to the exact bot service UID/GID before permissions; for the shipped container run `chown -R bun:bun /app/bots/chaos/.state`, then set the directory and private files to owner-only modes. Start in paused mode, and verify the signer, pending nonce, active workflow, index status, and safety-pause latch before resuming. Never stage a backup in an ordinary shared directory or unprotected NTFS path.
...
The shipped Compose file overlays `.state/process-locks` with the fixed host-wide Docker volume `zoltar-chaos-signer-locks`. This fences separate Compose projects on the same Docker host even when they use different runtime-state volumes. Any manually created container must mount that same named lock volume at the same path; a container that omits it is outside the guarantee. Filesystem locks do not coordinate separate hosts. Use one signer across exactly one host, or place an external lease/fencing service in front of startup before sharing a signer across hosts; a network filesystem alone is not treated as that lease.

The image entrypoint automatically runs the stopped-process launch gate before its normal operator command whenever the persisted configuration has `runtime.execute: true`. It validates the complete durable state and network while holding the state and signer locks, and aborts startup on failure. The first-boot paused dry-run template skips this live gate. `start.bat` also runs the gate before Compose startup; `start.bat doctor` performs the full preflight explicitly and fails if another service owns the state or signer lock.
...
On Windows, `start.bat` starts the persisted policy and explicitly warns that an existing live, unpaused volume may execute due work immediately. Run `start.bat doctor` while the service is stopped after every network/deployment edit and before the configured start. Do not copy a live `operator.json` into an ordinary PowerShell/NTFS working directory: inherited ACLs may expose a remembered key. Perform the keyless bootstrap from WSL2 on its Linux filesystem using the POSIX permission steps above, or provision the volume through a Windows secret manager that enforces a private ACL. Load the signer only after the file is back in the protected volume.
...
The fixture derives its operation rows from the production catalog and canonical mutation-surface coverage, adds representative live lifecycle instances, uses a four-ID staged canary rather than unrestricted novelty, uses raw atomic inventory values, and supplies a sanitized anchored topology. It therefore exercises the same catalog scale and grouping behavior as the live projection instead of maintaining a small independent list.
