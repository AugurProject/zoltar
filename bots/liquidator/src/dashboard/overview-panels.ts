...
function overviewAlertRows(snapshot: Pick<Snapshot, 'alerts' | 'pendingTransactions'>) {
	...
	if (snapshot.pendingTransactions.length > 0) {
		const recoveryAlert = alerts.find(alert => alert.message.startsWith(`${snapshot.pendingTransactions.length.toString()} transaction intent`) && alert.message.includes('recovery before execution can continue'))
