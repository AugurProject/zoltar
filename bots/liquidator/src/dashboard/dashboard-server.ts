...
function recognizedConnectivityValidationMessage(message: string) {
	if (
		message === 'Network and RPC settings must be an object' ||
		message === 'Network must be mainnet or sepolia' ||
		message === 'RPC connectivity settings must be an object' ||
		...
		message === 'Independent quorum RPC URLs must be an array of strings' ||
		...
		message === 'Live execution with RPC quorum 2 requires at least two independent quorum RPCs (three read endpoints total)'
	)
		return message
	return undefined
}
