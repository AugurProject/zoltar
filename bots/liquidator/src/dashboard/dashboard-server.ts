...
function publicOperatorSnapshot(value: unknown) {
	const source = record(value)
	if (source === undefined) return {}
	const snapshot = publicFields(value, ['deploymentCheckedBlock', 'deploymentCheckedTimestamp', 'execute', 'lastScanAt', 'lastScannedBlock', 'lastScannedTimestamp', 'network', 'operatorCapable', 'paused', 'scanning', 'status', 'wallet'])
