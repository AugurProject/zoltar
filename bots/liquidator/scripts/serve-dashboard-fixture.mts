...
function pool(address: string, questionId: string, selected: boolean, isPriceValid: boolean, candidateCount: number, universeId: string, approvedUniverse: boolean, parent = '0x0000000000000000000000000000000000000000', systemState = '0', forkOutcomeIndex?: string) {
	return {
		...
		botVault: {
			...
			coverageCommitmentDisplay: selected ? '32.5' : '0',
		...
		candidates: Array.from({ length: candidateCount }, (_, index) => ({
			...
			coverageCommitmentToTransferEth: (25 - index).toString(),
		...
		stagedOperations: [],
		systemState,
		totalCoverageCommitmentEth: '644.2',
...
const activities = [
	{
		...
		details: 'pool=0x1111…1111 target=0x0000…0004 coverageCommitmentAttoEth=25000000000000000000',
		kind: 'liquidation',
...
const server = startDashboardServer(4183, {
	...
	getState: () => ({
		...
		metrics: {
			...
			assumedCoverageCommitmentEth: '58.9',
			candidateCount: 4,
			deployedRep: '1636.84',
			eligiblePoolCount: 1,
			poolCount: 3,
