...
| `export` | `chainId`; `dataset=logs|timeline|reorgs`; optional block range; `canonical` filters only logs and timeline, while reorganization rows have no canonical/orphan classification | `limit` defaults to 5,000 and caps at 50,000; pass `x-augurscan-next-offset` as `offset` until the header is absent; offset caps at 10,000,000, so split larger exports by block range. A `start-boundary-advanced` sentinel is always included in a reorganization export because it records a coverage reset rather than an on-chain block. | Live offset, not snapshot-bound |
...
Export limits default to 5,000 and cannot exceed 50,000. `x-augurscan-returned` gives the row count, `x-augurscan-truncated` reports whether another page exists, and `x-augurscan-next-offset` supplies the next offset when truncated. A stream cursor older than seven days receives a reset event so the client reloads current state.
