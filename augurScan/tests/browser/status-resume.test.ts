	...
	browserTest(
		...
		async () => {
			...
			`,
				...
				expect(await evaluate(`document.querySelector('#freshness-banner').hidden`)).toBe(true)
				...
				await waitFor(`document.querySelector('.network-card .badge').textContent === 'status unavailable'`)
				expect(await evaluate(`document.querySelector('#freshness-banner').hidden`)).toBe(true)
				expect(await evaluate(`document.querySelectorAll('#refresh-stale, #detail-canonical-retry').length`)).toBe(0)
