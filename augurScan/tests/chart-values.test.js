...
import { chartValueBounds, uniswapPriceChartModel, uniswapPriceProvenance } from '../public/chart-values.js'
...
test('keeps each Uniswap pool as a provenance-preserving sparse series on one value scale', () => {
	...
	expect(model.rows[0]).toMatchObject({ market_id: '0xpool-a', uniswap_price_0: '18000000000000000000' })
	expect(model.rows[0].uniswap_price_1).toBeUndefined()
	expect(model.rows[1]).toMatchObject({ market_id: '0xpool-b', uniswap_price_1: '19000000000000000000' })
