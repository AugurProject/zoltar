...
test('accepts only the exact bundled PostgreSQL source from resolved Compose configuration', () => {
	...
	for (const invalid of ['postgres://other:secret@postgres:5432/augurscan', 'postgres://augurscan:secret@other:5432/augurscan', 'postgres://augurscan:secret@postgres:5433/augurscan', 'postgres://augurscan:secret@postgres:5432/other', 'not-a-url'])
