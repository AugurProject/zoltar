...
const capability = (file: string): 'api' | 'database' | 'projection' | 'indexer' | 'other' => {
	...
	if (file === 'src/database-projections.ts' || file === 'src/projections.ts') return 'projection'
	if (file === 'src/database.ts' || file.startsWith('src/database-') || file === 'src/schema.ts') return 'database'
