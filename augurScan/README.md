...
The Operations overview caps its event collections at 250 reports, 250 escalations, 250 auctions, 100 fork/migration events, 30 recent semantic changes, and the latest coordinator price, so its displayed counts describe those returned collections. Its risk arrays are currently unpaginated and contain every observed pool and vault. Report, escalation, auction, fork, trading, and timeline catalogs/details use opaque newest-first keyset cursors bound to the indexed block and hash; a changed head returns `409`, after which the client restarts from the first page. The cursor includes block, transaction hash, and log index so multiple transactions at one block cannot be skipped. Pool and vault history endpoints are separate bounded, non-cursor responses: their `limit` applies independently to each returned series and `truncated` indicates that at least one series has more records. Risk endpoints return the latest tagged snapshot evidence for each observed pool and vault, including block, method, availability, protocol state, and scanner severity. Vault risk is calculated only when the vault and its pool snapshots share one block hash; otherwise it is explicitly unavailable and exposes both evidence boundaries while coherent sampling catches up.
