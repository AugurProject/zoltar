...
The start boundary is the main backfill-speed control: do not scan blocks before the first possible project deployment. `BLOCK_BATCH_SIZE` controls how many sequential blocks one polling operation processes before refreshing the observed head and checkpoint context; the default is `20`. A higher value such as `100` reduces per-batch overhead on a reliable private RPC, but it does not parallelize blocks and may keep a rate-limited provider busy longer before failover. `POLL_INTERVAL_MS` affects caught-up polling and retry cadence, not continuous historical backfill.
