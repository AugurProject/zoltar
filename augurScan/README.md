...
Set private or higher-capacity RPC endpoints in `.env` for reliable historical indexing. The included public defaults are convenient for evaluation but can rate-limit large backfills. Set `MAINNET_START_BLOCK` and `SEPOLIA_START_BLOCK` to the verified earliest project deployment blocks before relying on the database as a complete production history. The conservative default of block `0` cannot omit protocol history but is expensive. When a manifest adds an activity-source address after the network checkpoint has passed its deployment, augurScan detects the uncovered interval, rewinds to the block before deployment, and replays forward with the existing canonical indexing path. The rewind is durable, so a restart resumes from the lowered checkpoint.
