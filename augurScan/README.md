...
Escalation balances and resolutions use tagged contract reads. Address share positions use indexed ERC-1155 transfers, including migration mints without counting them twice; source positions are marked locked after migration. Complete-set inventory includes INVALID, YES, and NO and is not a redemption quote. Tagged payout snapshots reconstruct inherited [carry membership and unspent proofs](../docs/reference/merkle-mountain-range.html#proofs), read recursive principal/reward allocations from the contract, and calculate exact [payouts and haircuts](../docs/explanation/escalation-game.html#payouts). The address and escalation pages show retained principal, auction haircut, payable REP, and protocol burn allocation. These are depositor claim-bundle amounts, not wallet transfer quotes; each proof is valid independently at the sampled block and must be refreshed after any settlement. Missing commitments, incomplete ancestry, and exhausted reconstruction limits produce unavailable evidence rather than a guessed payout. Refund balances and fee accrual totals cover indexed history and do not establish completeness before the configured history boundary.
