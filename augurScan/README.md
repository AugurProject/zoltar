...
The start boundary is the main backfill-speed control: do not scan blocks before the first possible project deployment. `LOG_SCAN_RANGE_SIZE` is the maximum inclusive `eth_getLogs` window size and defaults to `2000` blocks. If a provider rejects a multi-block query because its range or result set is too large, augurScan halves the attempted window until it succeeds, commits that contiguous segment, and then resumes at the following block with the configured maximum again. A one-block failure is never hidden by splitting and can trigger provider failover. `POLL_INTERVAL_MS` affects caught-up polling and retry cadence, not continuous historical backfill.
...
Local development expects PostgreSQL at `POSTGRES_URL`. The server applies SQL migrations under a PostgreSQL advisory lock before serving. Migration 005 introduces range-index dataset cursors and intentionally rejects every populated older database. Rebuild PostgreSQL from each network's verified start block; no legacy checkpoint or activity compatibility is retained. Empty databases migrate in place.
