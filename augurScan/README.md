...
The selector in the top-right header sets the `chainId` URL parameter for Activity, **Operations**, **System state**, **Contracts**, and **Rich list**. Operations summarizes index freshness, OpenOracle reports, escalation games, truth auctions, pool/vault accounting evidence, fork/migration evidence, price provenance, and recent semantic changes. Its catalogs live at `/operations/reports`, `/operations/escalations`, `/operations/auctions`, and `/operations/risk`. The Risk catalog does not yet calculate current health, liquidation, or severity: values that require tagged contract reads remain explicitly unavailable, as described in the [Operations evidence model](STATE_MODEL.md#operations-evidence-model). System state derives bounded registries for indexed security pools, questions, vaults, and Zoltar universes. Pool details show coordinator/OpenOracle history, conditional YES/NO prices, REP/WETH and REP/native-ETH curves, REP/USDC curves, and exact venue liquidity evidence. Contracts links each selection to deployment evidence without showing scanner-internal registry provenance or discovery bookkeeping.
...
Operations collections are newest-first bounded views, not complete cardinalities. The overview returns at most 250 reports, 250 escalations, 250 auctions, 100 fork/migration events, 30 recent semantic changes, and the latest coordinator price; its displayed counts describe those returned collections. The report, escalation, auction, and fork catalog endpoints return at most 250 newest records. The risk catalog returns the latest canonical accounting snapshot for each observed pool and vault, but snapshot-dependent health and severity remain unavailable. Use retained logs or PostgreSQL projections for complete analytics until these catalogs expose keyset pagination.
