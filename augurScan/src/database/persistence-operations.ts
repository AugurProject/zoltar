...
export async function storeBlocks(this: ScannerDatabase, chainId: number, blocks: readonly IndexedBlock[], lease: IndexerLease, provenance?: EvidenceProvenance, validateBeforeCommit: () => Promise<void> = async () => {}): Promise<void> {
	await withIndexerLease(lease, async transaction => {
		for (const block of blocks) await this.persistBlockInTransaction(transaction, chainId, block, provenance)
...
export async function updateObservedHead(this: ScannerDatabase, chainId: number, head: bigint, phase: string, lease: IndexerLease): Promise<void> {
	await withIndexerLease(lease, async transaction => {
...
export async function advanceNetworkStartBlock(this: ScannerDatabase, chainId: number, startBlock: bigint, lease: IndexerLease, provenance?: EvidenceProvenance): Promise<boolean> {
...
export async function recordFailure(this: ScannerDatabase, chainId: number, message: string, nextRetryAt: Date, lease: IndexerLease): Promise<void> {
	await withIndexerLease(lease, async transaction => {
