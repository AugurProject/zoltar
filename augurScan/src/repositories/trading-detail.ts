...
export type TradingDetailQuery = {
...
export const tradingDetailData = async (sql: SQL, query: TradingDetailQuery) => {
	const { chainId, market, asOfBlock, asOfTimestamp, cursorBlock, cursorLog, cursorTx, queryLimit } = query
	const events = await sql`
		...
		ORDER BY event.block_number DESC, event.log_index DESC, event.tx_hash DESC LIMIT ${queryLimit}
	`
	const observations = await sql`
		WITH window_observations AS (
			...
			WHERE event.chain_id = ${chainId} AND event.market_address = ${market} AND event.canonical
				...
				AND block.timestamp >= to_timestamp(${asOfTimestamp}::numeric - 604800)
	...
	const summaries = await sql`
		SELECT
			count(*) FILTER (WHERE event.event_name = 'Swap' AND block.timestamp >= to_timestamp(${asOfTimestamp}::numeric - 86400))::integer AS swaps_24h,
			count(*) FILTER (WHERE event.event_name = 'Swap' AND block.timestamp >= to_timestamp(${asOfTimestamp}::numeric - 604800))::integer AS swaps_7d,
			COALESCE(sum((event.event_data->>'amountIn')::numeric) FILTER (WHERE event.event_name = 'Swap' AND block.timestamp >= to_timestamp(${asOfTimestamp}::numeric - 86400)), 0)::text AS input_volume_24h,
			COALESCE(sum((event.event_data->>'amountIn')::numeric) FILTER (WHERE event.event_name = 'Swap' AND block.timestamp >= to_timestamp(${asOfTimestamp}::numeric - 604800)), 0)::text AS input_volume_7d,
			COALESCE(sum((event.event_data->>'feeAmount')::numeric) FILTER (WHERE event.event_name = 'Swap' AND block.timestamp >= to_timestamp(${asOfTimestamp}::numeric - 86400)), 0)::text AS fees_24h,
			COALESCE(sum((event.event_data->>'feeAmount')::numeric) FILTER (WHERE event.event_name = 'Swap' AND block.timestamp >= to_timestamp(${asOfTimestamp}::numeric - 604800)), 0)::text AS fees_7d,
			count(*) FILTER (WHERE event.event_name IN ('LiquidityInitialized', 'LiquidityAdded', 'LiquidityRemoved') AND block.timestamp >= to_timestamp(${asOfTimestamp}::numeric - 604800))::integer AS liquidity_events_7d
		FROM amm_trade_events event JOIN blocks block ON block.chain_id = event.chain_id AND block.hash = event.block_hash
		WHERE event.chain_id = ${chainId} AND event.market_address = ${market} AND event.canonical
			AND event.block_number <= ${asOfBlock}
	...
	const lpPositions = await sql`
		...
		LIMIT 250
	`
	return { events, observations, summaries, lpPositions }
}
