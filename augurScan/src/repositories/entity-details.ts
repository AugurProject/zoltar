...
export const latestEntitySnapshot = async (sql: SQL, chainId: number, entityType: string, entityIdentity: string, snapshotBlock: string) => {
	const rows = await sql`
		...
		WHERE snapshot.chain_id = ${chainId} AND snapshot.entity_type = ${entityType}
			AND snapshot.entity_identity = ${entityIdentity} AND snapshot.canonical AND snapshot.block_number <= ${snapshotBlock}
...
export const reportDetailData = async (
	...
	const { chainId, openOracleAddress, reportId, rounds, decisions, snapshotBlock } = query
	const rows = await sql`
		...
		WHERE event.chain_id = ${chainId} AND event.open_oracle_address = ${openOracleAddress}
			AND event.report_id = ${reportId} AND event.canonical AND event.block_number <= ${snapshotBlock}
	...
	const coordinatorDecisions = await sql`
		WITH coordinators AS (
			...
			JOIN logs request ON request.chain_id = report.chain_id AND request.block_hash = report.block_hash
				AND request.tx_hash = report.tx_hash AND request.canonical AND request.block_number <= ${snapshotBlock}
			WHERE report.chain_id = ${chainId} AND report.open_oracle_address = ${openOracleAddress}
				AND report.report_id = ${reportId} AND report.canonical AND report.block_number <= ${snapshotBlock} AND report.event_name = 'ReportSubmitted'
		...
		FROM logs log JOIN blocks block ON block.chain_id = log.chain_id AND block.hash = log.block_hash AND block.canonical
		JOIN coordinators coordinator ON coordinator.emitter_address = log.emitter_address
		WHERE log.chain_id = ${chainId} AND log.canonical AND log.block_number <= ${snapshotBlock} AND log.arguments->>'reportId' = ${reportId}
...
export const eventEntityRows = async (sql: SQL, query: { readonly snapshotBlock: string; readonly chainId: number; readonly address: string; readonly domain: 'auction' | 'escalation'; readonly page: PageBoundary }) => {
	const { chainId, address, domain, page, snapshotBlock } = query
	return domain === 'auction'
		? await sql`SELECT event.*, block.timestamp AS block_timestamp FROM truth_auction_events event JOIN blocks block ON block.chain_id = event.chain_id AND block.hash = event.block_hash AND block.canonical WHERE event.chain_id = ${chainId} AND event.auction_address = ${address} AND event.canonical AND event.block_number <= ${snapshotBlock} AND (event.block_number, event.log_index, event.tx_hash) < (${page.block}::bigint, ${page.log}::integer, ${page.tx}) ORDER BY event.block_number DESC, event.log_index DESC, event.tx_hash DESC LIMIT ${page.limit}`
		: await sql`SELECT event.*, block.timestamp AS block_timestamp FROM escalation_game_events event JOIN blocks block ON block.chain_id = event.chain_id AND block.hash = event.block_hash AND block.canonical WHERE event.chain_id = ${chainId} AND event.game_address = ${address} AND event.canonical AND event.block_number <= ${snapshotBlock} AND (event.block_number, event.log_index, event.tx_hash) < (${page.block}::bigint, ${page.log}::integer, ${page.tx}) ORDER BY event.block_number DESC, event.log_index DESC, event.tx_hash DESC LIMIT ${page.limit}`
}

export const auctionDetailData = async (sql: SQL, chainId: number, address: string, snapshotBlock: string) => {
	const [bids, finalizations] = await Promise.all([
		sql`SELECT event_data->>'tick' AS tick, sum((event_data->>'bidAmountAttoEth')::numeric)::text AS amount_atto_eth
			FROM truth_auction_events WHERE chain_id = ${chainId} AND auction_address = ${address}
				AND canonical AND block_number <= ${snapshotBlock} AND event_name = 'BidSubmitted'
		...
		sql`SELECT * FROM truth_auction_events WHERE chain_id = ${chainId} AND auction_address = ${address}
			AND canonical AND block_number <= ${snapshotBlock} AND event_name = 'AuctionFinalized' ORDER BY block_number DESC, log_index DESC LIMIT 1`,
	])
	return { bids, finalization: finalizations[0] }
}

export const forkDetailData = async (sql: SQL, query: { readonly snapshotBlock: string; readonly chainId: number; readonly identity: string; readonly page: PageBoundary }) => {
	const { chainId, identity, page, snapshotBlock } = query
	const rows = await sql`
		...
		WHERE event.chain_id = ${chainId} AND event.canonical AND event.block_number <= ${snapshotBlock}
			AND (event.universe_identity = ${identity} OR event.event_data->>'universeId' = ${identity}
				...
				OR EXISTS (SELECT 1 FROM pools pool WHERE pool.chain_id = event.chain_id AND pool.canonical AND pool.block_number <= ${snapshotBlock}
	...
	const branches = await sql`
		...
		FROM fork_migration_events WHERE chain_id = ${chainId} AND canonical AND block_number <= ${snapshotBlock}
