...
export async function assertLease(this: NetworkIndexer): Promise<void> {
	try {
...
export function requireLease(this: NetworkIndexer): IndexerLease {
	if (this.lease === undefined) throw new LeaseLostError('Indexer lease is unavailable; reacquiring')
	return this.lease
}

export function withManifestDeploymentBlocks(this: NetworkIndexer, contracts: ReadonlyMap<string, ContractMetadata>): Map<string, ContractMetadata> {
...
export async function reconcileManifestBackfill(this: NetworkIndexer): Promise<void> {
...
export async function reconcileReorg(this: NetworkIndexer): Promise<void> {
	const checkpoint = await this.database.checkpoint(this.network.chainId, this.requireLease())
	if (checkpoint === undefined) return
...
export async function refreshContractDeployment(this: NetworkIndexer, indexedBoundary: bigint): Promise<void> {
	const now = Date.now()
