...
import { ChainContinuityError, isPrunedHistoricalStateError, isSplittableLogRangeError, queryCanonicalLogRange, requireLogPosition } from '../indexer-runtime.ts'
...
import { chunks, type DeploymentAwareLogPlan, type LogScanInput, mapLimit, planDeploymentAwareLogScan, queryAdaptiveLogRange, type RpcBlockHeader, rpcLogQueryGroups, uniswapV2PairCreatedEvent, uniswapV3PoolCreatedEvent, uniswapV4InitializeEvent, uniswapV4PoolIds, uniswapV4SwapEvent } from './planning.ts'
...
export abstract class NetworkIndexerLogScanner extends NetworkIndexerSynchronization {
	protected async queryLogs(toBlock: bigint, inputs: readonly LogScanInput[], contracts: ReadonlyMap<string, ContractMetadata>): Promise<readonly Log[]> {
		...
		for (const log of [...ordinaryPages.flat(), ...v2Pages.flat(), ...v3Pages.flat(), ...initializePages.flat(), ...swapPages.flat()]) {
			...
			if (input === undefined || position.blockNumber < input.fromBlock || position.blockNumber > toBlock) throw new ChainContinuityError(`RPC returned a log outside its requested deployment-aware range through ${toBlock}`)
	...
	protected async getLogsForInputs(toBlock: bigint, inputs: readonly LogScanInput[], contracts: ReadonlyMap<string, ContractMetadata>): Promise<{ readonly logs: readonly Log[]; readonly endBlockHash: Hash }> {
	...
	protected async getLogs(fromBlock: bigint, toBlock: bigint, addresses: readonly Address[], contracts: ReadonlyMap<string, ContractMetadata>): Promise<{ readonly logs: readonly Log[]; readonly endBlockHash: Hash }> {
	...
	protected async getAllLogs(fromBlock: bigint, toBlock: bigint, addresses: readonly Address[], contracts: ReadonlyMap<string, ContractMetadata>, expectedBlockHash: (blockNumber: bigint) => Promise<Hash>): Promise<readonly Log[]> {
		const logs: Log[] = []
	...
	protected async getKnownLogs(blockNumber: bigint, addresses: readonly Address[], contracts: ReadonlyMap<string, ContractMetadata>, blockHash: Hash): Promise<Log[]> {
