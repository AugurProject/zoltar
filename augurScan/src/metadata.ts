...
export const referencedAddressesFrom = (parameters: readonly AbiParameter[], values: SerializedArguments): readonly Address[] => [...new Map(parameters.flatMap((parameter, index) => addressesFromParameter(parameter, values[parameter.name || String(index)])).map(address => [address.toLowerCase(), address])).values()]
