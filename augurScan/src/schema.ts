...
export const expectedSchemaLayout = (schema: string, version: SupportedSchemaVersion): SchemaLayout => {
	...
	for (const match of normalizedSchema.matchAll(/CREATE TABLE public\.([a-z_][a-z0-9_]*) \(\n([\s\S]*?)\n\);/g)) {
		...
		if (
			table === undefined ||
			body === undefined ||
			(version === INITIAL_MIGRATABLE_SCHEMA_VERSION && historicalIntegrityTables.has(table)) ||
			(version !== CURRENT_SCHEMA_VERSION && ownershipTables.has(table))
		)
			continue
...
export const schemaInitializationAction = (
	markerVersion: string | undefined,
	publicObjects: readonly string[],
): 'initialize' | 'migrate-from-1' | 'migrate-from-2' | 'current' => {
	...
	try {
		...
		if (action === 'migrate-from-1' || action === 'migrate-from-2') {
			...
			const migrations = [
				...
				await Bun.file(path.resolve(import.meta.dir, '../migrations/003-indexer-ownership.sql')).text(),
			]
