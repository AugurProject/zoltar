...
import { addressContractIdentity, addressInteractionHistory, addressTransactionHistory, interactionHistoryAnchor, interactionHistorySnapshot, transactionHistoryAnchor, transactionHistorySnapshot } from '../repositories/address-history.ts'
import { operationsAsOf } from '../repositories/operations.ts'
import { ApiConflictError, ApiRequestError, actionJsonColumns, addressHistoryCursorFor, decodedJsonColumns, evmAddress, integer, json, parseAddressHistoryCursor } from './shared.ts'
...
export const addressTransactions = async (sql: SQL, url: URL): Promise<Response> => {
	...
	if (cursor !== undefined && (cursor[6] !== String(asOf['invalidationId']) || cursor[7] !== String(asOf['abiSourceHash']) || cursor[8] !== String(asOf['applicationSourceHash']) || cursor[9] !== String(asOf['projectionSourceHash']))) throw new ApiConflictError('Indexed state changed; restart pagination')
	...
	if (cursor !== undefined) {
		...
		if (validationRows[0]?.['snapshot_canonical'] !== true || Number(validationRows[0]?.['snapshot_total'] ?? -1) !== cursor[10]) throw new ApiConflictError('Transaction history changed; restart pagination')
	}
	...
	const items = pageRows.map((row: Record<string, unknown>) => decodedJsonColumns(Object.fromEntries(Object.entries(row).filter(([key]) => key !== 'snapshot_total')), actionJsonColumns))
	return json({
		items,
		total,
		limit,
		snapshotBlock,
		nextCursor: hasMore && pageRows.length > 0 ? addressHistoryCursorFor('sent', chainId, address, snapshotBlock, snapshotHash, asOf, total, pageRows[pageRows.length - 1] as Record<string, unknown>) : undefined,
	})
}
...
export const addressInteractions = async (sql: SQL, url: URL): Promise<Response> => {
	...
	if (cursor !== undefined) {
		...
		if (validationRows[0]?.['snapshot_canonical'] !== true || Number(validationRows[0]?.['snapshot_total'] ?? -1) !== cursor[10]) throw new ApiConflictError('Interaction history changed; restart pagination')
	}
	...
	const pageRows = rows.slice(0, limit)
	return json({
		items: pageRows.map((row: Record<string, unknown>) => decodedJsonColumns(Object.fromEntries(Object.entries(row).filter(([key]) => key !== 'snapshot_total')), actionJsonColumns)),
		total,
		limit,
		snapshotBlock,
		nextCursor: hasMore && pageRows.length > 0 ? addressHistoryCursorFor('referenced', chainId, address, snapshotBlock, snapshotHash, asOf, total, pageRows[pageRows.length - 1] as Record<string, unknown>) : undefined,
	})
}
