...
- **liquidator:** create `bots/liquidator/.env` containing `ZOLTAR_BOT_RPC_QUORUM=1`, then from the repository root run `cd bots/liquidator` and `docker compose up --build --force-recreate`. Open `http://127.0.0.1:4183`, select Sepolia in **Chain and RPC connectivity**, enter `http://anvil:8545` for the primary and public RPC URLs, leave the independent quorum RPC list empty, save, and run `docker compose restart`. Keep execution disabled unless its deployment addresses and signer are configured for this chain.
- **open-oracle arbitrager:** from the repository root run `cd bots/open-oracle-arbitrager` and `docker compose up --build --force-recreate`. Open `http://127.0.0.1:4173`, select Sepolia and **1 reader · isolated development only** in **Chain and RPC connectivity**, enter `http://anvil:8545` for the primary and public RPC URLs, leave the independent quorum RPC list empty, save, and run `docker compose restart`. Keep execution disabled unless its deployment manifest, contract addresses, and signer are configured for this chain.

The one-reader setting removes independent RPC corroboration and must only be used with an isolated development network. The default is `2`, which preserves the production requirement for two agreeing readers and requires two independently operated quorum RPC URLs in addition to the primary read RPC so one endpoint may be unavailable. The liquidator still reads this policy from `ZOLTAR_BOT_RPC_QUORUM`; the open-oracle arbitrager saves it through the dashboard.
