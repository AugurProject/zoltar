...
Both bots accept the primary RPC alone by default. Set `ZOLTAR_BOT_RPC_QUORUM=2` for the liquidator or save **2 agreeing readers · independent quorum** for the arbitrager to require two agreeing readers and two independently operated quorum RPC URLs in addition to the primary read RPC so one endpoint may be unavailable.
