...
- **open-oracle arbitrager:** from the repository root run `cd bots/open-oracle-arbitrager` and `docker compose up --build --force-recreate`. Open `http://127.0.0.1:4173`, select Sepolia in **Chain and RPC connectivity**, enter `http://anvil:8545` for the primary and public RPC URLs, leave the optional independent quorum RPC list empty, save, and run `docker compose restart`. Keep execution disabled unless its deployment manifest, contract addresses, and signer are configured for this chain.

The default `ZOLTAR_BOT_RPC_QUORUM=1` accepts the primary RPC alone. Set `ZOLTAR_BOT_RPC_QUORUM=2` to require two agreeing readers and two independently operated quorum RPC URLs in addition to the primary read RPC so one endpoint may be unavailable.
