import { Blob4844Tx as BlobEIP4844Transaction, Capability, isAccessList2930Tx as isAccessListEIP2930Tx, isBlob4844Tx as isBlobEIP4844Tx, isFeeMarket1559Tx as isFeeMarketEIP1559Tx, isLegacyTx, } from "./tx.js";
import { Account, bytesToHex, bytesToUnprefixedHex, equalsBytes } from "./util.js";
...
export class TxPool {
    ...
    constructor({ vm, maxSize = MAX_POOL_SIZE, maxPerSender = MAX_TXS_PER_ACCOUNT }) {
        ...
        this.txsByNonce = new Map();
        this.txsInNonceOrder = new Map();
        this.opened = false;
        this.running = true;
    }
    deepCopy(opt) {
    ...
    async validate(tx, isLocalTransaction = false, requireSignature = true, skipBalance = false) {
        ...
        if (typeof block.header.baseFeePerGas === "bigint" && block.header.baseFeePerGas !== 0n) {
            if (currentGasPrice.maxFee < block.header.baseFeePerGas / 2n && !isLocalTransaction) {
                throw new Error(`Tx cannot pay basefee of ${block.header.baseFeePerGas}, have ${currentGasPrice.maxFee} (not within 50% range of current basefee)`);
            }
        }
        if (tx.gasLimit > block.header.gasLimit) {
            throw new Error(`Tx gaslimit of ${tx.gasLimit} exceeds block gas limit of ${block.header.gasLimit} (exceeds last block gas limit)`);
        }
    ...
    async onChainReorganization(removedBlocks, addedBlocks) {
        for (const block of removedBlocks) {
            for (const tx of block.transactions) {
                ...
                if (this.txsByHash.has(txHashUnprefixed))
                    continue;
    ...
    async txsByPriceAndNonce({ baseFee, allowedBlobs } = {}) {
        ...
        const byPrice = new PriorityQueue((a, b) => this.normalizedGasPrice(b, baseFee) - this.normalizedGasPrice(a, baseFee) < 0n);
