...
function bigEndianIntegerBytes(value) {
    if (value < 0n) {
...
function inputToBytes(input) {
    ...
    if (typeof input === "bigint")
        return bigEndianIntegerBytes(input);
    if (typeof input === "number")
        return numberToBytes(input);
...
function readLength(bytes, offset, lengthOfLength) {
    if (lengthOfLength === 0)
        throw invalid("empty length prefix");
    if (offset + lengthOfLength > bytes.length)
        throw invalid("length prefix exceeds input");
    if (bytes[offset] === 0)
        throw invalid("length prefix has leading zeros");
    let length = 0;
    for (const byte of bytes.subarray(offset, offset + lengthOfLength)) {
        length = length * 256 + byte;
        if (!Number.isSafeInteger(length))
            throw invalid("payload length exceeds safe integer range");
    }
    return length;
}
function requireAvailable(bytes, start, length) {
    if (start + length > bytes.length)
        throw invalid("payload exceeds input");
}
function decodeItem(bytes, offset) {
    ...
    if (first <= 0xbf) {
        ...
        if (length < 56)
            throw invalid("long string form used for short payload");
    ...
    const length = readLength(bytes, offset + 1, lengthOfLength);
    if (length < 56)
        throw invalid("long list form used for short payload");
    const start = offset + 1 + lengthOfLength;
    requireAvailable(bytes, start, length);
    return { data: decodeListPayload(bytes, start, start + length), nextOffset: start + length };
}
function decodeListPayload(bytes, start, end) {
    const out = [];
    let offset = start;
    while (offset < end) {
    ...
    if (offset !== end)
        throw invalid("list payload length mismatch");
    return out;
}
export function decode(input, stream = false) {
    ...
    if (stream)
        return { data: decoded.data, remainder };
    if (remainder.length !== 0)
        throw invalid("remainder must be empty");
    return decoded.data;
}
