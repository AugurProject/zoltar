...
export type arkKeyOf<o> = o extends array ? (number extends o["length"] ? NonNegativeIntegerLiteral : never) | {
    [k in keyof o]-?: k extends `${infer index extends number}` ? index | k : never;
}[keyof o & `${number}`] : {
    [k in keyof o]: k extends number ? k | `${k}` : k;
}[keyof o];
export type getArkKey<o, k extends arkKeyOf<o>> = o[Extract<k extends NonNegativeIntegerLiteral ? number : k, keyof o>];
export type TypeKey = termOrType<Key>;
export type typeKeyToString<k extends TypeKey> = typeToString<k extends type.cast<infer t> ? t : k>;
