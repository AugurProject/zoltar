...
exports.newAccountStateGasCost = newAccountStateGasCost;
exports.eip7928TrapPreTarget = eip7928TrapPreTarget;
exports.eip7928SchedulePostTargetCallOog = eip7928SchedulePostTargetCallOog;
exports.eip7928PostTargetCreateOog = eip7928PostTargetCreateOog;
exports.consumeEip7928PostTargetCallOog = consumeEip7928PostTargetCallOog;
exports.getCallNewAccountPostCosts = getCallNewAccountPostCosts;
exports.callFamilyGas = callFamilyGas;
exports.create7928Gas = create7928Gas;
...
function canAfford(runState, regularGas, stateGas = util_1.BIGINT_0) {
...
function newAccountStateGasCost(common, runState) {
    ...
    const costPerStateByte = (0, eip8037_ts_1.activeCostPerStateByte)(common, blockGasLimit);
    return stateBytesPerNewAccount * costPerStateByte;
}
/** Pre-target OOG: trap before any target access / BAL entry. */
function eip7928TrapPreTarget(runState, common, gas) {
...
function eip7928SchedulePostTargetCallOog(runState, outOffset, outLength) {
...
function eip7928PostTargetCreateOog(runState, common, gas) {
    if (!common.isActivatedEIP(7928)) {
        (0, util_ts_1.trap)(errors_ts_1.EVMError.errorMessages.OUT_OF_GAS);
    }
    runState.messageGasLimit = util_1.BIGINT_0;
...
function consumeEip7928PostTargetCallOog(runState) {
    const pending = runState.eip7928PostTargetCallOog;
    if (pending === undefined) {
        return;
    }
    runState.eip7928PostTargetCallOog = undefined;
    (0, util_ts_1.writeCallOutput)(runState, pending.outOffset, pending.outLength);
    (0, util_ts_1.trap)(errors_ts_1.EVMError.errorMessages.OUT_OF_GAS);
}
async function getCallNewAccountPostCosts(runState, common, toAddress) {
    ...
    if (!createsNewAccount) {
        return { regular: util_1.BIGINT_0, state: util_1.BIGINT_0, createsNewAccount: false };
    }
    return {
...
function finalizeCallMessageGas(runState, common, gas, currentGasLimit, newAccountStateGas, outOffset, outLength) {
    ...
    if (common.isActivatedEIP(8037) && newAccountStateGas > util_1.BIGINT_0) {
 ...
 */
async function callFamilyGas(runState, gas, common, opts) {
    ...
    if ((common.isActivatedEIP(6800) || common.isActivatedEIP(7864)) &&
        ...
        if (includeValueTransfer && value !== util_1.BIGINT_0) {
    ...
    if (common.isActivatedEIP(2929)) {
        gas += (0, EIP2929_ts_1.getAddressAccessCost)(runState, toAddress.bytes, common, charge2929Gas);
    }
    ...
    let newAccountStateGas = util_1.BIGINT_0;
    ...
    if (includeNewAccountPostCheck &&
        (value !== util_1.BIGINT_0 || !common.gteHardfork(common_1.Hardfork.SpuriousDragon))) {
 ...
 */
async function create7928Gas(runState, gas, common, opts, getCreateTargetAddressBytes) {
    const { offset, length, salt, extraPreTargetGas = util_1.BIGINT_0, preChargeLabel } = opts;
    ...
    if (common.isActivatedEIP(3860) &&
        ...
        (0, util_ts_1.trap)(errors_ts_1.EVMError.errorMessages.INITCODE_SIZE_VIOLATION);
    }
    ...
    if (common.isActivatedEIP(2929)) {
        (0, EIP2929_ts_1.warmAddress)(runState, targetAddress);
    }
    if (common.isActivatedEIP(2929)) {
    ...
    if (common.isActivatedEIP(8037)) {
        if (!canAfford(runState, gas, newAccountStateGas)) {
            (0, EIP2929_ts_1.addAddressToBAL)(runState, targetAddress, common);
