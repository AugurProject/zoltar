...
// ts/node/render-solidity/renderSystemInterface.ts
...
function renderSystemInterface(options) {
  ...
  return `
     ...
     */
    interface ${name} {
      ${renderList(errors, ({ name: name2, parameters }) => `error ${name2}(${renderArguments(parameters)});`)}
...
// ts/node/render-solidity/renderSystemLibrary.ts
import {
...
function renderSystemLibrary(options) {
  ...
  const errors = [
    ...
    ...systemErrors.map((error2) => ({
      ...error2,
      parameters: applyTypeQualifiers(error2.parameters, typeQualifiers)
    }))
  ];
  ...
  return `
    ...
    library ${libraryName} {
      ...
      ${renderList2(functions, (contractFunction) => renderUserTypeFunction(contractFunction, userTypeName))}

      ${renderList2(functions, (contractFunction) => renderCallWrapperFunction(contractFunction, callingFromRootSystemErrorName))}

      ${renderList2(functions, (contractFunction) => renderRootCallWrapperFunction(contractFunction, namespace))}
    ...
    ${functions.length > 0 ? `/**
         ...
         */` : ""}
    ${renderList2(functions, (contractFunction) => renderFunctionInterface(contractFunction))}
...
function renderErrors(errors) {
  return renderList2(errors, ({ name, parameters }) => `  error ${name}(${renderArguments2(parameters)});`);
}
...
function renderRootCallWrapperFunction(contractFunction, namespace) {
  const { name, parameters, stateMutability, returnParameters } = contractFunction;
...
function functionInterfaceName(contractFunction) {
  const { name, parameters } = contractFunction;
  const paramTypes = parameters.map((param) => param.split(" ")[0]).map((type5) => type5.replace(".", "_")).map((type5) => type5.replace("[]", "Array")).map((type5) => type5.replace(/\[.+\]/, (match2) => stringToHex(match2))).join("_");
...
function renderAbiDecode(expression, returnParameters) {
...
function renderWorldInterface({
  ...
  return `
     ...
     */
    interface ${interfaceName} is ${renderArguments3(importSymbols)} {
    }
  `;
}
...
import {
  ...
  applyTypeQualifiers as applyTypeQualifiers2
} from "@latticexyz/common/codegen";
...
var AST = class _AST {
  ...
  #emptyExt = false;
  constructor(type5, parent, options = {}) {
    this.type = type5;
    if (type5)
      this.#hasMagic = true;
    this.#parent = parent;
...
var defaults = (def) => {
  ...
  return Object.assign(m, {
    ...
    AST: class AST extends orig.AST {
      /* c8 ignore start */
      constructor(type5, parent, options = {}) {
        super(type5, parent, ext(def, options));
      }
      /* c8 ignore stop */
      static fromGlob(pattern, options = {}) {
...
// ../../node_modules/.pnpm/glob@10.4.2/node_modules/glob/dist/esm/glob.js
import { fileURLToPath as fileURLToPath2 } from "node:url";

// ../../node_modules/.pnpm/lru-cache@10.3.0/node_modules/lru-cache/dist/esm/index.js
var perf = typeof performance === "object" && performance && typeof performance.now === "function" ? performance : Date;
var warned = /* @__PURE__ */ new Set();
var PROCESS = typeof process === "object" && !!process ? process : {};
var emitWarning = (msg, type5, code, fn) => {
  typeof PROCESS.emitWarning === "function" ? PROCESS.emitWarning(msg, type5, code, fn) : console.error(`[${code}] ${type5}: ${msg}`);
};
...
var PathBase = class {
   ...
   * @internal
   */
  constructor(name, type5 = UNKNOWN, root, roots, nocase, children, opts) {
    this.name = name;
    this.#matchName = nocase ? normalizeNocase(name) : normalize(name);
    this.#type = type5 & TYPEMASK;
    this.nocase = nocase;
    this.roots = roots;
    this.root = root || this;
  ...
  #readdirAddNewChild(e, c) {
    const type5 = entToType(e);
    const child = this.newChild(e.name, type5, { parent: this });
...
var PathWin32 = class _PathWin32 extends PathBase {
   ...
   */
  constructor(name, type5 = UNKNOWN, root, roots, nocase, children, opts) {
    super(name, type5, root, roots, nocase, children, opts);
  }
  /**
   * @internal
   */
  newChild(name, type5 = UNKNOWN, opts = {}) {
    return new _PathWin32(name, type5, this.root, this.roots, this.nocase, this.childrenCache(), opts);
  }
  /**
   * @internal
   */
...
var PathPosix = class _PathPosix extends PathBase {
   ...
   */
  newChild(name, type5 = UNKNOWN, opts = {}) {
    return new _PathPosix(name, type5, this.root, this.roots, this.nocase, this.childrenCache(), opts);
  }
};
...
// ../../node_modules/.pnpm/glob@10.4.2/node_modules/glob/dist/esm/pattern.js
...
// ../../node_modules/.pnpm/glob@10.4.2/node_modules/glob/dist/esm/ignore.js
...
// ../../node_modules/.pnpm/glob@10.4.2/node_modules/glob/dist/esm/processor.js
...
// ../../node_modules/.pnpm/glob@10.4.2/node_modules/glob/dist/esm/walker.js
...
// ../../node_modules/.pnpm/glob@10.4.2/node_modules/glob/dist/esm/has-magic.js
...
// ts/node/findSolidityFiles.ts
async function findSolidityFiles({ cwd, pattern = "**" }) {
  const files = await glob(path2.join(pattern, "*.sol"), { cwd, windowsPathsNoEscape: true });
  return files.sort().map((filename) => ({
    filename,
    basename: path2.basename(filename, ".sol")
  }));
}
...
async function getSystemContracts({
  ...
  return files.map((file) => {
    ...
    const namespaceLabel = (() => {
      ...
      const [namespacesDir, namespaceDir] = relativePath.split(path3.sep);
      if (namespacesDir === "namespaces" && namespaceDir) {
        return namespaceDir;
      }
      throw new Error(
        `Expected system file at "${file.filename}" to be in a namespace directory like "${path3.join(config.sourceDirectory, "namespaces/{namespace}", relativePath)}"`
      );
    })();
    return {
...
async function resolveSystems({
  ...
  const missingSystems = configSystems.filter(
    (system) => !systemContracts.some((s) => s.namespaceLabel === system.namespaceLabel && s.systemLabel === system.label)
  );
  ...
  const systems = systemContracts.map((contract) => {
    const systemConfig = config.namespaces[contract.namespaceLabel]?.systems[contract.systemLabel] ?? resolveNamespace({
...
async function worldgen({
  rootDir,
  config,
  clean = true
}) {
  const remappings = await getRemappings();
  const worldgenOutDir = path4.join(
    rootDir,
  ...
  const systems = (await resolveSystems({ rootDir, config })).filter((system) => system.deploy.registerWorldFunctions || config.codegen.generateSystemLibraries).map((system) => {
    ...
    const libraryOutDir = path4.join(
      sourceDir,
      config.codegen.outputDirectory,
    ...
    return {
      ...
      interfaceName,
      libraryName,
      interfacePath: path4.join(worldgenOutDir, `${interfaceName}.sol`),
      libraryPath: path4.join(libraryOutDir, `${libraryName}.sol`)
    };
  });
  if (clean) {
  ...
  const storeImportPath = config.codegen.storeImportPath.startsWith(".") ? "./" + path4.relative(path4.dirname(outputPath), path4.join(rootDir, config.codegen.storeImportPath)) : config.codegen.storeImportPath;
  ...
  await Promise.all(
    systems.map(async (system) => {
      ...
      const findInheritedSymbol = await createInheritanceResolver(
        path4.join(rootDir, system.sourcePath),
        system.label,
        rootDir,
        remappings
      );
      ...
      const typeQualifiers = /* @__PURE__ */ new Map();
      ...
      if (system.deploy.registerWorldFunctions) {
        ...
        const qualifiedFunctions = functions.map((func) => ({
          ...func,
          parameters: applyTypeQualifiers2(func.parameters, typeQualifiers),
          returnParameters: applyTypeQualifiers2(func.returnParameters, typeQualifiers)
        }));
        const qualifiedErrors = errors.map((error2) => ({
          ...error2,
          parameters: applyTypeQualifiers2(error2.parameters, typeQualifiers)
        }));
      ...
      if (config.codegen.generateSystemLibraries) {
        const systemImport = {
          symbol: system.label,
          path: "./" + path4.relative(path4.dirname(system.libraryPath), system.sourcePath)
        };
        const libraryImports = symbolImports.map(
          ({ symbol, path: importPath }) => ({
            symbol,
            path: importPath.startsWith(".") ? "./" + path4.relative(
              path4.dirname(system.libraryPath),
              path4.join(rootDir, path4.dirname(system.sourcePath), importPath)
            ) : importPath
          })
        );
...
// ts/node/findContractArtifacts.ts
...
var systemsManifestFilename = ".mud/local/systems.json";
...
var types2 = scope({
  Bytecode: type("string").narrow(
    (input, ctx) => isHex2(input, { strict: false }) || ctx.mustBe("a hex string")
  ),
  ...
  Artifact: {
    ...
    abi: type("unknown[]").pipe((input) => input),
    "metadata?": {
      settings: {
        compilationTarget: {
...
var debug = createDebug("mud:world");
debug.log = console.debug.bind(console);
var error = createDebug("mud:world");
...
// ts/node/findContractArtifacts.ts
var parseArtifact = type2("string").pipe.try((s) => JSON.parse(s), types2.Artifact);
function hasMetadata(artifact) {
  return artifact.metadata !== void 0;
}
async function findContractArtifacts({ forgeOutDir }) {
  const files = (await glob("**/*.sol/*.json", {
  ...
  const artifactsJson = await Promise.all(
    files.map(async (filename) => ({
      filename,
      json: await readFile(path5.join(forgeOutDir, filename), "utf8")
    }))
  );
  return artifactsJson.map(({ filename, json }) => {
    const artifact = parseArtifact(json);
    if (artifact instanceof type2.errors) {
      debug2(`Skipping invalid artifact at "${filename}":
...
`
    ...
    const placeholders = Object.entries(artifact.bytecode.linkReferences ?? {}).flatMap(
      ([sourcePath2, names]) => Object.entries(names).flatMap(([name2, slices]) => slices.map((slice) => ({ sourcePath: sourcePath2, name: name2, ...slice })))
    );
    const pendingBytecode = [];
    let offset = 0;
    for (const { sourcePath: sourcePath2, name: name2, start, length } of placeholders) {
      pendingBytecode.push(sliceHex(bytecode, offset, start));
      pendingBytecode.push({ sourcePath: sourcePath2, name: name2 });
      offset = start + length;
    }
...
// ts/node/buildSystemsManifest.ts
  ...
  systems: [
    {
      ...
      namespace: "string",
      name: "string",
      systemId: ["string", ":", (s) => isHex3(s, { strict: false })],
      // abi
      abi: "string[]",
      worldAbi: "string[]"
    },
...
async function buildSystemsManifest(opts) {
  ...
  const manifest = {
    systems: systems.map((system) => {
      ...
      const worldAbi = system.deploy.registerWorldFunctions ? abi.map(
        (item) => item.type === "function" ? { ...item, name: system.namespace ? `${system.namespace}__${item.name}` : item.name } : item
      ) : [];
      return {
        // labels
  ...
  const outFile = path6.join(opts.rootDir, systemsManifestFilename);
  await mkdir(path6.dirname(outFile), { recursive: true });
  await writeFile(outFile, JSON.stringify(manifest, null, 2) + "\n");
  debug2("Wrote systems manifest to", systemsManifestFilename);
}
...
var parseManifest = type4("string").pipe.try((s) => JSON.parse(s), SystemsManifest);
...
export {
  SystemsManifest,
  buildSystemsManifest,
  contractSizeLimit,
  findContractArtifacts,
  findSolidityFiles,
  getSystemContracts,
  loadSystemsManifest,
