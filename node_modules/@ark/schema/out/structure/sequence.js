...
export class SequenceNode extends BaseConstraint {
    ...
    reduceJsonSchema(schema) {
        ...
        if (this.optionals.length) {
            throwParseError(writeUnsupportedJsonSchemaTypeMessage(`Optional tuple element${this.optionals.length > 1 ? "s" : ""} ${this.optionals.join(", ")}`));
        }
        if (this.variadic) {
        ...
        if (this.postfix.length) {
            throwParseError(writeUnsupportedJsonSchemaTypeMessage(`Postfix tuple element${this.postfix.length > 1 ? "s" : ""} ${this.postfix.join(", ")}`));
        }
        return schema;
    }
}
