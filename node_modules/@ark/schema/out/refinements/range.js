...
export const parseDateLimit = (limit) => typeof limit === "string" || typeof limit === "number" ?
    new Date(limit)
    : limit;
export const writeNonIntegerLengthBoundMessage = (kind, limit) => `${kind} bound must be an integer (was ${limit})`;
export const writeNegativeLengthBoundMessage = (kind, limit) => `${kind} bound must be an integer (was ${limit})`;
export const createLengthRuleParser = (kind) => (limit) => {
    if (!Number.isInteger(limit))
        throwParseError(writeNonIntegerLengthBoundMessage(kind, limit));
    if (limit < 0)
        throwParseError(writeNegativeLengthBoundMessage(kind, limit));
    return limit;
};
