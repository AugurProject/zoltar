import { EipNotEnabledError, InvalidArgsError, InvalidGasLimitError, InvalidParamsError, InvalidTransactionError, InsufficientFundsError, InvalidGasPriceError, NonceTooLowError, NonceTooHighError, InternalError, GasLimitExceededError, MisconfiguredClientError, InvalidBlobVersionedHashesError, BlockGasLimitExceededError } from '@tevm/errors';
import { createAddressFromString, hexToBytes, EthjsAccount, setLengthLeft, toBytes, createAccount, bytesToUnprefixedHex, equalsBytes, BIGINT_0, MAX_UINT64, SECP256K1_ORDER_DIV_2, BIGINT_1, eoaCode7702RecoverAuthority, concatBytes, EthjsAddress, KECCAK256_RLP, parseGwei, bytesToHex, Withdrawal, toType, TypeOutput, bytesToBigInt } from '@tevm/utils';
...
import { Capability, isBlobEIP4844Tx, BlobEIP4844Transaction } from '@evmts/zevm/tx';
import { createEvm } from '@tevm/evm';
import { EventEmitter } from 'eventemitter3';

// src/actions/accumulateParentBeaconBlockRoot.ts
var parentBeaconBlockRootAddress = createAddressFromString("0x000F3df6D732807Ef1319fB7B8bB8522d0Beac02");
...
var defaultHistoryStorageAddress = "0x0000F90827F1C53a10cb7A02335B175320002935";
var defaultHistoryServeWindow = 8191n;
var getHistoryStorageAddress = (vm) => {
  try {
    const address = vm.common.ethjsCommon.param("historyStorageAddress");
    return typeof address === "string" ? address : defaultHistoryStorageAddress;
  } catch {
    return defaultHistoryStorageAddress;
  }
};
var getHistoryServeWindow = (vm) => {
  try {
    const window = vm.common.ethjsCommon.param("historyServeWindow");
...
var KECCAK256_NULL = hexToBytes("0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470");
...
var generateTxReceipt = (vm) => async (tx, txResult, cumulativeGasUsed, blobGasUsed, blobGasPrice) => {
  const baseReceipt = {
...
function txLogsBloom(logs, common) {
  const bloom = new Bloom(void 0, common?.ethjsCommon);
  if (logs) {
...
var validateRunTx = (vm) => {
  const validateOpts = async (opts) => {
    const _opts = (
      ...
      {
        ...opts,
        block: opts.block !== void 0 ? opts.block : Block.fromBlockData({ header: {} }, { common: vm.common }),
    ...
    if (_opts.tx.supports(Capability.EIP2718TypedTransaction) && !vm.common.ethjsCommon.isActivatedEIP(2718)) {
      const msg = errorMsg2("Cannot run transaction: EIP 2718 is not activated.", _opts.block, _opts.tx);
    ...
    if (_opts.tx.supports(Capability.EIP2718TypedTransaction)) {
      ...
      if (_opts.tx instanceof BlobEIP4844Transaction && !vm.common.ethjsCommon.isActivatedEIP(4844)) {
        const msg = errorMsg2("Cannot run transaction: EIP 4844 is not activated.", _opts.block, _opts.tx);
        throw new EipNotEnabledError(msg);
      }
    }
    if (_opts.tx.supports(Capability.EIP7702EOACode) && !vm.common.ethjsCommon.isActivatedEIP(7702)) {
      const msg = errorMsg2("Cannot run transaction: EIP 7702 is not activated.", _opts.block, _opts.tx);
      throw new EipNotEnabledError(msg);
    }
    return _opts;
  };
...
var DELEGATION_7702_FLAG = new Uint8Array([239, 1, 0]);
var DELEGATION_7702_CODE_SIZE = DELEGATION_7702_FLAG.length + 20;
var isEip7702DelegationCode = (code) => code.length === DELEGATION_7702_CODE_SIZE && equalsBytes(code.slice(0, DELEGATION_7702_FLAG.length), DELEGATION_7702_FLAG);
var eip7702BytesToBigInt = (bytes) => bytes.length === 0 ? BIGINT_0 : bytesToBigInt(bytes);
...
var _runTx = (vm) => async (opts) => {
  ...
  if (vm.common.ethjsCommon.isActivatedEIP(1559)) {
    ...
    if (maxFeePerGas < baseFeePerGas) {
      const msg = errorMsg2(
        `Transaction's ${"maxFeePerGas" in tx ? "maxFeePerGas" : "gasPrice"} (${maxFeePerGas}) is less than the block's baseFeePerGas (${baseFeePerGas})`,
        block,
        tx
      );
  ...
  if (vm.common.ethjsCommon.isActivatedEIP(3607) && !equalsBytes(fromAccount.codeHash, KECCAK256_NULL)) {
    ...
    if (isActive7702) {
      ...
      if (!isEip7702DelegationCode(code)) {
        const msg = errorMsg2("invalid sender address, address is not EOA (EIP-3607)", block, tx);
  ...
  if (tx instanceof BlobEIP4844Transaction) {
    if (!vm.common.ethjsCommon.isActivatedEIP(4844)) {
      const msg = errorMsg2("blob transactions are only valid with EIP4844 active", block, tx);
      throw new EipNotEnabledError(msg);
    }
    const castTx = tx;
    ...
    if (opts.block === void 0) {
      const msg = errorMsg2("Block option must be supplied to compute blob gas price", block, tx);
      throw new InvalidParamsError(msg);
    }
  ...
  if (tx instanceof BlobEIP4844Transaction) {
    const rawHashes = tx.blobVersionedHashes;
  ...
  if (tx.supports(Capability.EIP7702EOACode)) {
    ...
    for (let i = 0; i < authorizationList.length; i++) {
      ...
      const accountExists = accountMaybeUndefined !== void 0;
      ...
      if (accountExists) {
        const refund = BigInt(vm.common.ethjsCommon.param("perEmptyAccountCost")) - BigInt(vm.common.ethjsCommon.param("perAuthBaseGas"));
        gasRefund += refund;
      }
      account.nonce++;
...
var applyTransactions = (vm) => async (block, opts) => {
  const bloom = new Bloom(void 0, vm.common.ethjsCommon);
  let gasUsed = 0n;
  let receiptTrie;
  ...
  const receiptsRoot = receiptTrie !== void 0 ? receiptTrie.root() : KECCAK256_RLP;
  return {
    bloom,
    gasUsed,
...
async function applyDAOHardfork(evm) {
  ...
  for (const addr of DAOAccountList) {
    const address = new EthjsAddress(hexToBytes(addr));
...
var BlockBuilder = class {
  ...
  constructor(vm, opts) {
    ...
    if (opts.withdrawals !== void 0 && this.vm.common.ethjsCommon.isActivatedEIP(4895) !== true) {
      throw new EipNotEnabledError("eip4895 not activated yet for building a block with withdrawals");
    }
    this.withdrawals = opts.withdrawals?.map((w) => Withdrawal.fromWithdrawalData(w));
   ...
   */
  logsBloom() {
    const bloom = new Bloom(void 0, this.vm.common.ethjsCommon);
  ...
  async rewardMiner() {
    ...
    const coinbase = this.headerData.coinbase !== void 0 ? new EthjsAddress(
      this.headerData.coinbase instanceof EthjsAddress ? this.headerData.coinbase.toBytes() : typeof this.headerData.coinbase === "string" ? hexToBytes(this.headerData.coinbase) : this.headerData.coinbase
  ...
  async addTransaction(tx, {
    ...
    const blockGasLimit = toType(gasLimit, TypeOutput.BigInt) ?? 0n;
  ...
  async build(sealOpts) {
    ...
    const withdrawalsRoot = this.withdrawals ? await Block.genWithdrawalsTrieRoot(this.withdrawals, new Trie({ common: this.vm.common.ethjsCommon })) : void 0;
    ...
    this.blockStatus = { status: "build" /* Build */, block };
    if (this.checkpointed) {
  ...
  async initState() {
    ...
    if (this.vm.common.ethjsCommon.isActivatedEIP(2935)) {
      ...
      const numberBigInt = toType(numberValue, TypeOutput.BigInt) ?? 0n;
      const parentHashSanitized = parentHash ? toType(parentHash, TypeOutput.Uint8Array) : new Uint8Array(32);
...
var UNSUPPORTED_VERKLE_EXECUTION_MESSAGE = "Verkle/state-witness execution paths (EIP-6800 family) are intentionally unsupported in Tevm";
var setRunBlockHardfork = (vm, block, setHardfork) => {
  if (setHardfork === void 0 || setHardfork === false) {
    return;
  }
  const previousVmHardfork = vm.common.ethjsCommon.hardfork();
  const evmCommon = vm.evm.common;
  const previousEvmHardfork = evmCommon !== vm.common.ethjsCommon ? evmCommon?.hardfork?.() : void 0;
  const previousBlockHardfork = block.common.ethjsCommon !== vm.common.ethjsCommon ? block.common.ethjsCommon.hardfork() : void 0;
  const hardforkOpts = {
  ...
  return () => {
    ...
    if (previousEvmHardfork !== void 0) {
      evmCommon.setHardfork(previousEvmHardfork);
    }
    if (previousBlockHardfork !== void 0) {
...
var runBlock = (vm) => async (opts) => {
  ...
  try {
    ...
    await vm.evm.journal.checkpoint();
    checkpointed = true;
    if (vm.common.ethjsCommon.hardforkIsActiveOnBlock("dao", block.header.number) === true && block.header.number === vm.common.ethjsCommon.hardforkBlock("dao")) {
    ...
    const stateRoot2 = await state.getStateRoot();
    if (generateFields) {
      const bloom = result.bloom.bitvector;
      const gasUsed = result.gasUsed;
      const receiptTrie = result.receiptsRoot;
      const transactionsTrie = await genTxTrie(block);
      const generatedFields = { stateRoot: stateRoot2, bloom, gasUsed, receiptTrie, transactionsTrie };
      const blockData = {
        ...block,
      ...
      if (!(equalsBytes(stateRoot2, block.header.stateRoot) === true)) {
        const msg = errorMsg(
          `invalid block stateRoot, got: ${bytesToHex(stateRoot2)}, want: ${bytesToHex(block.header.stateRoot)}`,
          vm,
          block
        );
        throw new InternalError(msg);
...
export { BlockBuilder, BuildStatus, DAOConfig, KECCAK256_NULL, accumulateParentBeaconBlockRoot, accumulateParentBlockHash, applyBlock, applyDAOHardfork, applyTransactions, assignBlockRewards, assignWithdrawals, buildBlock, calculateMinerReward, calculateOmmerReward, createVm, deepCopy, execHardfork, genTxTrie, generateTxReceipt, parentBeaconBlockRootAddress, rewardAccount, runBlock, runTx, txLogsBloom, validateRunTx, warmAddresses2929 };
