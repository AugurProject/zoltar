...
var receipt = require('@evmts/zevm/receipt');
var rlp = require('@evmts/zevm/rlp');
var trie = require('@evmts/zevm/trie');
...
var accumulateParentBlockHash = (vm) => async (currentBlockNumber, parentHash) => {
  ...
  const historyAddress = utils.createAddressFromString(getHistoryStorageAddress(vm));
...
function txLogsBloom(logs, common) {
  const bloom = new receipt.Bloom(void 0, common?.ethjsCommon);
  if (logs) {
...
var validateRunTx = (vm) => {
  const validateOpts = async (opts) => {
    ...
    if (_opts.tx.supports(tx.Capability.EIP2718TypedTransaction)) {
      if (_opts.tx.supports(tx.Capability.EIP2930AccessLists) && !vm.common.ethjsCommon.isActivatedEIP(2930)) {
      ...
      if (_opts.tx instanceof tx.BlobEIP4844Transaction && !vm.common.ethjsCommon.isActivatedEIP(4844)) {
        const msg = errorMsg2("Cannot run transaction: EIP 4844 is not activated.", _opts.block, _opts.tx);
        throw new errors.EipNotEnabledError(msg);
      }
    }
    if (_opts.tx.supports(tx.Capability.EIP7702EOACode) && !vm.common.ethjsCommon.isActivatedEIP(7702)) {
...
var isEip7702DelegationCode = (code) => code.length === DELEGATION_7702_CODE_SIZE && utils.equalsBytes(code.slice(0, DELEGATION_7702_FLAG.length), DELEGATION_7702_FLAG);
var eip7702BytesToBigInt = (bytes) => bytes.length === 0 ? utils.BIGINT_0 : utils.bytesToBigInt(bytes);
...
var _runTx = (vm) => async (opts) => {
  ...
  if (vm.common.ethjsCommon.isActivatedEIP(3607) && !utils.equalsBytes(fromAccount.codeHash, KECCAK256_NULL)) {
    ...
    if (isActive7702) {
      ...
      if (!isEip7702DelegationCode(code)) {
        const msg = errorMsg2("invalid sender address, address is not EOA (EIP-3607)", block, tx$1);
  ...
  if (tx$1.supports(tx.Capability.EIP7702EOACode)) {
    ...
    for (let i = 0; i < authorizationList.length; i++) {
      ...
      if (authChainIdBN !== utils.BIGINT_0 && authChainIdBN !== BigInt(vm.common.ethjsCommon.chainId())) {
        continue;
      }
      const delegateAddress = authTuple[1];
      const authNonce = authTuple[2];
      const authNonceBN = eip7702BytesToBigInt(authNonce);
      if (authNonceBN >= utils.MAX_UINT64) {
        continue;
      }
      const authS = authTuple[5];
      if (eip7702BytesToBigInt(authS) > utils.SECP256K1_ORDER_DIV_2) {
        continue;
      }
      const yParity = eip7702BytesToBigInt(authTuple[3]);
      if (yParity > utils.BIGINT_1) {
        continue;
      }
      let authority;
      try {
        authority = utils.eoaCode7702RecoverAuthority(authTuple);
      } catch {
        continue;
      }
      const accountMaybeUndefined = await vm.stateManager.getAccount(authority);
      const accountExists = accountMaybeUndefined !== void 0;
      const account = accountMaybeUndefined ?? new utils.EthjsAccount();
      ...
      if (caller.toString() === authority.toString()) {
        if (account.nonce + utils.BIGINT_1 !== authNonceBN) {
          continue;
        }
      ...
      if (utils.equalsBytes(delegateAddress, new Uint8Array(20))) {
        await vm.stateManager.putCode(authority, new Uint8Array());
      } else {
        const addressCode = utils.concatBytes(DELEGATION_7702_FLAG, delegateAddress);
  ...
  gasRefund += results.execResult.gasRefund ?? utils.BIGINT_0;
  const maxRefundQuotient = BigInt(vm.common.ethjsCommon.param("maxRefundQuotient"));
  if (gasRefund !== utils.BIGINT_0) {
...
var applyTransactions = (vm) => async (block, opts) => {
  const bloom = new receipt.Bloom(void 0, vm.common.ethjsCommon);
  let gasUsed = 0n;
  let receiptTrie;
  if (block.transactions.length !== 0) {
    receiptTrie = new trie.Trie({ common: vm.common.ethjsCommon });
  }
  const receipts = [];
  const txResults = [];
  for (let txIdx = 0; txIdx < block.transactions.length; txIdx++) {
    ...
    const encodedReceipt = receipt.encodeReceipt(txRes.receipt, tx.type);
    if (receiptTrie) {
...
var BlockBuilder = class {
  ...
  constructor(vm, opts) {
    ...
    if (opts.withdrawals !== void 0 && this.vm.common.ethjsCommon.isActivatedEIP(4895) !== true) {
      throw new errors.EipNotEnabledError("eip4895 not activated yet for building a block with withdrawals");
    }
   ...
   */
  logsBloom() {
    const bloom = new receipt.Bloom(void 0, this.vm.common.ethjsCommon);
  ...
  async receiptTrie() {
    ...
    for (const [i, txResult] of this.transactionResults.entries()) {
      ...
      const encodedReceipt = receipt.encodeReceipt(txResult.receipt, tx.type);
...
var runBlock = (vm) => async (opts) => {
  ...
  const restoreHardfork = setRunBlockHardfork(vm, block$1, opts.setHardfork);
  let result;
  let previousRoot;
  ...
  try {
    ...
    isVerkleExecutionEnabled = block$1.common.ethjsCommon.isActivatedEIP(6800);
    if (isVerkleExecutionEnabled) {
      throw new errors.InvalidParamsError(UNSUPPORTED_VERKLE_EXECUTION_MESSAGE);
    }
    ...
    if (generateFields) {
      ...
      if (!(utils.equalsBytes(stateRoot2, block$1.header.stateRoot) === true)) {
        const msg = errorMsg(
          `invalid block stateRoot, got: ${utils.bytesToHex(stateRoot2)}, want: ${utils.bytesToHex(block$1.header.stateRoot)}`,
          vm,
          block$1
        );
        throw new errors.InternalError(msg);
