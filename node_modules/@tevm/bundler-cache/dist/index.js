...
var cacheHash = (value) => createHash("sha256").update(value).digest("hex");
...
var getArtifactsPath = (entryModuleId, item, cwd, cacheDir) => {
  ...
  const cacheRoot = path.isAbsolute(cacheDir) ? path.join(path.resolve(cacheDir), "__projects__", cacheHash(resolvedCwd).slice(0, 16)) : path.resolve(cwd, cacheDir);
  ...
  const normalizedEntryModuleId = isOutsideCwd ? path.join(
    "__external__",
    entryPath.replace(/^[A-Za-z]:/, "").split(path.sep).filter(Boolean).join(path.sep)
  ) : relativeEntryPath;
