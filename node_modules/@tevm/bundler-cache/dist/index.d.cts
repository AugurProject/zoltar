import { statSync, mkdirSync, renameSync } from 'node:fs';
import { writeFile, stat, mkdir, rename } from 'node:fs/promises';
...
type ReadArtifactsSync = (entryModuleId: string, compileFingerprint?: string) => ResolvedArtifacts | undefined;
type ReadArtifacts = (entryModuleId: string, compileFingerprint?: string) => Promise<ResolvedArtifacts | undefined>;
...
type WriteArtifactsSync = (entryModuleId: string, artifacts: ResolvedArtifacts, compileFingerprint?: string) => string;
type WriteArtifacts = (entryModuleId: string, artifacts: ResolvedArtifacts, compileFingerprint?: string) => Promise<string>;
