...
import { TransactionFactory, createTxFromBlockBodyData, createTxFromRLP, Capability, BlobEIP4844Transaction } from '@evmts/zevm/tx';
import { tevmDefault, ConsensusAlgorithm, ConsensusType } from '@tevm/common';
import { concatBytes, numberToHex, keccak256, KECCAK256_RLP_ARRAY, EthjsAddress, KECCAK256_RLP, equalsBytes, bytesToHex, setLengthLeft, toBytes, bytesToBigInt, ecrecover, hexToBytes, bytesToUtf8, createWithdrawal, toType, TypeOutput, isHex } from '@tevm/utils';
...
var SHA256_EMPTY = Uint8Array.from([
  227,
  176,
  196,
  66,
...
var BlockHeader = class _BlockHeader {
  ...
  static fromValuesArray(values, opts) {
    ...
    const { baseFeePerGas, withdrawalsRoot, excessBlobGas, blobGasUsed, parentBeaconBlockRoot, requestsRoot } = headerData;
    ...
    if (header.common.ethjsCommon.isActivatedEIP(4895) && withdrawalsRoot === void 0) {
  ...
  constructor(headerData, opts) {
    ...
    const hardforkDefaults = {
      ...
      requestsRoot: this.common.ethjsCommon.isActivatedEIP(7685) ? SHA256_EMPTY.slice() : void 0
    };
...
// src/block.ts
var UNSUPPORTED_VERKLE_EXECUTION_MESSAGE = "Verkle/state-witness execution paths (EIP-6800 family) are not supported in the current Tevm VM/state pipeline";
var SHA256_EMPTY2 = Uint8Array.from([
  227,
  176,
  196,
  66,
...
var rotr = (word, bits) => word >>> bits | word << 32 - bits;
var sha256 = (message) => {
  if (message.length === 0) {
    return SHA256_EMPTY2.slice();
  }
...
var isTypedTransaction = (tx) => typeof tx === "object" && tx !== null && "serialize" in tx && typeof tx.serialize === "function" && "supports" in tx && typeof tx.supports === "function";
var Block = class _Block {
   ...
   */
  static computeRequestsHash(requests) {
    const nonEmptyRequestHashes = [];
    ...
    return nonEmptyRequestHashes.length === 0 ? SHA256_EMPTY2.slice() : sha256(concatBytes(...nonEmptyRequestHashes));
  }
  static async genRequestsTrieRoot(requests, _emptyTrie) {
    return _Block.computeRequestsHash(requests);
  }
  /**
  ...
  static fromBlockData(blockData, opts) {
    ...
    for (const txData of txsData ?? []) {
      transactions.push(
        isTypedTransaction(txData) ? txData : TransactionFactory(txData, {
          ...opts,
  ...
  static fromValuesArray(values, opts) {
    ...
    if (header.common.ethjsCommon.isActivatedEIP(7685)) {
      if (requestBytes === void 0 || !Array.isArray(requestBytes)) {
  ...
  async requestsTrieIsValid() {
    ...
    if (this.requests?.length === 0) {
      result = equalsBytes(this.header.requestsRoot, SHA256_EMPTY2);
      return result;
    }
    if (this.cache.requestsRoot === void 0) {
      this.cache.requestsRoot = _Block.computeRequestsHash(this.requests ?? []);
    }
