import { gen, die, fail, map, all, succeed, try as _try, tryPromise, async } from 'effect/Effect';
...
var resolveImportPath = (absolutePath, importPath, remappings, libs, sync) => {
  for (const [key, value] of Object.entries(remappings).sort(([a], [b]) => b.length - a.length)) {
    if (importPath.startsWith(key)) {
      return succeed(formatPath(resolve(importPath.replace(key, value))));
    }
  }
  if (isImportLocal(importPath)) {
    return succeed(formatPath(resolve(dirname(absolutePath), importPath)));
  }
  if (sync) {
    return _try({
      try: () => resolve$1.sync(importPath, {
...
var isInBlockComment = (source, offset) => source.lastIndexOf("/*", offset) > source.lastIndexOf("*/", offset);
