import { StorageCache, CacheType, AccountCache } from '@evmts/zevm/statemanager';
...
import { toBytes, bytesToHex as bytesToHex$1, getAddress, toHex, bytesToUnprefixedHex, createAccount, hexToBytes as hexToBytes$1, isHex, hexToBigInt, keccak256 as keccak256$1 } from '@tevm/utils';
...
var getAccount = (baseState, skipFetchingFromFork = false) => async (address) => {
  ...
  if (account.nonce === 0n && account.balance === 0n && bytesToHex$1(account.codeHash) === EMPTY_CODE_HASH && bytesToHex$1(account.storageRoot) === EMPTY_STORAGE_ROOT) {
    accounts.put(address, void 0);
...
var createBaseState = (options) => {
  ...
  const state = {
    ...
    forkCache: {
      contracts: new ContractCache(new StorageCache({ size: 1e5, type: CacheType.LRU })),
      accounts: new AccountCache({
        size: 1e5,
        type: CacheType.LRU
      }),
...
var dumpStorageRange = (state) => (_address, _startKey, _limit) => {
  ...
  const sortedStorage = [...storage.entries()].sort(([a], [b]) => {
    const aBigInt = hexToBigInt(
      /** @type {import('@tevm/utils').Hex}*/
      a
    );
    const bBigInt = hexToBigInt(
      /** @type {import('@tevm/utils').Hex}*/
      b
    );
