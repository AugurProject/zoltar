...
var statemanager = require('@evmts/zevm/statemanager');
...
var utils$1 = require('ethereum-cryptography/utils');
...
function fromRlpSerializedAccount(serialized) {
  const decoded = rlp.Rlp.decode(serialized);
  ...
  return utils.createAccount(accountData);
}
...
var getForkClient = ({ options: { fork } }) => {
  ...
  return viem.createPublicClient({
    ...
    transport: () => viem.createTransport({
      type: "tevm",
      key: "tevm",
...
var getAccountFromProvider = (baseState) => async (address) => {
  ...
  const account = fromAccountData({
    ...
    codeHash: utils.toBytes(accountData.codeHash),
    storageRoot: utils.toBytes(accountData.storageHash)
  });
  return account;
};

// src/actions/resolveForkBlockTag.js
var resolveForkBlockTag = async (baseState) => {
  ...
  if (typeof fork.blockTag === "bigint" && fork.blockHash !== void 0) {
    const block2 = await client.getBlock({ blockNumber: fork.blockTag });
    if (block2.hash !== fork.blockHash) {
      throw new Error(`Fork block ${fork.blockTag} changed from ${fork.blockHash} to ${block2.hash}`);
    }
    return;
  }
  const block = typeof fork.blockTag === "bigint" ? await client.getBlock({ blockNumber: fork.blockTag }) : await client.getBlock({ blockTag: fork.blockTag ?? "latest" });
  fork.blockTag = block.number;
  fork.blockHash = /** @type {import('@tevm/utils').Hex} */
  block.hash;
};

// src/actions/getAccount.js
var EMPTY_CODE_HASH = "0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470";
var EMPTY_STORAGE_ROOT = "0x56e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622fb5e363b421";
var getAccount = (baseState, skipFetchingFromFork = false) => async (address) => {
  ...
  if (elem !== void 0) {
    return (
      /** @type {any} */
      elem.accountRLP !== void 0 ? fromRlpSerializedAccount(elem.accountRLP) : void 0
    );
  }
  ...
  if (account.nonce === 0n && account.balance === 0n && utils.bytesToHex(account.codeHash) === EMPTY_CODE_HASH && utils.bytesToHex(account.storageRoot) === EMPTY_STORAGE_ROOT) {
    accounts.put(address, void 0);
...
var getAccountAddresses = (baseState) => () => {
  ...
  baseState.caches.accounts._orderedMapCache?.forEach((e) => {
    accountAddresses.add(utils.getAddress(e[0].startsWith("0x") ? e[0] : `0x${e[0]}`));
  });
  const { _lruCache, _orderedMapCache } = baseState.caches.accounts;
  if (_lruCache !== void 0) {
    for (const address of _lruCache.rkeys()) {
      accountAddresses.add(utils.getAddress(address.startsWith("0x") ? address : `0x${address}`));
    }
  }
...
var getContractCode = (baseState, skipFetchingFromFork = false) => async (address) => {
  ...
  const remoteCodeBytes = viem.hexToBytes(remoteCode);
  contracts.put(address, remoteCodeBytes);
  forkContracts.put(address, remoteCodeBytes);
  baseState.logger.debug(
    { address, deployedBytecode: utils.bytesToHex(remoteCodeBytes) },
...
var dumpCanonicalGenesis = (baseState) => async () => {
  ...
  const state = {};
  for (const address$1 of accountAddresses) {
    const hexAddress = utils.getAddress(address$1.startsWith("0x") ? address$1 : `0x${address$1}`);
    const ethAddress = address.createAddress(hexAddress);
    ...
    const dump = {
      ...
      storageRoot: utils.bytesToHex(account.storageRoot),
      codeHash: utils.bytesToHex(account.codeHash),
      storage,
      ...baseState.caches.contracts.has(ethAddress) ? { deployedBytecode: utils.toHex(deployedBytecode) } : {}
    };
  ...
  const newStateRoot = (() => {
    ...
    for (const [k, v] of Object.entries(state)) {
      ...
      jsonSerializableState[k] = {
        ...
        balance: viem.numberToHex(balance),
        storageRoot,
        codeHash,
        ...deployedBytecode !== void 0 ? { deployedBytecode } : {}
      };
    }
    const root = viem.keccak256(viem.toHex(JSON.stringify(jsonSerializableState)));
...
var ContractCache = class {
  constructor(storageCache = new statemanager.StorageCache({
    size: 1e5,
    type: statemanager.CacheType.LRU
  })) {
   ...
   */
  has(address) {
    const storageMap = this.storageCache._orderedMapCache?.getElementByKey(utils.bytesToUnprefixedHex(address.bytes));
    const hasOrderedMapCache = storageMap?.has(utils.bytesToUnprefixedHex(oneBytes)) ?? false;
    const hasLruCache = this.storageCache._lruCache?.has(utils.bytesToUnprefixedHex(address.bytes));
...
var stripZeros = (bytes) => {
  if (!(bytes instanceof Uint8Array)) {
    throw new errors.InternalError("Unexpected type");
  }
  const firstNonZero = bytes.findIndex(
    /** @param {number} entry*/
    (entry) => entry !== 0
  );
...
var putContractStorage = (baseState) => async (address, key, value) => {
  if (key.length !== 32) {
    throw new errors.InternalError(`Storage key must be 32 bytes long. Received ${key}`);
  }
...
var generateCanonicalGenesis = (baseState) => async (state) => {
  if (baseState.caches.accounts._checkpoints > 0 || baseState.caches.storage._checkpoints > 0 || baseState.caches.contracts._checkpoints > 0) {
    throw new errors.InternalError("Attempted to generateCanonicalGenesis state with uncommitted checkpoints");
  }
  const { caches: oldCaches, tombstones: oldTombstones } = baseState;
  baseState.caches = {
    contracts: new ContractCache(),
    accounts: new statemanager.AccountCache({
      size: 1e5,
      type: statemanager.CacheType.LRU
    }),
    storage: new statemanager.StorageCache({
      size: 1e5,
      type: statemanager.CacheType.LRU
    })
  };
  baseState.tombstones = {
    ...
    storageCleared: /* @__PURE__ */ new Set(),
    checkpoints: []
  };
  try {
    for (const [k, v] of Object.entries(
      ...
      const account = utils.createAccount({
        nonce,
        balance,
        storageRoot: utils.hexToBytes(storageRoot, { size: 32 }),
        codeHash: utils.hexToBytes(codeHash, { size: 32 })
      });
      const address$1 = address.createAddress(k);
      baseState.caches.accounts?.put(
        address$1,
        /** @type {any} */
        account
      );
      if (deployedBytecode) {
        baseState.caches.contracts.put(address$1, utils.hexToBytes(deployedBytecode));
      }
      if (storage !== void 0) {
        for (const [storageKey, storageData] of Object.entries(storage)) {
          const key = utils.hexToBytes(utils.isHex(storageKey) ? storageKey : `0x${storageKey}`);
          const data = (
            /** @type {Uint8Array}*/
            utils.hexToBytes(utils.isHex(storageData) ? storageData : `0x${storageData}`)
          );
          await putContractStorage(baseState)(address$1, key, data);
        }
      }
    }
  } catch (e) {
...
var createBaseState = (options) => {
  ...
  const state = {
    ...
    setCurrentStateRoot: (root) => {
      if (!state.stateRoots.has(root)) {
        throw new errors.InternalError("Cannot set state root to non existing state root");
      }
    ...
    options,
    caches: {
      contracts: options.contractCache ?? new ContractCache(new statemanager.StorageCache({ size: 1e5, type: statemanager.CacheType.LRU })),
      accounts: options.accountsCache ?? new statemanager.AccountCache({
        size: 1e5,
        type: statemanager.CacheType.LRU
      }),
      storage: options.storageCache ?? new statemanager.StorageCache({
        size: 1e5,
        type: statemanager.CacheType.LRU
      })
    },
    forkCache: {
      contracts: new ContractCache(new statemanager.StorageCache({ size: 1e5, type: statemanager.CacheType.LRU })),
      accounts: new statemanager.AccountCache({
        size: 1e5,
        type: statemanager.CacheType.LRU
      }),
  ...
  const genesisPromise = (options.genesisState !== void 0 && options.currentStateRoot === void 0 ? (async () => {
    ...
    await commit(state)();
    logger$1.debug("StateManager is ready");
  });
  return state;
};
...
var deepCopy = (baseState) => async () => {
  ...
  if (baseState.caches.accounts._checkpoints > 0 || baseState.caches.storage._checkpoints > 0 || baseState.caches.contracts._checkpoints > 0) {
    throw new errors.InternalError("Attempted to deepCopy state with uncommitted checkpoints");
  }
...
var deleteAccount = (baseState) => async (address) => {
  ...
  baseState.caches.accounts.put(address, void 0);
...
var dumpStorageRange = (state) => (_address, _startKey, _limit) => {
  ...
  if (!storage) {
    throw new errors.InvalidParamsError(`No storage found at address ${utils.getAddress(_address.toString())}`);
  }
  const entries = [];
  let nextKey = null;
  const sortedStorage = [...storage.entries()].sort(([a], [b]) => {
    const aBigInt = utils.hexToBigInt(
      /** @type {import('@tevm/utils').Hex}*/
      a
    );
    const bBigInt = utils.hexToBigInt(
      /** @type {import('@tevm/utils').Hex}*/
      b
    );
  ...
  for (const [storageKey, storageValue] of sortedStorage) {
    ...
    const value = utils.bytesToHex(storageValue);
...
var getAppliedKey = () => (address) => {
  return viem.keccak256(address, "bytes");
};
var getContractStorage = (baseState) => async (address, key) => {
  ...
  const value = stripZeros(viem.hexToBytes(storage ?? "0x0"));
...
var getProof = (baseState) => async (address, storageSlots = []) => {
  ...
  const proof = await client.getProof({
    ...
    storageKeys: storageSlots.map((slot) => utils.bytesToHex(slot)),
    ...blockTag
  });
  return {
    address: proof.address,
    accountProof: proof.accountProof,
    balance: utils.toHex(proof.balance),
    codeHash: proof.codeHash,
    nonce: utils.toHex(proof.nonce),
    storageHash: proof.storageHash,
    storageProof: proof.storageProof.map((p) => ({
      proof: p.proof,
      value: utils.toHex(p.value),
      key: p.key
    }))
  };
};
var getStateRoot = (baseState) => async () => {
  return utils$1.hexToBytes(baseState.getCurrentStateRoot());
};
var hasStateRoot = (baseState) => (root) => {
  return Promise.resolve(baseState.stateRoots.has(utils.bytesToHex(root)));
};
...
var modifyAccountFields = (baseState) => async (address, accountFields) => {
  ...
  await putAccount(baseState)(
    address,
    /** @type {any} */
    utils.createAccount(accountData)
  );
};
var originalStorageCache = (baseState) => {
  const state = /* @__PURE__ */ new Map();
  const put = (address, key, value) => {
    const addressHex = utils.bytesToUnprefixedHex(address.bytes);
    let map = state.get(addressHex);
    if (map === void 0) {
      map = /* @__PURE__ */ new Map();
      state.set(addressHex, map);
    }
    const keyHex = utils.bytesToUnprefixedHex(key);
...
var putContractCode = (baseState) => async (address, value) => {
  ...
  const accountData = {
    ...
    codeHash: utils.keccak256(value, "bytes")
  };
...
var setStateRoot = (baseState) => async (root) => {
  ...
  const genesis = baseState.stateRoots.get(viem.bytesToHex(root));
  if (!genesis) {
    throw new NoStateRootExistsError(`State root for ${viem.bytesToHex(root)} does not exist`);
  }
  const oldStateRoot = baseState.getCurrentStateRoot();
  try {
    baseState.setCurrentStateRoot(viem.bytesToHex(root));
...
var shallowCopy = (baseState) => () => {
  ...
  newState.caches.storage = new statemanager.StorageCache({
    size: 1e5,
    type: statemanager.CacheType.LRU
  });
  newState.caches.accounts = new statemanager.AccountCache({
    size: 1e5,
    type: statemanager.CacheType.LRU
  });
  if (baseState.forkCache) {
...
var saveStateRoot = (baseState) => (root, value) => {
  baseState.stateRoots.set(utils.bytesToHex(root), value);
  baseState.logger.debug({ root: utils.bytesToHex(root), value }, "Saved state root");
};
...
Object.defineProperty(exports, "AccountCache", {
  enumerable: true,
  get: function () { return statemanager.AccountCache; }
});
Object.defineProperty(exports, "CacheType", {
  enumerable: true,
  get: function () { return statemanager.CacheType; }
});
Object.defineProperty(exports, "StorageCache", {
  enumerable: true,
  get: function () { return statemanager.StorageCache; }
});
exports.ContractCache = ContractCache;
exports.checkpoint = checkpoint;
exports.clearCaches = clearCaches;
exports.clearContractStorage = clearContractStorage;
exports.commit = commit;
exports.createBaseState = createBaseState;
exports.createStateManager = createStateManager;
exports.deepCopy = deepCopy;
exports.deleteAccount = deleteAccount;
exports.dumpCanonicalGenesis = dumpCanonicalGenesis;
exports.dumpStorage = dumpStorage;
exports.dumpStorageRange = dumpStorageRange;
exports.generateCanonicalGenesis = generateCanonicalGenesis;
exports.getAccount = getAccount;
exports.getAccountAddresses = getAccountAddresses;
exports.getAccountFromProvider = getAccountFromProvider;
exports.getAppliedKey = getAppliedKey;
exports.getContractCode = getContractCode;
exports.getContractStorage = getContractStorage;
exports.getForkBlockTag = getForkBlockTag;
exports.getForkClient = getForkClient;
exports.getProof = getProof;
exports.getStateRoot = getStateRoot;
exports.hasStateRoot = hasStateRoot;
...
exports.putContractCode = putContractCode;
exports.putContractStorage = putContractStorage;
exports.revert = revert;
exports.setStateRoot = setStateRoot;
