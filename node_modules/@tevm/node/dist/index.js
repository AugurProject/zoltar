...
import { createStateManager, dumpCanonicalGenesis } from '@tevm/state';
import { TxPool } from '@tevm/txpool';
import { parseEther, bytesToHex, hexToBigInt, keccak256, KECCAK256_RLP, numberToHex, hexToBytes, hexToNumber, toHex, getAddress } from '@tevm/utils';
...
var INITIAL_BALANCE = parseEther("1000");
...
var createTevmNode = (options = {}) => {
  ...
  const getStateManagerOpts = async () => {
    ...
    return {
      ...
      ...options.persister !== void 0 ? { onCommit: statePersister(options.persister, logger) } : {}
    };
  };
  ...
  const fetchForkAnchor = async (transport2, blockTag) => {
    ...
    const fetcher = createJsonRpcFetcher(transport2);
  ...
  const stateManagerPromise = blockchainPromise.then(async (blockchain) => {
    ...
    return createStateManager({
      ...
      stateRoots: /* @__PURE__ */ new Map([[bytesToHex(headBlock.header.stateRoot), genesisState]]),
      genesisState
    });
  });
  ...
  const receiptManagerPromise = vmPromise.then((vm) => {
    ...
    return new ReceiptsManager(createMapDb({ cache: /* @__PURE__ */ new Map() }), vm.blockchain);
  });
  ...
  const deepCopy = (baseClient2) => async () => {
    ...
    const setImpersonatedAccount2 = (address) => {
      impersonatedAccount2 = address && getAddress(address);
    };
  ...
  const minePendingTransactions = async (client) => {
    ...
    const mixHash = overridePrevRandao !== void 0 ? hexToBytes(`0x${overridePrevRandao.toString(16).padStart(64, "0")}`) : void 0;
...
export { GENESIS_STATE, ProviderRpcError, createIntervalMiner, createTevmNode, prefundedAccounts };
