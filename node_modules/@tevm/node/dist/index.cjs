...
var receiptManager$1 = require('@evmts/zevm/receipt-manager');
...
// src/createIntervalMiner.js
var createIntervalMiner = (client) => {
  let timeoutId;
  let isRunning = false;
  let miningCallback;
  ...
  const scheduleNext = () => {
    ...
    const blockTime = client.miningConfig.blockTime * 1e3;
    if (blockTime <= 0) {
...
var chainIdToLightSyncNetwork = (chainId) => {
  if (chainId === 1) return "mainnet";
  if (chainId === 11155111) return "sepolia";
  if (chainId === 17e3) return "holesky";
  return "unknown";
};
var normalizeSlots = (status) => {
...
var selectStartupCheckpoint = (opts) => {
  const nowMs = opts.nowMs ?? Date.now();
  const strict = opts.strictCheckpointAge ?? false;
  const maxAge = opts.maxCheckpointAgeMs ?? 1e3 * 60 * 60 * 24 * 3;
  const fsOps = opts.fsOps ?? { existsSync: fs.existsSync, readFileSync: fs.readFileSync };
...
var createTevmNode = (options = {}) => {
  ...
  const emitExExEvent = async (event) => {
    for (const hook of exExHooks) {
      try {
        ...
        logger$1.error(e, "ExEx hook failed");
      }
    }
  };
  const transport = (() => {
  ...
  const consensus$1 = options.consensus ?? consensus.createNoopConsensusService();
  ...
  const chainIdPromise = (async () => {
    if (options?.fork?.chainId !== void 0) {
      logger$1.debug({ chainId: options.fork.chainId }, "Using fork chainId override");
  ...
  chainIdPromise.then((chainId) => {
    if (consensus$1.mode !== "light-client" || !consensus$1.updateLightSyncStatus) return;
    const checkpoint = selectStartupCheckpoint(options.lightSync ?? {});
    consensus$1.updateLightSyncStatus({
  ...
  const getLightSyncStatus = () => normalizeSlots(
    consensus$1.getLightSyncStatus?.() ?? {
      ready: true,
      status: "ready",
      network: "unknown",
  ...
  const fetchForkAnchor = async (transport2, blockTag) => {
    if (blockTag === "pending") {
      throw new Error("Cannot use pending as a fork block tag");
    }
    const fetcher = jsonrpc.createJsonRpcFetcher(transport2);
    const { result, error } = await fetcher.request({
      jsonrpc: "2.0",
      id: 1,
      method: "eth_getBlockByNumber",
      params: [typeof blockTag === "bigint" ? utils.numberToHex(blockTag) : blockTag, false]
    });
    if (error) {
      throw error;
    }
    const block = (
      /** @type {{ number?: import('@tevm/utils').Hex; hash?: import('@tevm/utils').Hex }} */
      result ?? {}
    );
    if (!block.number || !block.hash || typeof block.number !== "string" || !block.number.startsWith("0x") || typeof block.hash !== "string") {
      return { blockTag: typeof blockTag === "bigint" ? blockTag : 0n };
    }
    return {
      blockTag: utils.hexToBigInt(block.number),
  ...
  const forkAnchorPromise = (async () => {
    ...
    logger$1.debug(anchor, "fetched fork block anchor from provided forkurl");
    return anchor;
  })();
  ...
  const receiptManagerPromise = vmPromise.then((vm) => {
    ...
    return new receiptManager.ReceiptsManager(receiptManager$1.createMapDb({ cache: /* @__PURE__ */ new Map() }), vm.blockchain);
  });
  ...
  const deepCopy = (baseClient2) => async () => {
    ...
    let copiedMinGasPrice = baseClient2.getMinGasPrice();
    const setCopiedMinGasPrice = (price) => {
      copiedMinGasPrice = price;
    };
    let copiedBlockTimestampInterval = baseClient2.getBlockTimestampInterval();
    const setCopiedBlockTimestampInterval = (interval) => {
      copiedBlockTimestampInterval = interval;
    };
    const copiedSnapshots = new Map(baseClient2.getSnapshots());
    ...
    const copiedClient = {
      ...
      consensus: baseClient2.consensus,
      ...
      setMiningConfig: (config) => {
        ...
        baseClient2.logger.debug({ newConfig: config }, "Copied client mining configuration updated");
      },
      ...
      status: "READY",
      getLightSyncStatus: baseClient2.getLightSyncStatus,
      registerExExHook: baseClient2.registerExExHook,
      emitExExEvent: baseClient2.emitExExEvent,
      close: () => {
        copiedClient.status = "STOPPED";
        baseClient2.logger.debug("Copied TevmNode closed");
      }
    };
    return copiedClient;
  };
  ...
  const minePendingTransactions = async (client) => {
    ...
    } else if (timestampInterval !== void 0) {
    ...
    if (overridePrevRandao !== void 0 && client.setNextBlockPrevRandao) {
    ...
    const state = vm.stateManager._baseState.stateRoots.get(utils.bytesToHex(block.header.stateRoot));
    if (state !== void 0) {
  ...
  const baseClient = {
    ...
    setMiningConfig: (config) => {
      ...
      logger$1.debug({ oldConfig, newConfig: config }, "Mining configuration updated");
    ...
    close: () => {
      ...
      intervalMiner?.stop();
      logger$1.debug("TevmNode closed");
    },
    debug: async () => {
...
exports.createIntervalMiner = createIntervalMiner;
