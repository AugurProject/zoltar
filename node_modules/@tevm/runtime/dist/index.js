...
var escapeJSDoc = (value) => String(value ?? "").replace(/\*\//g, "*\\/").replace(/\r?\n/g, "\n * ");
var generateDtsBody = (artifacts, includeBytecode) => {
  return succeed(
    `
			${Object.entries(artifacts).flatMap(([contractName, { abi, userdoc = {}, evm }]) => {
      var _a, _b;
      const contract = {
      ...
      if (includeBytecode) {
        const bytecodeType = ((_a = evm == null ? void 0 : evm.bytecode) == null ? void 0 : _a.object) && evm.bytecode.object !== "" ? "`0x${string}`" : "undefined";
        const deployedBytecodeType = ((_b = evm == null ? void 0 : evm.deployedBytecode) == null ? void 0 : _b.object) && evm.deployedBytecode.object !== "" ? "`0x${string}`" : "undefined";
        const bytecodeLabel = bytecodeType === "undefined" && deployedBytecodeType === "undefined" ? "no bytecode" : "with bytecode";
        return [
...
var escapeJSDoc2 = (value) => String(value ?? "").replace(/\*\//g, "*\\/").replace(/\r?\n/g, "\n * ");
var generateTevmBody = (artifacts, moduleType, includeBytecode) => {
  ...
  return succeed(
    Object.entries(artifacts).flatMap(([contractName, { abi, userdoc = {}, evm }], i) => {
      ...
      const natspec = Object.entries(userdoc.methods ?? {}).map(
        ([method, { notice }]) => ` * @property ${escapeJSDoc2(method)} ${escapeJSDoc2(notice)}`
      );
      if (userdoc.notice) {
        natspec.unshift(` * ${escapeJSDoc2(userdoc.notice)}`);
      }
