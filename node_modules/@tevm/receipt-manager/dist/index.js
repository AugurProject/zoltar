export { createMapDb } from '@evmts/zevm/receipt-manager';
import { encodeTxHashIndex, decodeTxHashIndex, encodeReceiptLogs, decodeReceiptLogs, encodeStoredReceipts, decodeStoredReceipts, Bloom } from '@evmts/zevm/receipt';
import { getBlock } from '@tevm/blockchain';
...
var ReceiptsManager = class _ReceiptsManager {
  ...
  async saveReceipts(block, receipts) {
    ...
    await this.updateIndex(0 /* Save */, 0 /* TxHash */, block);
  }
  /**
  ...
  async deleteReceipts(block) {
    ...
    await this.updateIndex(1 /* Delete */, 0 /* TxHash */, block);
  }
  ...
  async getLogs(from, to, addresses, topics = []) {
    ...
    for (let i = from.header.number; i <= to.header.number; i++) {
      ...
      const logsForSizing = logs.map(({ log, txIndex, logIndex: logIndex2 }) => ({ log, txIndex, logIndex: logIndex2 }));
    ...
    switch (type) {
      case 0 /* TxHash */: {
        ...
        if (operation === 0 /* Save */) {
          ...
          for (const [i, tx] of block.transactions.entries()) {
            const currentIndex = await this.getIndex(0 /* TxHash */, tx.hash());
    ...
    switch (type) {
      case 0 /* Receipts */: {
        if (conversion === 0 /* Encode */) {
          return encodeStoredReceipts(value);
        }
        return decodeStoredReceipts(value);
      }
      case 1 /* Logs */:
        if (conversion === 0 /* Encode */) {
          return encodeReceiptLogs(value);
        }
        return decodeReceiptLogs(value);
      case 2 /* TxHash */: {
        if (conversion === 0 /* Encode */) {
          return encodeTxHashIndex(value);
        }
        return decodeTxHashIndex(value);
      }
      default:
