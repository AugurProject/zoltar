...
import { blockFromRpc, createBlock } from '@tevm/block';
import { createLogger } from '@tevm/logger';
import { withRetry, bytesToHex as bytesToHex$1 } from 'viem';
import { ConsensusAlgorithm } from '@tevm/common';
import { UnknownBlockError, InvalidBlockError, InternalError } from '@tevm/errors';
import { numberToHex, bytesToHex, bytesToBigInt, isHex, hexToBytes, hexToNumber } from '@tevm/utils';
...
var getBlockFromRpc = async (baseChain, { transport, blockTag = "latest" }, common) => {
  ...
  const asEthjsBlock = (rpcBlock) => {
    const transactions = rpcBlock.transactions?.filter((tx) => {
      ...
      if (tx.chainId !== void 0 && BigInt(tx.chainId) !== BigInt(common.id)) {
        return false;
      }
...
// src/actions/putBlock.js
var getJsonRpcBlockHash = (block) => {
  const hash = (
...
var blockHasHash = (block, hash) => bytesToHex$1(block.hash()) === hash || getJsonRpcBlockHash(block) === hash;
var putBlock = (baseChain) => async (block) => {
...
var getJsonRpcBlockHash2 = (block) => {
  const hash = (
    /** @type {{ __tevmJsonRpcBlockHash?: unknown }} */
    block.__tevmJsonRpcBlockHash
  );
...
var getBlockHashes = (block) => {
  const hashes = [bytesToHex$1(block.hash())];
  const jsonRpcHash = getJsonRpcBlockHash2(block);
  if (jsonRpcHash !== void 0) {
...
var delBlock = (baseChain) => async (blockHash) => {
  ...
  const blocksToDelete = /* @__PURE__ */ new Set([block]);
  ...
  while (foundDescendant) {
    ...
    for (const candidate of new Set(baseChain.blocks.values())) {
      ...
      if (!blocksToDelete.has(candidate) && deletedHashes.has(bytesToHex$1(candidate.header.parentHash))) {
        blocksToDelete.add(candidate);
        foundDescendant = true;
      }
    }
  }
  const parent = await getBlock(baseChain)(block.header.parentHash).catch(() => void 0);
...
var getBlockByTag = (baseChain) => async (blockId) => {
  ...
  if (!block && baseChain.options.fork?.transport) {
    ...
    const forkedBlock = baseChain.blocksByTag.get("forked");
    const latestBlock = await getCanonicalHeadBlock(baseChain)();
    if (forkedBlock && block2.header.number > forkedBlock.header.number) {
      throw new UnknownBlockError(
        `The fetched block ${block2.header.number} has a higher block height than the forked block ${forkedBlock.header.number} but less than the latest block ${latestBlock.header.number}`
      );
    }
...
var setIteratorHead = (baseChain) => (tag, headHash) => {
  const block = baseChain.blocks.get(bytesToHex$1(headHash));
  if (!block) {
    return Promise.reject(new InvalidBlockError(`No block with hash ${bytesToHex$1(headHash)} exists`));
  }
...
var createChain = async (options) => {
  const decorate = (baseChain2) => {
    return (
      /** @type {import('./Chain.js').Chain} */
      Object.assign(baseChain2, {
        getBlockByTag: getBlockByTag(baseChain2),
        deepCopy: async () => decorate(await deepCopy(baseChain2)()),
        shallowCopy: () => decorate(shallowCopy(baseChain2)()),
        getBlock: getBlock(baseChain2),
        putBlock: putBlock(baseChain2),
        validateHeader: validateHeader(baseChain2),
        getCanonicalHeadBlock: getCanonicalHeadBlock(baseChain2),
        delBlock: delBlock(baseChain2),
        getIteratorHead: getIteratorHead(baseChain2),
        setIteratorHead: setIteratorHead(baseChain2),
        consensus: new CasperConsensus(baseChain2.common.ethjsCommon.consensusAlgorithm()),
        iterator: () => {
...
export { createBaseChain, createChain, deepCopy, delBlock, getBlock, getBlockByTag, getBlockFromRpc, getCanonicalHeadBlock, getIteratorHead, isTevmBlockTag, putBlock, setIteratorHead, shallowCopy, validateHeader };
