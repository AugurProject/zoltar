...
var mergeConfigs = (configs) => {
  ...
  return Effect.succeed({
    ...
    foundryProject: (_a = configsByPrecedence.find((config) => config.foundryProject !== void 0)) == null ? void 0 : _a.foundryProject,
    libs: [...new Set(configs.flatMap((config) => config.libs ?? []))],
    debug: (_b = configsByPrecedence.find((config) => config.debug !== void 0)) == null ? void 0 : _b.debug,
    cacheDir: (_c = configsByPrecedence.find((config) => config.cacheDir !== void 0)) == null ? void 0 : _c.cacheDir
  });
};
...
var loadFoundryConfig = (foundryProject, configFilePath) => {
  ...
  let stdout;
  try {
    stdout = child_process.execFileSync(forgeCommand, ["config", "--json"], {
      cwd: configFilePath
    }).toString();
  } catch (cause) {
    console.error(cause);
  ...
  if (forgeConfig.remappings) {
    for (const remap of forgeConfig.remappings) {
      ...
      remappings[key.trim()] = path4__namespace.join(configFilePath, value.trim());
    }
  }
  return Effect.tap(
    Effect.succeed({
...
var loadJsonConfig = (configFilePath) => {
  const tevmConfigPath0 = path4__namespace.join(configFilePath, "tevm.config.json");
  const tevmConfigPath1 = path4__namespace.join(configFilePath, "tevm.json");
...
var loadRemappings = (configFilePath) => {
  const remappingsPath = path4__namespace.join(configFilePath, "remappings.txt");
  const readConfig = (configPath) => Effect.try({
    try: () => fs.readFileSync(configPath, "utf8"),
...
var getTevmConfigFromTsConfig = (tsConfig, configPath) => {
  ...
  const configDir = path4__namespace.default.resolve(configPath);
  const baseDir = baseUrl ? path4__namespace.default.resolve(configDir, baseUrl) : configDir;
  const pathRemappings = Object.fromEntries(
    Object.entries(paths ?? {}).map(([key, value]) => [
      key.replace(/\/\*$/, "/"),
      (value == null ? void 0 : value[0]) ? `${path4__namespace.default.resolve(baseDir, value[0].replace(/\/\*$/, "")).replace(/\\/g, "/")}/` : ""
    ])
  );
  return validateUserConfig(() => plugin).pipe(
    ...
    Effect.map(
      (config) => baseUrl ? {
        ...
        libs: [.../* @__PURE__ */ new Set([baseDir, ...config.libs ?? []])]
      } : config
    ),
    Effect.tap((config) => {
...
var loadTsConfig = (configFilePath) => {
  const tsConfigPath = path4__namespace.join(configFilePath, "tsconfig.json");
  const jsConfigPath = path4__namespace.join(configFilePath, "jsconfig.json");
  return (
    /** @type {import("effect/Effect").Effect<TsConfig, LoadTsConfigError, never>} */
    Effect.try({
      ...
      Effect.catchTag("ParseJsonError", (cause) => Effect.fail(new InvalidTsConfigError({ cause }))),
      Effect.tap((tsConfig) => Effect.logDebug(`loading tsconfig from ${configFilePath}: ${JSON.stringify(tsConfig)}`))
    )
  );
};
