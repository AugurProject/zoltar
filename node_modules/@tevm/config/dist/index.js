import { flatMap, all, tap, logDebug, catchTags, catchTag, fail, tapError, try as _try, succeed, die, map } from 'effect/Effect';
...
var loadFoundryConfig = (foundryProject, configFilePath) => {
  ...
  if (forgeConfig.remappings) {
    for (const remap of forgeConfig.remappings) {
      ...
      remappings[key.trim()] = path4.join(configFilePath, value.trim());
    }
  }
  return tap(
    succeed({
...
var loadJsonConfig = (configFilePath) => {
  const tevmConfigPath0 = path4.join(configFilePath, "tevm.config.json");
  const tevmConfigPath1 = path4.join(configFilePath, "tevm.json");
...
var loadRemappings = (configFilePath) => {
  const remappingsPath = path4.join(configFilePath, "remappings.txt");
  const readConfig = (configPath) => _try({
    try: () => readFileSync(configPath, "utf8"),
...
var getTevmConfigFromTsConfig = (tsConfig, configPath) => {
  ...
  const configDir = path4__default.resolve(configPath);
  const baseDir = baseUrl ? path4__default.resolve(configDir, baseUrl) : configDir;
  const pathRemappings = Object.fromEntries(
    Object.entries(paths ?? {}).map(([key, value]) => [
      key.replace(/\/\*$/, "/"),
      (value == null ? void 0 : value[0]) ? `${path4__default.resolve(baseDir, value[0].replace(/\/\*$/, "")).replace(/\\/g, "/")}/` : ""
    ])
  );
...
var loadTsConfig = (configFilePath) => {
  const tsConfigPath = path4.join(configFilePath, "tsconfig.json");
  const jsConfigPath = path4.join(configFilePath, "jsconfig.json");
  return (
