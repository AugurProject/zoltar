...
var generate = async (cwd2 = process.cwd(), include2 = ["src/**/*.sol"]) => {
  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      const tsContent = await plugin.resolveTsModule(`./${file}`, cwd2, false, true);
      await promises.writeFile(path__default.default.join(cwd2, fileDir, `${fileName}.ts`), tsContent.code);
    })
  );
};
