...
var utils_js = require('solidity-ast/utils.js');
...
function convertSolcAstToTsDefinitionInfo(astNode, fileName, containerName, solcInput, ts) {
  ...
  const actualLength = fs.readFileSync(fileName, "utf8").length;
...
var init_solidityModuleResolver = __esm({
  "src/utils/solidityModuleResolver.ts"() {
    ...
    init_isSolidity();
    solidityModuleResolver = (moduleName, ts, createInfo, containingFile, options = {}) => {
      var _a, _b, _c, _d;
      if (isRelativeSolidity(moduleName)) {
        const basedir = options.isRemapped ? options.projectRoot ?? ((_b = (_a = createInfo.project).getCurrentDirectory) == null ? void 0 : _b.call(_a)) ?? path__default.default.dirname(containingFile) : path__default.default.dirname(containingFile);
        return {
          extension: ts.Extension.Dts,
          isExternalLibraryImport: false,
          resolvedFileName: path__default.default.resolve(basedir, moduleName)
        };
      }
      if (isSolidity(moduleName)) {
        const isPathLikeRemap = options.isRemapped && (path__default.default.isAbsolute(moduleName) || moduleName.startsWith("node_modules/") || moduleName.startsWith("lib/") || moduleName.startsWith("src/") || moduleName.startsWith("contracts/"));
        return {
          extension: ts.Extension.Dts,
          isExternalLibraryImport: false,
          resolvedFileName: isPathLikeRemap ? path__default.default.resolve(
            options.projectRoot ?? ((_d = (_c = createInfo.project).getCurrentDirectory) == null ? void 0 : _d.call(_c)) ?? path__default.default.dirname(containingFile),
            moduleName
          ) : module$1.createRequire(path__default.default.dirname(containingFile)).resolve(moduleName)
        };
      }
  ...
  "src/decorators/getDefinitionAtPosition.ts"() {
    ...
    getDefinitionServiceDecorator = (service, config, logger, ts, fao, solcCache) => {
      const getDefinitionAtPosition = (fileName, position) => {
        ...
        const plugin = baseBundler.bundler(config, logger, fao, solc__namespace, solcCache);
        const includedAst = true;
        const configFilePath = (_c = (_b = service.getProgram()) == null ? void 0 : _b.getCompilerOptions) == null ? void 0 : _c.call(_b).configFilePath;
        const projectRoot = typeof configFilePath === "string" ? path__default.default.dirname(configFilePath) : path__default.default.dirname(fileName);
        let asts;
        let solcInput;
        try {
        ...
        for (const [fileName2, ast] of Object.entries(asts)) {
          for (const functionDef of utils_js.findAll("EventDefinition", ast)) {
          ...
          for (const functionDef of utils_js.findAll("FunctionDefinition", ast)) {
            if (functionDef.name === (node == null ? void 0 : node.getText())) {
              definitions.push({
                node: functionDef,
                fileName: fileName2
        ...
        const contractName = ((_d = ContractPath.split("/").pop()) == null ? void 0 : _d.split(".")[0]) ?? "Contract";
        if (!solcInput) {
  ...
  "src/utils/resolveJsonAsConst.ts"() {
    resolveJsonAsConst = (config, jsonFilePath, fao, languageServiceHost, ts) => {
      for (const matcher of config.jsonAsConst) {
        if (minimatch.minimatch(jsonFilePath, matcher)) {
    ...
    getScriptSnapshotDecorator = (solcCache) => createHostDecorator(({ languageServiceHost }, ts, logger, config, fao) => {
      return {
        getScriptSnapshot: (filePath) => {
          ...
          if (!isSolidity(filePath) || !fs.existsSync(filePath) || fs.existsSync(`${filePath}.d.ts`) || fs.existsSync(`${filePath}.ts`)) {
            return languageServiceHost.getScriptSnapshot(filePath);
          }
          try {
            const plugin = baseBundler.bundler(config, logger, fao, solc__namespace, solcCache);
            const resolveBytecode = filePath.endsWith(".s.sol");
            const projectRoot = ((_a = languageServiceHost.getCurrentDirectory) == null ? void 0 : _a.call(languageServiceHost)) ?? process.cwd();
...
var getMatchingRemapping, resolveModuleNameLiteralsDecorator;
var init_resolveModuleNameLiterals = __esm({
  "src/decorators/resolveModuleNameLiterals.ts"() {
    init_factories();
    init_utils();
    init_invariant();
    getMatchingRemapping = (remappings, moduleName) => {
      return Object.entries(remappings).filter(([from]) => moduleName.startsWith(from)).sort(([a], [b]) => b.length - a.length)[0];
    };
          ...
          return moduleNames.map(({ text: moduleName }, index) => {
            ...
            try {
              const resolvedModule = solidityModuleResolver(remappedName, ts, createInfo, containingFile, {
                isRemapped: remapping !== void 0,
                projectRoot: (_b2 = (_a2 = createInfo.project).getCurrentDirectory) == null ? void 0 : _b2.call(_a2)
              });
              if (resolvedModule) {
...
var hashText, getScriptSnapshotText, getScriptVersion, getVirtualMtimeMs, createFileAccessObject, createRealFileAccessObject;
var init_fileAccessObject = __esm({
  "src/factories/fileAccessObject.ts"() {
    ...
    getScriptSnapshotText = (lsHost, fileName, cwd) => {
      var _a;
      const candidates = [fileName, path__default.default.resolve(cwd, fileName)];
      for (const candidate of candidates) {
        try {
          const snapshot = (_a = lsHost.getScriptSnapshot) == null ? void 0 : _a.call(lsHost, candidate);
          if (snapshot) {
    ...
    getScriptVersion = (lsHost, fileName, cwd) => {
      var _a;
      const candidates = [fileName, path__default.default.resolve(cwd, fileName)];
      for (const candidate of candidates) {
        try {
          const version = (_a = lsHost.getScriptVersion) == null ? void 0 : _a.call(lsHost, candidate);
          if (version !== void 0) {
            return version;
          }
        } catch (_e) {
    ...
    createRealFileAccessObject = (lsHost, cwd = process.cwd()) => {
      if (!lsHost) {
        return {
      ...
      return {
        ...
        statSync: ((fileName) => {
          ...
          if (virtualMtimeMs === void 0) {
            return fs.statSync(filePath);
          }
          try {
            return { ...fs.statSync(filePath), mtimeMs: virtualMtimeMs };
          } catch (_e) {
            return { mtimeMs: virtualMtimeMs };
          }
        }),
        stat: (async (fileName) => {
          const filePath = fileName.toString();
          ...
          try {
            return { ...await promises.stat(filePath), mtimeMs: virtualMtimeMs };
          } catch (_e) {
            return { mtimeMs: virtualMtimeMs };
          }
        }),
      ...
      return {
        create: (createInfo) => {
          ...
          const config$1 = Effect.runSync(
            config.loadConfig(createInfo.project.getCurrentDirectory()).pipe(
              Effect.catchTag(
          ...
          const cache = bundlerCache.createCache(
            config$1.cacheDir,
