  ...
  "src/utils/solidityModuleResolver.ts"() {
    ...
    solidityModuleResolver = (moduleName, ts, createInfo, containingFile, options = {}) => {
      ...
      if (isRelativeSolidity(moduleName)) {
        const basedir = options.isRemapped ? options.projectRoot ?? ((_b = (_a = createInfo.project).getCurrentDirectory) == null ? void 0 : _b.call(_a)) ?? path.dirname(containingFile) : path.dirname(containingFile);
        return {
      ...
      if (isSolidity(moduleName)) {
        const isPathLikeRemap = options.isRemapped && (path.isAbsolute(moduleName) || moduleName.startsWith("node_modules/") || moduleName.startsWith("lib/") || moduleName.startsWith("src/") || moduleName.startsWith("contracts/"));
        return {
          extension: ts.Extension.Dts,
          isExternalLibraryImport: false,
          resolvedFileName: isPathLikeRemap ? path.resolve(
            options.projectRoot ?? ((_d = (_c = createInfo.project).getCurrentDirectory) == null ? void 0 : _d.call(_c)) ?? path.dirname(containingFile),
            moduleName
    ...
    createRealFileAccessObject = (lsHost, cwd = process.cwd()) => {
      ...
      return {
        ...
        statSync: ((fileName) => {
          ...
          try {
            return { ...statSync(filePath), mtimeMs: virtualMtimeMs };
          } catch (_e) {
        ...
        stat: (async (fileName) => {
          ...
          try {
            return { ...await stat(filePath), mtimeMs: virtualMtimeMs };
          } catch (_e) {
            return { mtimeMs: virtualMtimeMs };
          }
        }),
