...
type SolcAst = SourceUnit;
type SolcLanguage = 'Solidity' | 'Yul' | 'SolidityAST';
type SolcInputSource<T extends SolcLanguage = SolcLanguage> = {
    keccak256?: HexNumber;
} & ({
...
type SolcEvmVersion = 'homestead' | 'tangerineWhistle' | 'spuriousDragon' | 'byzantium' | 'constantinople' | 'petersburg' | 'istanbul' | 'berlin' | 'london' | 'paris' | 'shanghai' | 'cancun' | 'prague' | 'osaka';
type SolcSettings = {
...
type SolcInputSources<T extends SolcLanguage = SolcLanguage> = {
    [globalName: string]: SolcInputSource<T> & {
        destructible?: SolcInputSourcesDestructibleSettings;
    };
};
type SolcInputDescription<T extends SolcLanguage = SolcLanguage> = {
    language: T;
