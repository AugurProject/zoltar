...
var _solc__default = /*#__PURE__*/_interopDefault(_solc);
...
var solcCompile = (solc, input) => {
  const output = solc.compile(JSON.stringify(input));
  return typeof output === "string" ? JSON.parse(output) : output;
};
var createSolc = async (release) => {
  const s = await new Promise(
    (resolve, reject) => _solc__default.default.loadRemoteVersion(releases[release], (e, s2) => {
      if (e) {
        reject(e);
      } else {
        resolve(s2);
