...
var getContractPath = (basePath) => {
  const require2 = module$1.createRequire(basePath.endsWith("/") ? basePath : `${basePath}/`);
...
var resolveCacheKey = (modulePath, basedir) => {
  if (modulePath.startsWith(".") || path.isAbsolute(modulePath)) {
    return formatPath(path.resolve(basedir, modulePath));
  }
  return modulePath;
};
...
var resolveModuleAsync = async (logger, config, fao, solc, modulePath, basedir, includeAst, includeBytecode, moduleType, cache, contractPackage) => {
  ...
  try {
    const { solcInput, solcOutput, asts, artifacts, modules } = cachedResult ?? await compiler.resolveArtifacts(cacheModulePath, basedir, logger, config, includeAst, includeBytecode, fao, solc);
    let code = "";
    const artifactsExist = artifacts && Object.keys(artifacts).length > 0;
    if (artifactsExist) {
      code = await Effect.runPromise(runtime.generateRuntime(artifacts, moduleType, includeBytecode, contractPackage));
    } else {
...
var resolveModuleSync = (logger, config, fao, solc, modulePath, basedir, includeAst, includeBytecode, moduleType, cache, contractPackage) => {
  ...
  try {
    const { solcInput, solcOutput, asts, artifacts, modules } = cachedResult ?? compiler.resolveArtifactsSync(cacheModulePath, basedir, logger, config, includeAst, includeBytecode, fao, solc);
    let code = "";
    const artifactsExist = artifacts && Object.keys(artifacts).length > 0;
    if (artifactsExist) {
      code = Effect.runSync(runtime.generateRuntime(artifacts, moduleType, includeBytecode, contractPackage));
    } else {
      const message = `there were no artifacts for ${modulePath}. This is likely a bug in tevm`;
      code = `// ${message}`;
      logger.warn(message);
