...
import { releases, createSolc, solcCompile } from '@tevm/solc';
import { readFile } from 'fs/promises';
import { resolve } from 'path';
import { PrettyFormatter, ASTWriter, DefaultASTWriterMapping, ASTReader, FunctionVisibility, StateVariableVisibility, extractSpecifiersFromSource, getCompilerVersionsBySpecifiers } from 'solc-typed-ast';
...
// src/compiler/new/compiler/internal/errors.js
var VersionResolutionError = class extends Error {
  /**
...
var SolcError = class extends Error {
  /**
...
var CompilerOutputError = class extends Error {
  /**
...
var AstParseError = class extends Error {
  /**
...
var ShadowValidationError = class extends Error {
  /**
...
var compileContracts = (solc, sources, options, logger) => {
  const settings = Object.assign(
    ...
    options.viaIR !== void 0 && { viaIR: options.viaIR },
  ...
  const solcInput = {
    ...
    sources: Object.fromEntries(
      Object.entries(sources).map(([sourcePath, sourceCode]) => [
        sourcePath,
        {
          ...options.language === "SolidityAST" ? { ast: (
            /** @type {import('@tevm/solc').SolcAst} */
            sourceCode
          ) } : { content: (
            /** @type {string} */
            sourceCode
  ...
  Object.keys(sources).forEach((sourcePath) => {
    ...
    const solcAstOutput = (_a = solcOutput.sources) == null ? void 0 : _a[sourcePath];
    const solcContractOutput = (_b = solcOutput.contracts) == null ? void 0 : _b[sourcePath];
    ...
    result.compilationResult[
      /** @type {keyof typeof result.compilationResult} */
      sourcePath
    ] = output;
  });
...
var latestSolcVersion = (
  /** @type {keyof import('@tevm/solc').Releases | undefined} */
  Object.keys(releases)[0]
);
...
var getSolc = async (version, logger) => {
  try {
...
// src/compiler/new/compiler/internal/validateFiles.js
var validateFiles = (filePaths, language, logger) => {
...
var readSourceFiles = async (filePaths, language, logger) => {
...
var extractCompatibleSolcVersions = (sourceOrSpecifiers, logger) => {
  const specifiers = typeof sourceOrSpecifiers === "string" ? extractSpecifiersFromSource(sourceOrSpecifiers) : sourceOrSpecifiers;
  if (specifiers.length === 0) {
    logger.warn("Could not extract Solidity version from pragma statements in the source code");
  }
  const versions = Object.keys(releases);
  const compatibleVersions = (
    /** @type {(keyof import('@tevm/solc').Releases)[]} */
...
// src/compiler/new/compiler/internal/validateSolcVersion.js
var validateSolcVersion = (source, options, logger) => {
  ...
  if (options.language === "SolidityAST") {
    const astSources = (
      /** @type {import('../AstInput.js').AstInput[]} */
      Array.isArray(source) ? source : [source]
    );
    const versionLiterals = astSources.flatMap(
      (astSource) => "vPragmaDirectives" in astSource ? astSource.vPragmaDirectives.map((p) => p.literals) : astSource.nodes.map((n) => n.nodeType === "PragmaDirective" ? n.literals : [])
    );
...
// src/compiler/new/compiler/internal/validateBaseOptions.js
var validateBaseOptions = (source, options = {}, logger) => {
  const language = (
    /** @type {TLanguage} */
  ...
  if (language === "SolidityAST") {
    const isSourceUnit = (entry) => {
      if (typeof entry !== "object" || entry === null) {
        return false;
      }
      const astEntry = (
        /** @type {{nodeType?: string, name?: string}} */
        entry
      );
  ...
  const solcVersion = validateSolcVersion(
    /** @type {ValidatedSource<TLanguage>} */
    source,
    options,
    logger
  );
...
// src/compiler/new/compiler/compileFiles.js
var compileFiles = async (filePaths, options) => {
  const logger = createLogger({ name: "@tevm/compiler", level: (options == null ? void 0 : options.loggingLevel) ?? defaults.loggingLevel });
  const sources = await readSourceFiles(filePaths, options == null ? void 0 : options.language, logger);
  ...
  return (
    /** @type {import('./CompileFilesResult.js').CompileFilesResult<TCompilationOutput, TFilePaths>} */
    compileContracts(
      solc,
...
var extractContractsFromAstNodes = (sourceUnits, options) => {
  const logger = createLogger({ name: "@tevm/compiler", level: (options == null ? void 0 : options.loggingLevel) ?? defaults.loggingLevel });
...
var instrumentAst = (astNode, options, logger) => {
  const filteredContracts = options.contractFilter ? astNode.vContracts.filter((contract) => contract.name === options.contractFilter) : astNode.vContracts;
  filteredContracts.forEach((contract) => {
    {
      contract.vFunctions.forEach((node) => {
        if (options.exposeInternalFunctions && // We do only want to expose internal/private functions, and not blindly turn them public as it would change the behavior of external functions
...
// src/compiler/new/compiler/internal/validateShadowOptions.js
var validateShadowOptions = (astSourceNodes, options, sourceLanguage, logger) => {
  var _a;
  ...
  const { injectIntoContractPath, injectIntoContractName } = resolveContractLocation(
    ...
    astSourceNodes.length === 1 && !options.injectIntoContractPath ? (_a = astSourceNodes[0]) == null ? void 0 : _a.absolutePath : options.injectIntoContractPath,
...
var resolveContractLocation = (astSources, injectIntoContractPath, injectIntoContractName, logger) => {
  const failToResolve = (code, message, meta = {}) => {
    const err = new ShadowValidationError(message, { meta: { code: (
      /** @type {any} */
      code
    ), ...meta } });
    logger.error(err.message);
  ...
  if (!injectIntoContractPath && injectIntoContractName) {
    const matchingFiles = astSources.filter((ast) => ast.vContracts.some((c) => c.name === injectIntoContractName)).map((ast) => ast.absolutePath);
...
var solcSourcesToAstNodes = (sources, logger) => {
...
// src/compiler/new/compiler/compileSourcesWithShadow.js
var compileSourcesWithShadow = async (sources, shadow, options) => {
  const logger = createLogger({ name: "@tevm/compiler", level: (options == null ? void 0 : options.loggingLevel) ?? defaults.loggingLevel });
  const { sourceLanguage, shadowLanguage, injectIntoContractPath, injectIntoContractName, ...baseOptions } = options ?? {};
  const validatedOptions = validateBaseOptions(
    Object.values(sources),
...
var compileSourcesWithShadowInternal = (solc, sources, shadow, validatedOptions, shadowOptions, logger) => {
  var _a, _b;
  let astSources = validatedOptions.language === "SolidityAST" ? (
    /** @type {Record<string, import('@tevm/solc').SolcAst>} */
    sources
  ) : void 0;
  if (!astSources) {
  ...
  if (validatedShadowOptions.shadowMergeStrategy === "safe") {
    const { sources: soliditySources2 } = validatedOptions.language === "SolidityAST" ? extractContractsFromAstNodes(astSourceNodes, validatedOptions) : { sources: (
      /** @type {Record<string, string>} */
      sources
    ) };
    const targetSourceKey = validatedOptions.language === "SolidityAST" ? validatedShadowOptions.injectIntoContractPath : ((_b = Object.entries(astSources).find(
      ([_, ast]) => {
        var _a2;
        return ((_a2 = astSourceNodes.find((n) => n.id === ast.id)) == null ? void 0 : _a2.absolutePath) === validatedShadowOptions.injectIntoContractPath;
      }
    )) == null ? void 0 : _b[0]) ?? validatedShadowOptions.injectIntoContractPath;
    const targetSourceCode = (
      /** @type {string} */
      soliditySources2[targetSourceKey]
    );
    const lastChildNodeEnd2 = lastChildNode.sourceInfo.offset + lastChildNode.sourceInfo.length;
    const shadowedSolidityTarget2 = `${targetSourceCode.slice(0, lastChildNodeEnd2)}

${shadow}

${targetSourceCode.slice(lastChildNodeEnd2)}`;
    logger.debug(`Compiling sources with shadow code injected in safe mode`);
    return compileContracts(
      solc,
      { ...soliditySources2, [targetSourceKey]: shadowedSolidityTarget2 },
  ...
  const instrumentedSolidityTarget = (
    /** @type {string} */
...
// src/compiler/new/compiler/compileFilesWithShadow.js
var compileFilesWithShadow = async (filePaths, shadow, options) => {
  const logger = createLogger({ name: "@tevm/compiler", level: (options == null ? void 0 : options.loggingLevel) ?? defaults.loggingLevel });
  const { sourceLanguage, shadowLanguage, injectIntoContractPath, injectIntoContractName, ...baseOptions } = options ?? {};
...
var compileSource = async (source, options) => {
  const logger = createLogger({ name: "@tevm/compiler", level: (options == null ? void 0 : options.loggingLevel) ?? defaults.loggingLevel });
...
var compileSourceInternal = (solc, source, validatedOptions, logger) => {
...
var compileSources = async (sources, options) => {
  const logger = createLogger({ name: "@tevm/compiler", level: (options == null ? void 0 : options.loggingLevel) ?? defaults.loggingLevel });
...
var compileSourceWithShadow = async (source, shadow, options) => {
  const logger = createLogger({ name: "@tevm/compiler", level: (options == null ? void 0 : options.loggingLevel) ?? defaults.loggingLevel });
  const { sourceLanguage, shadowLanguage, injectIntoContractPath, injectIntoContractName, ...baseOptions } = options ?? {};
...
var compileSourceWithShadowInternal = (solc, source, shadow, validatedOptions, shadowOptions, logger) => {
...
var extractContractsFromSolcOutput = (solcOutput, options) => {
  const logger = createLogger({ name: "@tevm/compiler", level: (options == null ? void 0 : options.loggingLevel) ?? defaults.loggingLevel });
...
// src/compiler/new/compiler/internal/mergeOptions.js
var mergeOptions = (options = {}, overrides = {}) => {
  ...
  return Object.assign(
    ...
    (overrides.optimizer || options.optimizer) && {
      optimizer: Object.assign(
        ...
        options.optimizer,
        overrides.optimizer,
        (((_a = overrides.optimizer) == null ? void 0 : _a.details) || ((_b = options.optimizer) == null ? void 0 : _b.details)) && {
          details: Object.assign(
            {},
            (_c = options.optimizer) == null ? void 0 : _c.details,
            (_d = overrides.optimizer) == null ? void 0 : _d.details,
            (((_f = (_e = overrides.optimizer) == null ? void 0 : _e.details) == null ? void 0 : _f.yulDetails) || ((_h = (_g = options.optimizer) == null ? void 0 : _g.details) == null ? void 0 : _h.yulDetails)) && {
              yulDetails: Object.assign(
                {},
                (_j = (_i = options.optimizer) == null ? void 0 : _i.details) == null ? void 0 : _j.yulDetails,
                (_l = (_k = overrides.optimizer) == null ? void 0 : _k.details) == null ? void 0 : _l.yulDetails
              )
            }
          )
        }
    ...
    (overrides.viaIR !== void 0 || options.viaIR !== void 0) && {
...
var readSourceFilesSync = (filePaths, language, logger) => {
  const validatedPaths = validateFiles(filePaths, language, logger);
  logger.debug(`Preparing to read ${validatedPaths.length} files`);
  const effectiveLanguage = language ?? (() => {
    const first = validatedPaths[0] ?? "";
...
var createCompiler = (options) => {
  var _a;
  const logger = (options == null ? void 0 : options.logger) ?? createLogger({ name: "@tevm/compiler", level: (options == null ? void 0 : options.loggingLevel) ?? defaults.loggingLevel });
  let _solcInstance = options == null ? void 0 : options.solc;
  let _solcVersion = (_a = options == null ? void 0 : options.solc) == null ? void 0 : _a.semver;
  ...
  return {
    ...
    compileSourceWithShadow: async (source, shadow, compileOptions) => {
      requireSolcLoaded();
      const { sourceLanguage, shadowLanguage, injectIntoContractPath, injectIntoContractName, ...baseOptions } = compileOptions ?? {};
      const validatedOptions = validateBaseOptions(
        source,
    ...
    compileFiles: async (files, compileOptions) => {
      ...
      const sources = await readSourceFiles(files, mergedOptions == null ? void 0 : mergedOptions.language, logger);
    ...
    compileFilesSync: (files, compileOptions) => {
      requireSolcLoaded();
      const mergedOptions = mergeOptions(options, compileOptions);
      const sources = readSourceFilesSync(files, mergedOptions == null ? void 0 : mergedOptions.language, logger);
      const validatedOptions = validateBaseOptions(Object.values(sources), mergedOptions, logger);
      const solc = requireSolcLoadedSync(validatedOptions.solcVersion);
      return (
        /** @type {any} */
...
var compileContract = async (filePath, basedir, config, includeAst, includeBytecode, fao, logger, solc) => {
  ...
  if (includeAst) {
    ...
    return {
      artifacts: (_c = output.contracts) == null ? void 0 : _c[entryModule.id],
      modules: (
        /** @type {any} */
        modules
      ),
  ...
  return {
    artifacts: (_d = output.contracts) == null ? void 0 : _d[entryModule.id],
    modules: (
      /** @type {any} */
      modules
    ),
...
function compileContractSync(filePath, basedir, config, includeAst, includeBytecode, fao, logger, solc) {
  ...
  if (includeAst) {
    ...
    return {
      artifacts: (_c = solcOutput.contracts) == null ? void 0 : _c[entryModule.id],
      modules,
      asts,
      solcInput,
      solcOutput
  ...
  return {
    artifacts: (_d = solcOutput.contracts) == null ? void 0 : _d[entryModule.id],
    modules,
    asts: void 0,
    solcInput,
    solcOutput
...
export { compileFiles, compileFilesWithShadow, compileSource, compileSourceWithShadow, compileSources, compileSourcesWithShadow, createCompiler, extractContractsFromAstNodes, extractContractsFromSolcOutput, resolveArtifacts, resolveArtifactsSync, solcSourcesToAstNodes };
