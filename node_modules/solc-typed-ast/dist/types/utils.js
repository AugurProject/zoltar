...
function specializeType(type, loc) {
    ...
    if (type instanceof ast_2.MappingType) {
        ...
        const concreteValueT = specializeType(type.valueType, ast_1.DataLocation.Storage); // @todo update when maps supported in transient
        // Maps always live in storage
        return new ast_2.PointerType(new ast_2.MappingType(concreteKeyT, concreteValueT), ast_1.DataLocation.Storage); // @todo update when maps supported in transient
    }
...
function castable(fromT, toT, compilerVersion) {
    ...
    if (fromT instanceof ast_2.PointerType &&
        ...
        toT.location === ast_1.DataLocation.Storage && // @todo (dimo) Update this when arrays are supported in transient
