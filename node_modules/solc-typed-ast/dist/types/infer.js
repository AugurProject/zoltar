...
exports.builtinTypes = {
    ...
    require: (arg) => {
        let argTs;
        if (arg.parent instanceof ast_1.FunctionCall && arg.parent.vArguments.length === 2) {
            const secondArg = arg.parent.vArguments[1];
            if (secondArg.typeString === "error") {
                argTs = [reserved_1.types.bool, reserved_1.types.error];
            }
            else {
                argTs = [reserved_1.types.bool, reserved_1.types.stringMemory];
            }
        }
        else {
            argTs = [reserved_1.types.bool];
        }
...
class InferType {
    ...
    typeOfFunctionCall(node) {
        ...
        else if (resolvedCalleeT instanceof ast_2.EventType) {
            rets = [];
        }
        else if (resolvedCalleeT instanceof ast_2.ErrorType) {
            rets = [reserved_1.types.error];
        }
        else {
    ...
    typeOfIndexRangeAccess(node) {
        ...
        if (!(baseT instanceof ast_2.PointerType &&
            (baseT.to instanceof ast_2.BytesType ||
                baseT.to instanceof ast_2.StringType ||
                baseT.to instanceof ast_2.ArrayType) &&
                    ...
                    if (node.parent instanceof ast_1.FunctionCall) {
                        ...
                        for (const argT of argTs) {
                            if (!((argT instanceof ast_2.PointerType && (0, misc_1.eq)(argT.to, baseT.type)) ||
                                argT instanceof ast_2.FixedBytesType)) {
                                throw new misc_2.SolTypeError(`Unexpected argument type ${argT.pp()} to builtin function bytes.concat in ${(0, misc_1.pp)(node.parent)}.`);
                            }
                        }
                    }
    ...
    inferVariableDeclLocation(decl) {
        if (decl.stateVariable) {
            return decl.constant
                ? constants_1.DataLocation.Memory
                : decl.storageLocation === constants_1.DataLocation.Default
                    ? constants_1.DataLocation.Storage
    ...
    typeNameToTypeNode(node) {
        ...
        if (node instanceof ast_1.UserDefinedTypeName) {
            ...
            throw new Error(`NYI converting user-defined AST type ${def != undefined ? def.print() : undefined} to TypeNode`);
        }
    ...
    toABIEncodedType(type, encoderVersion, normalizePointers = false) {
        ...
        if (type instanceof ast_2.ArrayType) {
            ...
            if (type.size !== undefined) {
                return new ast_2.TupleType((0, misc_1.repeat)(elT, Number(type.size)));
            }
            return new ast_2.ArrayType(elT, type.size);
        }
        if (type instanceof ast_2.PointerType) {
            const toT = this.toABIEncodedType(type.to, encoderVersion, normalizePointers);
            if (toT instanceof ast_2.TupleType) {
                return toT;
            }
            if (toT instanceof ast_2.PackedArrayType || toT instanceof ast_2.ArrayType) {
                return new ast_2.PointerType(toT, normalizePointers ? constants_1.DataLocation.Memory : type.location);
            }
            (0, misc_1.assert)(false, `Unexpected ABI pointer type {0}`, toT);
        }
        if (type instanceof ast_2.UserDefinedType) {
