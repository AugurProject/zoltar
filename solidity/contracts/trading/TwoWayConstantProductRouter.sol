...
contract TwoWayConstantProductRouter is IERC1155Receiver {
	...
	function exitPosition(ITwoWayConstantProductPair pair, BinaryOutcomes.BinaryOutcome longOutcome, uint256 completeSetSharesToRedeem, uint256 maxLongSharesIn, uint256 minEthOut, address payable recipient, uint256 deadline) external nonReentrant beforeDeadline(deadline) returns (ExitResult memory result) {
		_validatePair(pair);
		bool longYes = _isDirectionalOutcome(longOutcome);
		require(recipient != address(0) && recipient != address(this), 'Invalid recipient');
		...
		result = ExitResult(completeSetSharesToRedeem, longSwapInput, totalLong, completeSetSharesToRedeem, ethOut, feeAmount);
	}

	function redeemCompleteSet(ISecurityPool pool, uint256 completeSetSharesToRedeem, uint256 minEthOut, address payable recipient, uint256 deadline) external nonReentrant beforeDeadline(deadline) returns (uint256 ethOut) {
	...
	function removeLiquidity(ITwoWayConstantProductPair pair, uint256 liquidity, uint256 minYesOut, uint256 minNoOut, address recipient, uint256 deadline) external nonReentrant beforeDeadline(deadline) returns (uint256 yesOut, uint256 noOut) {
		_validatePair(pair);
		require(recipient != address(0) && recipient != address(this), 'Invalid recipient');
	...
	function onERC1155BatchReceived(address, address, uint256[] calldata ids, uint256[] calldata, bytes calldata) external view returns (bytes4) {
	...
	function _pullExitShares(ISecurityPool pool, address owner, bool longYes, uint256 invalidAmount, uint256 longAmount) private {
