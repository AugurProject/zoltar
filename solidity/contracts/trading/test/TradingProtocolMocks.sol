...
contract TradingMockShareToken {
	...
	function safeTransferFrom(address from, address to, uint256 id, uint256 value, bytes calldata data) external {
		...
		if (to.code.length > 0)
			require(IERC1155Receiver(to).onERC1155Received(msg.sender, from, id, value, data) == IERC1155Receiver.onERC1155Received.selector, 'Receiver rejected');
	}
	...
	function safeBatchTransferFrom(address from, address to, uint256[] calldata ids, uint256[] calldata values, bytes calldata data) external {
		...
		if (to.code.length > 0)
			require(IERC1155Receiver(to).onERC1155BatchReceived(msg.sender, from, ids, values, data) == IERC1155Receiver.onERC1155BatchReceived.selector, 'Receiver rejected');
	}
	...
	function mintCompleteSets(uint248 universeId, address account, uint256 amount) external {
		...
		if (account.code.length > 0)
			require(IERC1155Receiver(account).onERC1155BatchReceived(msg.sender, address(0), ids, values, '') == IERC1155Receiver.onERC1155BatchReceived.selector, 'Receiver rejected');
	}
	...
	function mint(address account, uint256 id, uint256 amount) external {
		...
		if (account.code.length > 0)
			require(IERC1155Receiver(account).onERC1155Received(msg.sender, address(0), id, amount, '') == IERC1155Receiver.onERC1155Received.selector, 'Receiver rejected');
	}
...
contract TradingMockSecurityPool {
	...
	constructor(TradingMockShareToken token, TradingMockCoreFactory coreFactory, TradingMockZoltar mockZoltar, TradingMockQuestionData mockQuestionData, TradingMockForker mockForker, uint248 mockUniverseId, uint256 mockQuestionId, uint256 initialSharesPerEth) {
		shareToken = token;
