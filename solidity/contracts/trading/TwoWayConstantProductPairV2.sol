...
contract TwoWayConstantProductPairV2 is IERC1155Receiver {
	enum TradingStatus {
		Open,
		QuestionEnded,
		PoolInactive,
	...
	bytes32 private constant NAME_HASH = keccak256('Zoltar Two-Way LP');
	...
	event Swap(address indexed sender, address indexed recipient, bool yesForNo, bool exactOutput, uint256 amountIn, uint256 amountOut, uint256 feeAmount, uint256 resultingYesReserve, uint256 resultingNoReserve);
	...
	function DOMAIN_SEPARATOR() public view returns (bytes32) {
		return AuthorizationSignatures.domainSeparator(NAME_HASH, VERSION_HASH, address(this));
	}

	function permit(address owner, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external {
		require(block.timestamp <= deadline, 'Permit expired');
		require(owner != address(0), 'Permit owner is zero');
		uint256 nonce = nonces[owner]++;
		bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH, owner, spender, value, nonce, deadline));
		address signer = AuthorizationSignatures.recover(AuthorizationSignatures.hashTypedData(DOMAIN_SEPARATOR(), structHash), v, r, s);
	...
	function addLiquidity(uint256 maxYes, uint256 maxNo, uint256 minLiquidity, address recipient) external nonReentrant returns (uint256 yesUsed, uint256 noUsed, uint256 liquidity) {
	...
	function swapExactInput(bool yesForNo, uint256 amountIn, uint256 minAmountOut, address recipient) external nonReentrant returns (uint256 amountOut, uint256 feeAmount) {
		_requireLifecycleOpen(true);
		require(recipient != address(0), 'Recipient is zero');
		_synchronize();
	...
	function swapExactOutput(bool yesForNo, uint256 amountOut, uint256 maxAmountIn, address recipient) external nonReentrant returns (uint256 amountIn, uint256 feeAmount) {
		_requireLifecycleOpen(true);
		require(recipient != address(0), 'Recipient is zero');
		_synchronize();
		(uint256 reserveIn, uint256 reserveOut) = yesForNo ? (yesReserve, noReserve) : (noReserve, yesReserve);
	...
	function onERC1155BatchReceived(address operator, address, uint256[] calldata ids, uint256[] calldata, bytes calldata) external view returns (bytes4) {
	...
	function _requireLifecycleOpen(bool requireInitialized) private view {
		...
		require(ISecurityPoolForker(securityPool.securityPoolForker()).getQuestionOutcome(securityPool) == BinaryOutcomes.BinaryOutcome.None, 'Question resolved');
	}
