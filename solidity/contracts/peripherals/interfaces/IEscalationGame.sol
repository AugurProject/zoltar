...
interface IEscalationGameEvents {
	...
	event DepositOnOutcome(address indexed depositor, BinaryOutcomes.BinaryOutcome indexed outcome, uint256 attoRepAmount, uint256 depositIndex, uint256 cumulativeRepAmountAttoRep, uint256 resultingVaultDisputeStakedAttoRep, uint256 resultingTotalDisputeStakedAttoRep);
	/// @notice One replayable carry leaf. `nodeId` is the stable source node identity; REP values use attoREP.
	event LocalDepositAppended(uint256 indexed nodeId, BinaryOutcomes.BinaryOutcome indexed outcome, address indexed depositor, uint256 attoRepAmount, uint256 parentDepositIndex, uint256 cumulativeRepAmountAttoRep);
