...
contract ShareToken is ForkedERC1155, IShareToken {
	...
	function mintCompleteSets(uint248 _universeId, address _account, uint256 _cashAmount) external payable {
		...
		for (uint8 i = 0; i < Constants.NUM_OUTCOMES; i++) {
			_tokenIds[i] = TokenId.getTokenId(_universeId, Outcomes.Outcome(i));
	...
	function totalSupplyForOutcome(uint248 _universeId, Outcomes.Outcome _outcome) public view returns (uint256) {
		uint256 _tokenId = getTokenId(_universeId, _outcome);
		return totalSupply(_tokenId);
	}

	function balanceOfOutcome(uint248 _universeId, Outcomes.Outcome _outcome, address _account) public view returns (uint256) {
		uint256 _tokenId = getTokenId(_universeId, _outcome);
	...
	function balanceOfShares(uint248 _universeId, address _account) public view returns (uint256[3] memory balances) {
		balances[0] = balanceOf(_account, getTokenId(_universeId, Outcomes.Outcome.Invalid));
		balances[1] = balanceOf(_account, getTokenId(_universeId, Outcomes.Outcome.Yes));
		balances[2] = balanceOf(_account, getTokenId(_universeId, Outcomes.Outcome.No));
	}

	function getTokenId(uint248 _universeId, Outcomes.Outcome _outcome) public pure returns (uint256 _tokenId) {
		return TokenId.getTokenId(_universeId, _outcome);
	}

	function getTokenIds(uint248 _universeId, Outcomes.Outcome[] memory _outcomes) public pure returns (uint256[] memory _tokenIds) {
		return TokenId.getTokenIds(_universeId, _outcomes);
	}

	function unpackTokenId(uint256 _tokenId) public override pure returns (uint248 _universe, Outcomes.Outcome _outcome) {
