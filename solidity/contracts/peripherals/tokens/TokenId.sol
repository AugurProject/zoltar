...
library TokenId {
	...
	function getTokenId(uint248 _universeId, Outcomes.Outcome _outcome) internal pure returns (uint256 _tokenId) {
		assembly {
			_tokenId := or(shl(8, and(_universeId, 0x00FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF)), and(_outcome, 0xFF))
		}
	}

	function getTokenIds(uint248 _universeId, Outcomes.Outcome[] memory _outcomes) internal pure returns (uint256[] memory _tokenIds) {
	...
	function unpackTokenId(uint256 _tokenId) internal pure returns (uint248 _universe, Outcomes.Outcome _outcome) {
		assembly {
