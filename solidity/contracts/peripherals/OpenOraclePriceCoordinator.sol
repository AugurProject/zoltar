...
import { IOperationBountyResultReceiver } from './interfaces/IOperationBountyResultReceiver.sol';
...
contract OpenOraclePriceCoordinator {
	...
	string private constant INVALID_ORACLE_CONFIGURATION = 'Invalid oracle configuration';
	string private constant INVALID_COORDINATOR_SETUP = 'Invalid coordinator setup';
	string private constant INVALID_ORACLE_REQUEST = 'Invalid oracle request';
	string private constant ORACLE_VALUE_EXCEEDS_LIMIT = 'Oracle value exceeds limit';
	string private constant ORACLE_TOKEN_FUNDING_FAILED = 'Oracle token funding failed';
	string private constant INVALID_STAGED_OPERATION = 'Invalid staged operation';
	string private constant INVALID_STAGED_ROUTE = 'Invalid staged route';
	string private constant ETH_REFUND_FAILED = 'ETH refund failed';
	...
	address private bountyOperationCreator;
	address private bountyReportSponsor;
	uint256 private bountyOperationId;
	...
	mapping(uint256 => uint256) private operationBountyIds;
	...
	constructor(OpenOracle _openOracle, ReputationToken _reputationToken, IWeth9 _weth, uint256 _gasConsumedOpenOracleReportPrice, uint32 _gasConsumedSettlement, uint256 _gasUnitsForOneDispute, uint256 _initialReportPriorityFeeAttoEthPerGas, uint256 _targetPriceErrorForDispute, uint256 _openOracleSecurityMultiplierBps, uint48 _settlementTime, uint24 _disputeDelay, uint24 _protocolFee, uint24 _feePercentage, uint16 _multiplier, bool _timeType, bool _trackDisputes, address _protocolFeeRecipient, uint256 _escalationHaltMultiplierBps, uint256 _maxSettlementBaseFeeMultiplierBps, uint256 _minLiquidationPriceDistanceBps) {
		...
		require(_gasUnitsForOneDispute > 0, INVALID_ORACLE_CONFIGURATION);
		require(_initialReportPriorityFeeAttoEthPerGas > 0, INVALID_ORACLE_CONFIGURATION);
		require(_targetPriceErrorForDispute <= OPEN_ORACLE_PERCENTAGE_PRECISION, INVALID_ORACLE_CONFIGURATION);
		require(_openOracleSecurityMultiplierBps >= SecurityPoolUtils.BPS_DENOMINATOR, INVALID_ORACLE_CONFIGURATION);
		require(uint256(_protocolFee) + uint256(_feePercentage) < _targetPriceErrorForDispute, INVALID_ORACLE_CONFIGURATION);
		require(_escalationHaltMultiplierBps > 0, INVALID_ORACLE_CONFIGURATION);
		require(_openOracleSecurityMultiplierBps <= type(uint256).max / (OPEN_ORACLE_PERCENTAGE_PRECISION + _targetPriceErrorForDispute), INVALID_ORACLE_CONFIGURATION);
		...
		require(_initialReportPriorityFeeAttoEthPerGas <= maximumInitialReportPriorityFeeAttoEthPerGas, INVALID_ORACLE_CONFIGURATION);
		...
		require(_maxSettlementBaseFeeMultiplierBps >= SecurityPoolUtils.BPS_DENOMINATOR, INVALID_ORACLE_CONFIGURATION);
	...
	function setLiquidationApprovalRegistry(LiquidationApprovalRegistry registry) external {
		require(msg.sender == coordinatorFactory && address(liquidationApprovalRegistry) == address(0) && address(registry) != address(0), INVALID_COORDINATOR_SETUP);
		liquidationApprovalRegistry = registry;
	}

	function setOperationBountyBoard(address board) external {
		require(msg.sender == coordinatorFactory && operationBountyBoard == address(0) && board.code.length > 0, INVALID_COORDINATOR_SETUP);
		operationBountyBoard = board;
		emit OperationBountyBoardSet(board);
	}

	function setSecurityPool(ISecurityPool _securityPool) public {
		require(address(securityPool) == address(0x0), INVALID_COORDINATOR_SETUP);
	...
	function getSettlementCallbackGasLimit() public view returns (uint32) {
		...
		require(callbackGasLimit <= type(uint32).max, ORACLE_VALUE_EXCEEDS_LIMIT);
	...
	function requestPrice(uint256 proposedRepPerEthPrice, uint256 requestedInitialAttoWeth) public payable {
		...
		require(msg.value >= costAttoEth, INVALID_ORACLE_REQUEST);
		require(!isPriceValid(), INVALID_ORACLE_REQUEST);
		...
		if (excess > 0) {
			...
			require(sent, ETH_REFUND_FAILED);
		}
	}
	...
	function _requestPrice(address sponsor, uint256 costAttoEth, uint256 proposedRepPerEthPrice, uint256 requestedInitialAttoWeth) private {
		require(pendingReportId == 0, INVALID_ORACLE_REQUEST);
		require(proposedRepPerEthPrice > 0, INVALID_ORACLE_REQUEST);
		...
		require(initialWethReportAttoEth <= type(uint128).max, ORACLE_VALUE_EXCEEDS_LIMIT);
		require(initialRepReportAttoRep <= type(uint128).max, ORACLE_VALUE_EXCEEDS_LIMIT);
		require(escalationHaltAttoEth <= type(uint128).max, ORACLE_VALUE_EXCEEDS_LIMIT);
		require(settlerRewardAttoEth <= type(uint96).max, ORACLE_VALUE_EXCEEDS_LIMIT);
		...
		require(weth.transferFrom(sponsor, address(this), initialWethReportAttoEth), ORACLE_TOKEN_FUNDING_FAILED);
		require(reputationToken.transferFrom(sponsor, address(this), initialRepReportAttoRep), ORACLE_TOKEN_FUNDING_FAILED);
		require(weth.approve(address(openOracle), initialWethReportAttoEth), ORACLE_TOKEN_FUNDING_FAILED);
		require(reputationToken.approve(address(openOracle), initialRepReportAttoRep), ORACLE_TOKEN_FUNDING_FAILED);
	...
	function requestPriceIfNeededAndStageOperation(OperationType operation, address targetVault, uint256 operationAmountAttoRepOrAttoEth, uint256 validForSeconds, uint256 proposedRepPerEthPrice, uint256 requestedInitialAttoWeth) public payable {
		address actor = msg.sender == address(this) ? bountyOperationCreator : msg.sender;
		_requestPriceIfNeededAndStageOperation(operation, targetVault, actor, bytes32(0), operationAmountAttoRepOrAttoEth, validForSeconds, proposedRepPerEthPrice, requestedInitialAttoWeth);
	}
	...
	function _requestPriceIfNeededAndStageOperation(OperationType operation, address targetVault, address receiverVault, bytes32 approvalId, uint256 operationAmountAttoRepOrAttoEth, uint256 validForSeconds, uint256 proposedRepPerEthPrice, uint256 requestedInitialAttoWeth) private {
		address operator = msg.sender == address(this) ? bountyOperationCreator : msg.sender;
		address reportSponsor = msg.sender == address(this) ? bountyReportSponsor : msg.sender;
		require(operationAmountAttoRepOrAttoEth > 0, INVALID_STAGED_OPERATION);
		require(validForSeconds > 0, INVALID_STAGED_OPERATION);
		require(validForSeconds <= MAX_OPERATION_VALID_FOR_SECONDS, INVALID_STAGED_OPERATION);
		if (operation != OperationType.Liquidation) {
			require(targetVault == operator, INVALID_STAGED_ROUTE);
			require(receiverVault == operator && approvalId == bytes32(0), INVALID_STAGED_ROUTE);
		} else {
			require(receiverVault != targetVault, INVALID_STAGED_ROUTE);
		}
		require(!securityPool.isEscalationResolved(), INVALID_STAGED_OPERATION);
		if (pendingReportId != 0) {
			require(reportSponsor == pendingReportSponsor, 'Only the pending report sponsor can queue more operations until settlement');
		}
		if (operation == OperationType.WithdrawRep) {
			(, uint256 withdrawRepAmountAttoRep) = _previewWithdrawRep(operator, operationAmountAttoRepOrAttoEth);
			require(withdrawRepAmountAttoRep > 0, INVALID_STAGED_OPERATION);
		}
		...
		if (operation == OperationType.Liquidation && receiverVault != operator) {
			reservedLiquidationDebtAttoEth = liquidationApprovalRegistry.reserve(operationId, approvalId, receiverVault, targetVault, operator, operationAmountAttoRepOrAttoEth, snapshotTargetOpenInterestAttoEth, block.timestamp + uint256(settlementTime) + validForSeconds);
		} else if (operation == OperationType.Liquidation) {
			require(approvalId == bytes32(0), INVALID_STAGED_ROUTE);
		}
		...
		if (msg.sender == address(this)) operationBountyIds[operationId] = bountyOperationId;
		_trackActiveStagedOperation(operationId);
		if (operation == OperationType.Liquidation) {
			emit LiquidationRouteStaged(operationId, operator, receiverVault, targetVault, approvalId, operationAmountAttoRepOrAttoEth, reservedLiquidationDebtAttoEth);
		}
		...
		if (isPriceValid()) {
			...
			if (shouldRequestPrice && isPendingSettlementOperationId) {
				...
				_requestPrice(reportSponsor, costAttoEth, proposedRepPerEthPrice, requestedInitialAttoWeth);
			}
		}
		...
		if (refund > 0) {
			(bool sent, ) = payable(reportSponsor).call{value: refund}('');
			require(sent, ETH_REFUND_FAILED);
		}
	}

	function stageAndRequestOperationBounty(uint256 bountyId, address sponsor, address creator, OperationType operation, address targetVault, uint256 amount, uint256 validForSeconds, uint256 proposedRepPerEthPrice, uint256 requestedInitialAttoWeth) external payable returns (uint256 operationId, uint256 reportId) {
		require(msg.sender == operationBountyBoard && bountyOperationCreator == address(0), 'Invalid bounty caller');
		require(isPriceValid() || pendingSettlementOperationIds.length < MAX_PENDING_SETTLEMENT_OPERATIONS, 'Operation bounty queue full');
		operationId = stagedOperationCounter + 1;
		bountyOperationCreator = creator;
		bountyReportSponsor = sponsor;
		bountyOperationId = bountyId;
		this.requestPriceIfNeededAndStageOperation{value: msg.value}(operation, targetVault, amount, validForSeconds, proposedRepPerEthPrice, requestedInitialAttoWeth);
		bountyOperationCreator = address(0);
		bountyReportSponsor = address(0);
		bountyOperationId = 0;
		return (operationId, pendingReportId);
	}
	...
	function _emitExecutedStagedOperation(uint256 operationId, OperationType operation, bool success, string memory errorMessage) private {
		uint256 bountyId = operationBountyIds[operationId];
		if (bountyId != 0) {
			delete operationBountyIds[operationId];
			IOperationBountyResultReceiver(operationBountyBoard).recordOperationResult(bountyId, success);
		}
