...
{
    if (nodeId == 0) {
        return (false, lastValidTick, accEth, 0);
    }

    Node storage node = nodes[nodeId];

    uint256 rightEth =
        node.right != 0 ? nodes[node.right].subtreeEth : 0;

    // --------------------------------------------------
    // 1️⃣ If higher prices alone might clear, recurse right
    // --------------------------------------------------
    if (rightEth > 0) {
        (
            bool found,
        ...
        if (found) {
            return (found, tickOut, ethOut, ethAtTickOut);
        }
    ...
    // --------------------------------------------------
    // 2️⃣ Retroactive REP check (must return previous tick)
    // --------------------------------------------------
    if (accEth > 0) {
        uint256 repIfRepriced =
            accEth * price / PRICE_PRECISION;

        if (repIfRepriced > maxRepBeingSold) {
            return (
                true,
                lastValidTick,
                lastValidEth,
    ...
    if (ethToTake > remainingCap)
        ethToTake = remainingCap;

    uint256 newAccEth = accEth + ethToTake;

    uint256 totalRep =
        newAccEth * price / PRICE_PRECISION;

    // --------------------------------------------------
    // 4️⃣ REP cap triggered at this tick (partial fill)
    // --------------------------------------------------
    if (totalRep >= maxRepBeingSold) {
        uint256 maxEthAtThisPrice =
            maxRepBeingSold * PRICE_PRECISION / price;

        uint256 ethUsedAtTick = 0;

        if (maxEthAtThisPrice > accEth)
            ethUsedAtTick =
                maxEthAtThisPrice - accEth;

        if (ethUsedAtTick > ethToTake)
            ethUsedAtTick = ethToTake;

        return (
            true,
            node.tick,
            accEth + ethUsedAtTick,
            ethUsedAtTick
        );
    }

    // --------------------------------------------------
    // 5️⃣ Fully valid tick
    // --------------------------------------------------
    accEth = newAccEth;
