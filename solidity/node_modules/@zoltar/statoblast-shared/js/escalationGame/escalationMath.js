...
export function getEscalationBindingCapitalAttoRep(balancesAttoRep) {
    const [invalidBalanceAttoRep, yesBalanceAttoRep, noBalanceAttoRep] = balancesAttoRep;
    if ((invalidBalanceAttoRep >= yesBalanceAttoRep && invalidBalanceAttoRep <= noBalanceAttoRep) || (invalidBalanceAttoRep >= noBalanceAttoRep && invalidBalanceAttoRep <= yesBalanceAttoRep))
        return invalidBalanceAttoRep;
    if ((yesBalanceAttoRep >= invalidBalanceAttoRep && yesBalanceAttoRep <= noBalanceAttoRep) || (yesBalanceAttoRep >= noBalanceAttoRep && yesBalanceAttoRep <= invalidBalanceAttoRep))
        return yesBalanceAttoRep;
    return noBalanceAttoRep;
}
function computeAtanhScaled(z) {
    const z2 = (z * z) / SCALE;
    let term = z;
...
function computeLnRatioScaled(lowValue, highValue) {
    ...
    if (z === 0n)
        return log2Count * LN2_SCALED;
    return log2Count * LN2_SCALED + 2n * computeAtanhScaled(z);
}
export function computeIterativeAttritionCostAttoRep(startBondAttoRep, nonDecisionThresholdAttoRep, lnRatioScaled, timeSinceStart) {
    if (timeSinceStart <= 0n)
        return startBondAttoRep;
    if (timeSinceStart >= ESCALATION_TIME_LENGTH)
        return nonDecisionThresholdAttoRep;
...
export function computeEscalationBindingCapitalAttoRep(startBondAttoRep, nonDecisionThresholdAttoRep, timeSinceStart) {
    if (startBondAttoRep <= 0n || nonDecisionThresholdAttoRep <= startBondAttoRep)
        throw new Error('Escalation threshold must exceed the start bond');
...
export function computeEscalationTimeSinceStartFromAttritionCostAttoRep(startBondAttoRep, nonDecisionThresholdAttoRep, attritionCostAttoRep) {
    if (attritionCostAttoRep <= startBondAttoRep)
        return 0n;
    if (attritionCostAttoRep >= nonDecisionThresholdAttoRep)
        return ESCALATION_TIME_LENGTH;
...
function getEscalationOutcomeIndex(outcome) {
    switch (outcome) {
        case 'invalid':
            return 0;
        case 'yes':
...
function getMaxEscalationBalanceAttoRep(balancesAttoRep) {
    const [invalidBalanceAttoRep, yesBalanceAttoRep, noBalanceAttoRep] = balancesAttoRep;
    if (invalidBalanceAttoRep > yesBalanceAttoRep) {
        if (invalidBalanceAttoRep > noBalanceAttoRep)
            return invalidBalanceAttoRep;
        return noBalanceAttoRep;
    }
    if (yesBalanceAttoRep > noBalanceAttoRep)
        return yesBalanceAttoRep;
    return noBalanceAttoRep;
}
export function hasReachedNonDecision(balancesAttoRep, nonDecisionThresholdAttoRep) {
    let thresholdHits = 0;
    if (balancesAttoRep[0] >= nonDecisionThresholdAttoRep)
        thresholdHits += 1;
    if (balancesAttoRep[1] >= nonDecisionThresholdAttoRep)
        thresholdHits += 1;
    if (balancesAttoRep[2] >= nonDecisionThresholdAttoRep)
        thresholdHits += 1;
    return thresholdHits >= 2;
}
function setBalanceAtIndex(balancesAttoRep, index, valueAttoRep) {
    switch (index) {
        case 0:
...
export function projectEscalationDeposit({ amountAttoRep, balancesAttoRep, nonDecisionThresholdAttoRep, outcome, startBondAttoRep, }) {
    const outcomeIndex = getEscalationOutcomeIndex(outcome);
    const currentBalanceAttoRep = balancesAttoRep[outcomeIndex];
    if (currentBalanceAttoRep >= nonDecisionThresholdAttoRep)
        return undefined;
    ...
    const otherHasMax = (() => {
        if (outcomeIndex === 0)
            return balancesAttoRep[1] === maxBalanceAttoRep || balancesAttoRep[2] === maxBalanceAttoRep;
        if (outcomeIndex === 1)
            return balancesAttoRep[0] === maxBalanceAttoRep || balancesAttoRep[2] === maxBalanceAttoRep;
    ...
    if (acceptedAmountAttoRep < startBondAttoRep && newBalanceAttoRep !== nonDecisionThresholdAttoRep)
        return undefined;
...
function getWinningWithdrawalAmount({ bindingCapitalAttoRep, depositAmountAttoRep, depositEndAttoRep, depositStartAttoRep, forkThresholdAttoRep, nonDecisionThresholdAttoRep, winningOutcomeBalanceAttoRep, }) {
    ...
    if (forkThresholdAttoRep < nonDecisionThresholdAttoRep)
        return (amountToWithdrawAttoRep * forkThresholdAttoRep) / nonDecisionThresholdAttoRep;
    return amountToWithdrawAttoRep;
}
export function getWinningEscalationDepositClaimAmount({ bindingCapitalAttoRep, depositAmountAttoRep, cumulativeAmountAttoRep, forkThresholdAttoRep, nonDecisionThresholdAttoRep, winningOutcomeBalanceAttoRep, }) {
...
export function getWinningImportedEscalationDepositClaimAmount({ bindingCapitalAttoRep, depositAmountAttoRep, postDepositCumulativeAmountAttoRep, forkThresholdAttoRep, nonDecisionThresholdAttoRep, winningOutcomeBalanceAttoRep, }) {
    return getWinningWithdrawalAmount({
        bindingCapitalAttoRep,
        depositAmountAttoRep,
