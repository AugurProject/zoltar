import { bigintToSafeNumber, bytesToHex, encodeAbiParameters, getAddress, hexToBytes, keccak256 } from '@zoltar/core-shared/evm/ethereum';
...
function readPackedUint(bytes, offset, width) {
    if (!Number.isInteger(offset) || !Number.isInteger(width) || offset < 0 || width < 1 || offset + width > bytes.length)
        throw new Error('OpenOracle packed event data is truncated');
    let value = 0n;
...
function readPackedAddress(bytes, offset) {
    return getAddress(bytesToHex(bytes.slice(offset, offset + 20)));
}
function writePackedUint(bytes, offset, width, value) {
    if (value < 0n || value >= 1n << (BigInt(width) * 8n))
        throw new Error(`OpenOracle packed value does not fit in ${width.toString()} bytes`);
    let remaining = value;
...
function writePackedAddress(bytes, offset, value) {
    bytes.set(hexToBytes(value), offset);
}
export function encodeOpenOracleStatePreimagePacked(preimage) {
...
export function decodeOpenOracleStatePreimage(data, reportId) {
    const bytes = hexToBytes(data);
    if (bytes.length !== 235)
        throw new Error(`OpenOracle packed event data must be 235 bytes, received ${bytes.length.toString()}`);
    return {
        game: {
...
export function getOpenOracleGameTuple(game) {
    return [
        game.currentAmount1,
        game.currentAmount2,
...
export function getOpenOracleHelperTuple(helper) {
    return [helper.reportId, helper.creator, helper.blockTimestamp, helper.blockNumber];
}
export function hashOpenOracleStatePreimage(preimage) {
...
export function getOpenOracleReportIdFromTopic(topic) {
    return BigInt(topic);
}
export function hasOpenOracleFlag(game, flag) {
