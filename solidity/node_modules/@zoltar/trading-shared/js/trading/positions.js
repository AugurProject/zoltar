import { BPS_DENOMINATOR, quoteExactInput, quoteExactOutput } from './math.js';
export function quoteEnterPosition(longOutcome, completeSetShares, yesReserve, noReserve, feeBps) {
...
export function quoteExitPosition(longOutcome, completeSetShares, yesReserve, noReserve, feeBps) {
...
export function maximumInsuredExit(parameters) {
    const oppositeReserve = parameters.longOutcome === 'YES' ? parameters.noReserve : parameters.yesReserve;
    if (parameters.yesReserve <= 0n || parameters.noReserve <= 0n)
        return 0n;
    let low = 0n;
...
export function minimumAfterSlippage(amount, slippageBps) {
    if (slippageBps < 0n || slippageBps >= BPS_DENOMINATOR)
        throw new Error('slippageBps is out of range');
    return (amount * (BPS_DENOMINATOR - slippageBps)) / BPS_DENOMINATOR;
}
export function maximumAfterSlippage(amount, slippageBps) {
    if (slippageBps < 0n || slippageBps >= BPS_DENOMINATOR)
        throw new Error('slippageBps is out of range');
    return (amount * (BPS_DENOMINATOR + slippageBps) + BPS_DENOMINATOR - 1n) / BPS_DENOMINATOR;
}
export function inputOutcomeConditionalPriceImpact(inputOutcome, beforeInputReserve, beforeOutputReserve, quote) {
