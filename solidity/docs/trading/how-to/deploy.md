...
The root mainnet and Sepolia manifests describe planned deterministic addresses; they do not prove that the contracts are live. Continue only after the selected network has reviewed code at those addresses against the pinned runtime hashes. See the root [contract architecture](../../../../docs/explanation/contract-architecture.html) for the discovery and deployment boundaries.
