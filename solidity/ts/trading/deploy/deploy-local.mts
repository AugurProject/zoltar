...
import { encodeDeployData, keccak256, type Address, type Hex } from '@zoltar/core-shared/evm/ethereum'
...
const deploymentDirectory = path.join(projectRoot, 'deployments', 'trading')
await fs.mkdir(deploymentDirectory, { recursive: true })
await fs.writeFile(path.join(deploymentDirectory, 'local.json'), `${JSON.stringify(manifest, undefined, 2)}\n`)
