...
export class EthereumClientService {
	...
	public readonly call = async (transaction: PartialEthereumTransaction, blockTag: EthereumBlockTag, requestAbortController: AbortController | undefined) => {
		const response = await this.requestHandler.jsonRpcRequest({ method: 'eth_call', params: [transaction, blockTag] }, requestAbortController)
