...
function hasJsonRpcBaseFields(value) {
...
function parseJsonRpcResponse(raw) {
...
export const getDefaultAnvilRpcUrl = () => (process.platform === 'win32' ? 'http://127.0.0.1:8545' : 'http://host.docker.internal:8545')
export const getMockedEthSimulateWindowEthereum = async rpcUrl => {
	...
	const validateLocalhostUrl = url => {
		try {
	...
	const addStateOverrides = async stateOverrides => {
	...
	// Time manipulation
	const manipulateTime = async blockTimeManipulation => {
	...
	const advanceTime = async amountInSeconds => {
	...
	const setNextBlockBaseFeePerGasToZero = async () => {
		await request({ method: 'anvil_setNextBlockBaseFeePerGas', params: ['0x0'] })
	}
	// Reset Anvil to a clean state before each test
...
//# sourceMappingURL=AnvilWindowEthereum.js.map
