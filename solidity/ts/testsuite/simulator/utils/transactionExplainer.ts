...
import { Deployment, printLogs } from './peripheralLogs.js'
...
export const createTransactionExplainer = (deployments: Deployment[]) => {
	return (request: SendTransactionParams, result: SimulatedTransaction) => {
		...
		if (contract === undefined) { console.log(`UNKNOWN CALL: ${ jsonStringify(request)} `)}
			...
			printDecodedFunction(contract.deploymentName, data === undefined ? '0x0' : dataStringWith0xStart(data), contract.abi)
		}
