...
import { writeContractAndWait } from './viem'
...
const mintETH = async (anvilWindowEthereum, mintAmounts) => {
...
const mintERC20 = async (anvilWindowEthereum, erc20Address, mintAmounts, balanceSlot = 2n) => {
...
export const approveToken = async (client, tokenAddress, spenderAddress) =>
...
export const getERC20Balance = async (client, tokenAddress, ownerAddress) =>
...
export const getETHBalance = async (client, address) => await client.getBalance({ address })
export const setupTestAccounts = async anvilWindowEthereum => {
...
export async function ensureProxyDeployerDeployed(client) {
...
export const contractExists = async (client, contract) => (await client.getCode({ address: contract })) !== undefined
const uint248BitMask = (1n << 248n) - 1n
export function getChildUniverseId(parentUniverseId, outcome) {
