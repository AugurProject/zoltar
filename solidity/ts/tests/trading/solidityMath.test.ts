...
import { quoteExactInput, quoteExactOutput } from '@zoltar/trading-shared/trading/math'
...
describe('Solidity and TypeScript AMM math parity', () => {
	...
	let harness: Address
	let artifact: TradingContracts['contracts/trading/test/TwoWayConstantProductMathHarness.sol']['TwoWayConstantProductMathHarness']

	beforeAll(async () => {
		const contracts = await compileArtifactsForTests()
		artifact = contracts['contracts/trading/test/TwoWayConstantProductMathHarness.sol'].TwoWayConstantProductMathHarness
