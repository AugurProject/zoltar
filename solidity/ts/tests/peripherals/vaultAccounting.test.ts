...
describe('Peripherals: vault accounting', () => {
	...
	test('oracle-staged collateral operations are rejected once escalation resolves', async () => {
		...
		await assert.rejects(requestPriceIfNeededAndStageOperation(client, securityPoolAddresses.priceOracleManagerAndOperatorQueuer, OperationType.WithdrawRep, client.account.address, 1n), /Invalid staged operation/)
	})
