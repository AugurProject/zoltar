...
describe('Solidity bytecode coverage helpers', () => {
	...
	test('attributes raw transaction deployment coverage using transaction input fetched by hash', async () => {
		...
		if (isCoverageEnabled()) {
			...
			const constructorAssignmentLine = await findLineNumberByExactSource('solidity/contracts/ReputationToken.sol', 'zoltar = _zoltar;')
			assert.ok((deploymentCoverage.lineHits[constructorAssignmentLine.toString()] ?? 0) > 0, 'raw deployment coverage should attribute the constructor assignment using input fetched by transaction hash')
		}
	})
