...
import { peripherals_SecurityPoolUtils_SecurityPoolUtils, test_peripherals_LiquidationApprovalTestMocks_CoarseLiquidationRoundingHarness } from '../types/contractArtifact'
...
describe('SecurityPoolUtils', () => {
	...
	test('REP-denominated ownership converts aggregate capacity at the live oracle price', async () => {
		...
		const capacityAtOneRepPerEth = await client.readContract({ abi: peripherals_SecurityPoolUtils_SecurityPoolUtils.abi, address: securityPoolUtilsAddress, functionName: 'calculateMintingCapacityAttoEth', args: [ownershipAttoRep, PRICE_PRECISION, 20_000n] })
		const capacityAfterRepPriceDoubles = await client.readContract({ abi: peripherals_SecurityPoolUtils_SecurityPoolUtils.abi, address: securityPoolUtilsAddress, functionName: 'calculateMintingCapacityAttoEth', args: [ownershipAttoRep, 2n * PRICE_PRECISION, 20_000n] })
	...
	test('open interest remains and is allocated by price-independent capacity ownership', async () => {
		const vaultOpenInterest = await client.readContract({ abi: peripherals_SecurityPoolUtils_SecurityPoolUtils.abi, address: securityPoolUtilsAddress, functionName: 'calculateVaultOpenInterestAttoEth', args: [101n, 1n, 3n] })
	...
	test('receiver health factor rounds both associated and free REP requirements upward', async () => {
		const requiredBoundary = await client.readContract({ abi: peripherals_SecurityPoolUtils_SecurityPoolUtils.abi, address: securityPoolUtilsAddress, functionName: 'isVaultHealthyAtFactor', args: [225n, 75n, 100n, PRICE_PRECISION, 20_000n, 15_000n] })
		const belowAssociatedBoundary = await client.readContract({ abi: peripherals_SecurityPoolUtils_SecurityPoolUtils.abi, address: securityPoolUtilsAddress, functionName: 'isVaultHealthyAtFactor', args: [225n, 74n, 100n, PRICE_PRECISION, 20_000n, 15_000n] })
		const belowFreeBoundary = await client.readContract({ abi: peripherals_SecurityPoolUtils_SecurityPoolUtils.abi, address: securityPoolUtilsAddress, functionName: 'isVaultHealthyAtFactor', args: [224n, 76n, 100n, PRICE_PRECISION, 20_000n, 15_000n] })
