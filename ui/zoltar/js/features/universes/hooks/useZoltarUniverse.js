...
function buildQuestionPageFromQuestions(questions, currentPage) {
...
function mergeQuestionLists(existingQuestions, nextQuestions) {
    const getQuestionKey = (question) => normalizeQuestionId(question.questionId) ?? question.questionId.toLowerCase();
    const questionsById = new Map(existingQuestions.map(question => [getQuestionKey(question), question]));
    for (const question of nextQuestions)
        questionsById.set(getQuestionKey(question), question);
    return [...questionsById.values()];
}
function includesQuestionId(questions, normalizedQuestionId) {
    return questions.some(question => normalizeQuestionId(question.questionId) === normalizedQuestionId);
}
const defaultUseZoltarUniverseDependencies = {
...
export function useZoltarUniverse({ accountAddress, activeUniverseId, autoLoadInitialData, deploymentStatuses, environmentRefreshKey, onTransactionFailed, onTransactionFinished, onTransactionPresented, onTransactionPrepared, onTransactionRequested, onTransactionSubmitted }, dependencies = defaultUseZoltarUniverseDependencies) {
    ...
    const zoltarUniverseLoadedId = useSignal(undefined);
    const zoltarUniverseResolvedId = useSignal(undefined);
    const hasLoadedZoltarQuestions = useSignal(false);
    const zoltarQuestionCount = useSignal(undefined);
    const zoltarQuestionPage = useSignal(undefined);
    const zoltarQuestions = useSignal([]);
    const zoltarUniverse = useSignal(undefined);
    const zoltarChildUniverseError = useSignal(undefined);
    const zoltarQuestionsError = useSignal(undefined);
    const zoltarQuestionLookupError = useSignal(undefined);
    const zoltarQuestionLookupId = useSignal(undefined);
    const zoltarUniverseError = useSignal(undefined);
    const zoltarChildUniverseFeedback = useSignal(undefined);
    const zoltarChildUniversePendingOutcomeIndex = useSignal(undefined);
    ...
    const isCurrentZoltarContext = (context) => {
    ...
    const isCurrentQuestionLoad = (generation, context) => {
        return questionLoadGenerationRef.current === generation && isCurrentZoltarContext(context);
    };
    const clearResolvedQuestionLookupError = (questions) => {
        const lookupId = zoltarQuestionLookupId.value;
        if (lookupId !== undefined && includesQuestionId(questions, lookupId))
            zoltarQuestionLookupError.value = undefined;
    };
    const ensureZoltarUniverse = async () => {
        if (zoltarUniverse.value !== undefined && zoltarUniverseLoadedId.value === activeUniverseId)
            return zoltarUniverse.value;
        const loadedUniverse = await loadZoltarUniverse();
        if (loadedUniverse !== undefined)
            return loadedUniverse;
        if (zoltarUniverseMissing.value)
            throw new Error('Zoltar universe does not exist');
        throw new Error('Failed to load current Zoltar universe');
    };
    const loadZoltarUniverse = async (options = {}) => {
        ...
        return await universeLoad.run({
            ...
            onSuccess: universe => {
                if (!isCurrentZoltarContext(universeLoadContext))
                    return;
    ...
    const loadZoltarQuestionCountData = async () => {
        ...
        await questionCountLoad.run({
            ...
            onSuccess: questionCount => {
                if (!isMounted.current)
                    return;
                if (!isCurrentQuestionLoad(questionLoadGeneration, questionLoadContext))
                    return;
    ...
    const loadQuestions = async () => {
        ...
        if (!isMounted.current || !isCountCurrent() || !isQuestionsCurrent() || !isCurrentQuestionLoad(questionLoadGeneration, questionLoadContext))
            return;
    ...
    const loadQuestionsPage = async (pageIndex, pageSize) => {
        if (!isMounted.current)
            return;
        if (!zoltarDeployed)
            return;
    ...
    const loadQuestionById = async (questionId) => {
        if (!isMounted.current || !zoltarDeployed)
            return;
        ...
        await questionByIdLoad.run({
            ...
            onSuccess: question => {
                if (!isMounted.current || !isCurrentZoltarContext(questionLoadContext) || zoltarQuestionLookupId.value !== normalizedQuestionId)
                    return;
                if (!question.exists) {
                    if (!includesQuestionId(zoltarQuestions.value, normalizedQuestionId))
    ...
    const createChildUniverse = async (outcomeIndex) => {
        ...
        try {
            ...
            try {
                ...
                if (!universe.hasForked)
                    throw new Error('This universe must fork before child universes can be deployed');
            ...
            try {
                ...
                if (result !== undefined)
                    onTransactionPresented(createChildUniverseWarningPresentation(result, message));
            }
        }
        finally {
            zoltarChildUniversePendingOutcomeIndex.value = undefined;
    ...
    useLayoutEffect(() => {
        if (!autoLoadInitialData)
            return;
        const initialLoads = [loadZoltarUniverse()];
        if (zoltarDeployed)
            initialLoads.push(loadZoltarQuestionCountData());
...
//# sourceMappingURL=useZoltarUniverse.js.map