...
function resolvePrepareMigrationAmount(amountAttoRep, preparedRepBalanceAttoRep, repBalanceAttoRep) {
    ...
    if (missingAmountAttoRep === 0n)
        throw new Error('Selected amount is already prepared');
    const currentRepBalanceAttoRep = repBalanceAttoRep ?? 0n;
    if (currentRepBalanceAttoRep < missingAmountAttoRep)
        throw new Error('Not enough REP in this universe to prepare the selected amount');
    return missingAmountAttoRep;
}
export function useZoltarMigration({ accountAddress, activeUniverseId, ensureZoltarUniverse, onTransactionFailed, onTransactionFinished, onTransactionPresented, onTransactionPrepared, onTransactionRequested, onTransactionSubmitted, refreshState, refreshZoltarForkAccess, refreshZoltarUniverse, zoltarForkRepBalanceAttoRep, zoltarMigrationPreparedRepBalanceAttoRep, }) {
    const zoltarMigrationError = useSignal(undefined);
    const zoltarMigrationPending = useSignal(false);
    const zoltarMigrationFeedback = useSignal(undefined);
    const zoltarMigrationResult = useSignal(undefined);
    const zoltarMigrationActiveAction = useSignal(undefined);
    const { state: zoltarMigrationForm, setState: setZoltarMigrationForm } = useFormState(getDefaultZoltarMigrationFormState());
    const resolveActionResultName = (actionName) => (actionName === 'prepare' ? 'addRepToMigrationBalance' : 'splitMigrationRep');
    const getPendingTitle = (actionName) => (actionName === 'prepare' ? 'Preparing REP for migration' : 'Migrating REP');
    const getSuccessTitle = (actionName) => (actionName === 'prepare' ? 'REP prepared for migration' : 'REP migrated');
    const getFailureTitle = (actionName) => (actionName === 'prepare' ? 'REP preparation failed' : 'REP migration failed');
    const runZoltarMigrationAction = useCallback(async ({ actionName, action, errorFallback, refreshAfter, requiresOutcomeIndexes, resolveAmount = amount => amount }) => {
        ...
        try {
            ...
            if (!universe.hasForked)
                throw new Error('Migration is unavailable because this universe has not forked.');
        ...
        try {
            ...
            if (latestResult !== undefined)
                onTransactionPresented(createZoltarMigrationWarningPresentation(latestResult, message));
        }
    }, [
        accountAddress,
...
//# sourceMappingURL=useZoltarMigration.js.map