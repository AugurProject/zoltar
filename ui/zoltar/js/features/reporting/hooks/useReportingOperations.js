...
import { getDefaultReportingFormState, getDefaultReportingWithdrawDepositIndexesByOutcome } from '../../../lib/formDefaults.js';
...
const defaultUseReportingOperationsDependencies = {
...
function getAvailableWithdrawDepositIndexes(details, outcome) {
...
function filterAvailableWithdrawDepositIndexes(selectedDepositIndexes, availableDepositIndexes) {
    return selectedDepositIndexes.filter(index => availableDepositIndexes.includes(index));
}
function sameSelectedWithdrawDepositIndexes(left, right) {
    return left.length === right.length && left.every((value, index) => value === right[index]);
}
function sameSelectedWithdrawDepositIndexesByOutcome(left, right) {
...
function pruneSelectedWithdrawDepositIndexesByOutcome(currentSelections, details) {
    if (details.status !== 'active')
        return getDefaultReportingWithdrawDepositIndexesByOutcome();
    return {
...
export function useReportingOperations({ accountAddress, onTransactionCanceled, onTransactionFailed, onTransactionFinished, onTransactionPresented, onTransactionPrepared, onTransactionRequested, onTransactionSubmitted, refreshState, selectedSecurityPoolAddress }, dependencies = defaultUseReportingOperationsDependencies) {
    const reportingLoad = useLoadController();
    const reportingDetails = useSignal(undefined);
    const reportingError = useSignal(undefined);
    const { state: reportingForm, setState: setReportingForm } = useFormState(getDefaultReportingFormState());
    const reportingActiveAction = useSignal(undefined);
    const reportingFeedback = useSignal(undefined);
    const reportingResult = useSignal(undefined);
    ...
    const getPendingTitle = (actionName) => (actionName === 'reportOutcome' ? 'Submitting report' : 'Settling escalation deposits');
    const getSuccessTitle = (actionName) => (actionName === 'reportOutcome' ? 'Report submitted' : 'Escalation deposits settled');
    const getFailureTitle = (actionName) => (actionName === 'reportOutcome' ? 'Report failed' : 'Settlement failed');
    const requireSelectedOutcome = (selectedOutcome) => {
        if (selectedOutcome !== undefined)
            return selectedOutcome;
        throw new Error('Select an outcome side before reporting on a question.');
    };
    const isReportingSelectionCurrent = (selectionKey) => currentReportingSelectionKeyRef.current === selectionKey;
    ...
    const runReportingAction = async (actionName, action, errorFallback, outcomeOverride) => {
        ...
        try {
            ...
            await runWriteAction({
                ...buildWriteActionConfig({ accountAddress, onTransactionCanceled, onTransactionFailed, onTransactionFinished, onTransactionPresented, onTransactionPrepared, onTransactionRequested, refreshState }, reportingError, 'Connect a wallet before reporting on a question', createReportingTransactionIntent(actionName, transactionContext)),
                onRefreshError: (message, hash) => {
                    ...
                    if (result !== undefined)
                        onTransactionPresented(createReportingWarningPresentation(result, message));
                },
                onWriteCanceled: () => {
                ...
                if (!isReportingSelectionCurrent(actionSelectionKey))
                    return;
                ...
                setReportingForm(current => {
                    ...
                    if (sameSelectedWithdrawDepositIndexesByOutcome(current.selectedWithdrawDepositIndexesByOutcome, selectedWithdrawDepositIndexesByOutcome))
                        return current;
                    return {
                        ...current,
    ...
    const reportOutcome = async () => await runReportingAction('reportOutcome', async (walletAddress, securityPoolAddress, currentForm, isCurrentSelection) => {
        ...
        if (!isCurrentSelection())
            return undefined;
        if (latestDetails.systemState !== 'operational')
            throw new Error('Reporting actions are unavailable until this pool is operational.');
        ...
        if (contributionPreview.actualDepositAmount === undefined)
            throw new Error(contributionPreview.reason ?? 'Unable to preview the REP backing that would become dispute-staked for this report.');
        if (!latestDetails.viewerVaultExists)
            throw new Error('Reporting moves pool-held REP backing from your security vault into dispute-staked REP. Deposit REP into your vault before reporting.');
        ...
        if (contributionPreview.actualDepositAmount > remainingSelectedOutcomeCapacity) {
            if (remainingSelectedOutcomeCapacity === 0n)
                throw new Error('No remaining contribution capacity is available on the selected side.');
        ...
        if (contributionPreview.actualDepositAmount > poolHeldVaultRepBackingAttoRep)
            throw new Error(`Insufficient pool-held vault REP backing. Deposit ${formatCurrencyBalance(contributionPreview.actualDepositAmount - poolHeldVaultRepBackingAttoRep)} more REP into your vault before reporting.`);
        if (!isCurrentSelection())
            return undefined;
    ...
    const withdrawEscalation = async (outcome, depositIndexesOverride) => await runReportingAction('withdrawEscalation', async (walletAddress, securityPoolAddress, currentForm, isCurrentSelection) => {
        const latestDetails = await dependencies.loadReportingDetails(securityPoolAddress, walletAddress);
        if (!isCurrentSelection())
            return undefined;
        ...
        if (latestDetails.settlementState === 'migration-required')
            throw new Error('Settle winning carried proofs in the child continuation after it finalizes; parent deposits do not need migration.');
        if (latestDetails.settlementState === 'migration-expired')
            throw new Error('Settle winning carried proofs in the finalized child; the optional unresolved parent escalation-deposit accounting cleanup window has closed.');
        if (!latestDetails.parentWithdrawalEnabled)
            throw new Error('Escalation deposits cannot be settled until the question is finalized.');
    ...
    }, 'Failed to settle escalation deposits', outcome);
    return {
...
//# sourceMappingURL=useReportingOperations.js.map