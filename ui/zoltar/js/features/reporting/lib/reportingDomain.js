import { computeEscalationTimeSinceStartFromAttritionCostAttoRep, getEscalationBindingCapitalAttoRep, getWinningEscalationDepositClaimAmount as computeWinningEscalationDepositClaimAmount, getWinningImportedEscalationDepositClaimAmount as computeWinningImportedEscalationDepositClaimAmount, projectEscalationDeposit, } from '@zoltar/shared/escalationMath';
...
function roundUpToRepUnit(value) {
    if (value <= 0n)
        return 0n;
    return ((value + REP_UNIT - 1n) / REP_UNIT) * REP_UNIT;
}
function getSelectedAndOtherSides(details, selectedOutcome) {
...
function computeHypotheticalBindingDuration(startBondAttoRep, nonDecisionThresholdAttoRep, bindingCapitalAttoRep) {
    if (bindingCapitalAttoRep <= 0n)
        return 0n;
...
export function projectEscalationEndTime(details, outcome, amount) {
    if (amount <= 0n)
        return undefined;
...
function getWinningEscalationDepositClaimAmount(details, outcome, deposit) {
    const winningOutcomeBalance = details.sides.find(side => side.key === outcome)?.balance;
    if (winningOutcomeBalance === undefined)
        return undefined;
...
function getWinningImportedEscalationDepositClaimAmount(details, outcome, deposit) {
    const winningOutcomeBalance = details.sides.find(side => side.key === outcome)?.balance;
    if (winningOutcomeBalance === undefined)
        return undefined;
...
export function getEscalationDepositClaimAmount(details, outcome, deposit) {
    if (details === undefined || details.status !== 'active' || !details.parentWithdrawalEnabled)
        return undefined;
    if (!isPoolQuestionFinalized(details))
        return undefined;
    if (details.questionOutcome !== outcome)
        return 0n;
    return getWinningEscalationDepositClaimAmount(details, outcome, deposit);
}
export function getImportedEscalationDepositClaimAmount(details, outcome, deposit) {
...
export function getRemainingSelectedOutcomeContributionCapacity(details, outcome) {
    if (details.status === 'not-started')
        return details.nonDecisionThresholdAttoRep;
...
export function getReportingTimerPreview(details, outcome, amount) {
    if (amount <= 0n)
        return undefined;
    ...
    if (details.status === 'not-started') {
        ...
        if (preview.actualDepositAmount === undefined)
            return undefined;
        return {
...
export function getLeadingEscalationOutcome(sides) {
    let leadingSide;
...
export function getMinimumOutcomeChangeContribution(details, selectedOutcome) {
    const { largestOtherBalance, selectedSide } = getSelectedAndOtherSides(details, selectedOutcome);
    if (selectedSide === undefined)
        return { amountAttoRep: undefined, reason: 'Selected side is unavailable.' };
    if ((isPoolQuestionFinalized(details) && details.questionOutcome === selectedOutcome) || isUniqueWinner(selectedSide.balance, largestOtherBalance))
        return { amountAttoRep: 0n, reason: undefined };
...
export function getReportingMinimumOutcomeChangeContribution(details, selectedOutcome) {
...
export function getMaxProfitContribution(details, selectedOutcome) {
    const minContribution = getMinimumOutcomeChangeContribution(details, selectedOutcome);
...
export function getReportingMaxProfitContribution(details, selectedOutcome) {
    if (details === undefined)
        return {
            amountAttoRep: undefined,
            reason: LOAD_REPORTING_PRESETS_REASON,
...
export function getSelectedOutcomeRewardWindowFillTimestamp(details, selectedOutcome, acceptedAmountAttoRep) {
    if (acceptedAmountAttoRep <= 0n)
        return undefined;
    const { largestOtherBalance, selectedSide } = getSelectedAndOtherSides(details, selectedOutcome);
    if (selectedSide === undefined)
        return undefined;
    ...
    const rewardEligibleCap = largestOtherBalance + largestOtherBalance / 2n;
    if (rewardEligibleCap <= 0n)
        return undefined;
    const targetFinalBalance = rewardEligibleCap < details.nonDecisionThresholdAttoRep ? rewardEligibleCap : details.nonDecisionThresholdAttoRep;
    if (projectedSelectedBalance >= targetFinalBalance)
        return undefined;
...
export function calculateEstimatedEscalationReturn(details, selectedOutcome, amount) {
    if (amount <= 0n)
        return {
            payout: 0n,
            profit: 0n,
...
function getEscalationSide(details, outcome) {
    return details.sides.find(side => side.key === outcome);
}
function previewEscalationContribution(details, outcome, amount) {
    if (isPoolQuestionFinalized(details))
        return {
...
export function previewReportingContribution(details, outcome, amount) {
    if (details.status === 'not-started') {
        if (amount < details.startBondAttoRep)
            return {
                actualDepositAmount: undefined,
...
//# sourceMappingURL=reportingDomain.js.map