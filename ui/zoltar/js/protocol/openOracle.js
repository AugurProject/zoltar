...
function normalizeOpenOracleTokenMetadata(tokenAddress, decimalsValue, symbolValue) {
    let decimals;
    if (typeof decimalsValue === 'bigint')
        decimals = bigintToSafeNumber(decimalsValue, 'Token decimals');
    if (typeof decimalsValue === 'number')
        decimals = decimalsValue;
    const symbol = String(symbolValue).trim();
    if (decimals === undefined || !Number.isInteger(decimals) || decimals < 0 || decimals > 255)
        throw new Error(`Token metadata for ${tokenAddress} returned invalid decimals`);
    if (symbol === '')
        throw new Error(`Token metadata for ${tokenAddress} returned an empty symbol`);
    if (sameAddress(tokenAddress, getWethAddress()) && (decimals !== 18 || symbol !== 'WETH'))
        throw new Error(`WETH metadata is invalid for ${tokenAddress}`);
    return { decimals, symbol };
}
function getStagedOracleExecutionResult(receipt, managerAddress, expectedOperation) {
    for (const log of receipt.logs) {
        if (!sameAddress(log.address, managerAddress))
            continue;
        try {
            ...
            if (decodedLog.eventName !== 'ExecutedStagedOperation')
                continue;
...
function getStagedOracleQueuedResult(receipt, managerAddress, expectedOperation) {
    for (const log of receipt.logs) {
        if (!sameAddress(log.address, managerAddress))
            continue;
        try {
            ...
            if (decodedLog.eventName !== 'StagedOperationQueued')
                continue;
            const operation = decodeOracleQueueOperation(BigInt(decodedLog.args.operation));
            if (operation !== expectedOperation)
                continue;
...
function requireBigintValue(value, context) {
...
export async function loadOracleManagerDetails(client, managerAddress, openOracleAddress) {
    ...
    if (activeStagedOperationCount > 0n) {
        ...
        if (!Array.isArray(activeStagedOperationsResponse) || activeStagedOperationsResponse.length !== 2)
            throw new Error('Unexpected active staged operations response');
        ...
        stagedOperations = operationIds
            ...
            if (stagedOperation === undefined)
                throw new Error('Missing staged operation details');
            return {
...
function compareStagedOperationIdsDescending(left, right) {
...
function calculateOpenOraclePrice(amount1, amount2) {
    return amount2 === 0n ? 0n : (amount1 * 10n ** OPEN_ORACLE_PRICE_UNITS) / amount2;
}
export async function loadOpenOracleReportDetails(client, openOracleAddress, reportId) {
    const [eventState, stateHash, block] = await Promise.all([
        loadOpenOracleEventState(client, openOracleAddress, reportId).catch(error => {
            if (error instanceof Error && error.message === `Oracle report #${reportId.toString()} does not exist`)
                throw createOpenOracleReportMissingError(reportId);
            throw error;
        }),
        client.readContract({
            abi: peripherals_openOracle_OpenOracle_OpenOracle.abi,
    ...
    if (!hasTimestampAndNumber(block))
    ...
    if (stateHash.toLowerCase() !== expectedStateHash.toLowerCase())
        throw new Error(`OpenOracle report #${reportId.toString()} event state does not match its on-chain state hash`);
...
export async function loadOpenOracleReportSummaries(client, pageIndex, pageSize) {
    ...
    if (uniqueTokenAddresses.length > 0) {
        const tokenDecimals = await readRequiredMulticall(client, uniqueTokenAddresses.map(tokenAddress => ({
            abi: ABIS.mainnet.erc20,
            functionName: 'decimals',
            address: tokenAddress,
            args: [],
        })));
        const tokenSymbols = await readRequiredMulticall(client, uniqueTokenAddresses.map(tokenAddress => ({
            abi: ABIS.mainnet.erc20,
            functionName: 'symbol',
            address: tokenAddress,
            args: [],
        ...
        for (const [index, tokenAddress] of uniqueTokenAddresses.entries()) {
            ...
            if (decimals === undefined || symbol === undefined)
                throw new Error('Unexpected token metadata response');
    ...
    const reports = reportIds.map(reportId => {
        const state = eventStates.get(reportId);
        if (state === undefined)
            throw new Error('Unexpected oracle report summary response');
        ...
        if (token1Metadata === undefined || token2Metadata === undefined)
            throw new Error('Unexpected oracle token metadata response');
        return {
            currentAmount1: game.currentAmount1,
            currentAmount2: game.currentAmount2,
            currentReporter: game.currentReporter,
...
export async function createOpenOracleReportInstance(client, parameters) {
    ...
    if (sameAddress(parameters.token1Address, getWethAddress()))
        wethFundingAmountAttoEth = parameters.exactToken1Report;
    else if (sameAddress(parameters.token2Address, getWethAddress()))
        wethFundingAmountAttoEth = parameters.initialToken2Amount;
    if (wethFundingAmountAttoEth > 0n) {
        ...
        if (wethBalanceAttoEth < wethFundingAmountAttoEth)
            await wrapWeth(client, wethFundingAmountAttoEth - wethBalanceAttoEth);
    }
...
export async function loadOracleManagerQueueOperationEthValue(client, managerAddress) {
    ...
    if (funding === undefined || funding.costAttoEth === 0n)
        return 0n;
    return funding.includeBuffer ? addOpenOracleBountyBuffer(funding.costAttoEth) : funding.costAttoEth;
}
async function getCoordinatorInitialReportPrice(client, managerAddress, requestedInitialAttoWeth = 0n) {
...
export async function loadCoordinatorInitialReportFundingRequirement(client, managerAddress, walletAddress, proposedRepPerEthPrice, requestedInitialAttoWeth = 0n) {
...
async function assertCoordinatorRequestPriceAllowed(client, managerAddress) {
...
async function fundCoordinatorInitialReport(client, managerAddress, proposedRepPerEthPrice, requestedInitialAttoWeth = 0n) {
    ...
    if (fundingRequirement.currentRepBalanceAttoRep < fundingRequirement.initialReportAmount2)
        throw new Error('Insufficient REP balance for coordinator initial report');
...
export async function requestOraclePrice(client, managerAddress, proposedRepPerEthPrice, requestedInitialAttoWeth = 0n, reviewedRequestValueAttoEth) {
...
export async function executeOracleManagerStagedOperation(client, managerAddress, operationId) {
...
export async function wrapWeth(client, amountAttoEth) {
...
export async function loadOpenOracleWithdrawableBalances(client, openOracleAddress, holder, token1, token2) {
    const loadBalance = async (token) => requireBigintValue(await client.readContract({
        address: openOracleAddress,
        abi: peripherals_openOracle_OpenOracle_OpenOracle.abi,
    ...
    const availableBalance = (balance) => (balance > 1n ? balance - 1n : 0n);
    return {
...
export async function withdrawOpenOracleBalance(client, openOracleAddress, token, amount, recipient) {
    const hash = await writeContractAndWait(client, () => ({
        address: openOracleAddress,
        abi: peripherals_openOracle_OpenOracle_OpenOracle.abi,
...
export async function settleOracleReport(client, openOracleAddress, reportId, preimage) {
    let resolvedPreimage = preimage;
    if (resolvedPreimage === undefined) {
        const { getBlock, getLogs } = client;
        if (getBlock === undefined || getLogs === undefined)
            throw new Error('OpenOracle settlement requires a client that can load report events');
...
export async function disputeOracleReport(client, openOracleAddress, reportId, tokenToSwap, newAmount1, newAmount2, _amt2Expected, stateHash) {
    ...
    if (currentStateHash.toLowerCase() !== stateHash.toLowerCase())
        throw new Error('This report changed on-chain while the dispute was being prepared. Retry to use the latest state.');
    const derivedTokenToSwap = getOpenOracleDisputeSwapToken(state.latest.game, newAmount1, newAmount2);
    if (derivedTokenToSwap.toLowerCase() !== tokenToSwap.toLowerCase())
        throw new Error('The dispute price direction does not match the selected swap token.');
    const hash = await writeContractAndWait(client, () => ({
        address: openOracleAddress,
        abi: peripherals_openOracle_OpenOracle_OpenOracle.abi,
...
export async function loadLiquidationApprovalRegistry(client, managerAddress) {
...
export async function loadLiquidationApproval(client, managerAddress, approvalId) {
...
export async function setLiquidationApproval(client, registryAddress, params) {
...
export async function permitLiquidationApproval(client, registryAddress, params, signature) {
    return await writeContractAndWait(client, () => ({
        address: registryAddress,
        abi: peripherals_LiquidationApprovalRegistry_LiquidationApprovalRegistry.abi,
...
export async function revokeLiquidationApproval(client, registryAddress, approvalId) {
    return await writeContractAndWait(client, () => ({
        address: registryAddress,
        abi: peripherals_LiquidationApprovalRegistry_LiquidationApprovalRegistry.abi,
...
export async function invalidateLiquidationApprovalNonce(client, registryAddress, newNonce) {
    return await writeContractAndWait(client, () => ({
        address: registryAddress,
        abi: peripherals_LiquidationApprovalRegistry_LiquidationApprovalRegistry.abi,
...
export async function queueSecurityPoolLiquidation(client, managerAddress, targetVault, amount, validForSeconds, requestedInitialAttoWeth = 0n, receiverVault = client.account.address, approvalId = `0x${'00'.repeat(32)}`) {
...
export async function queueOracleManagerOperation(client, managerAddress, operation, targetVault, amount, validForSeconds, proposedRepPerEthPrice, requestedInitialAttoWeth = 0n) {
    const queueOperationValueAttoEth = await loadOracleManagerQueueOperationEthValue(client, managerAddress);
