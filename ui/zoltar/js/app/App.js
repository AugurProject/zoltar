...
export function App() {
    ...
    const activeRoute = resolveEnumValue(route, 'not-found', ['deploy', 'zoltar', 'open-oracle', 'not-found']);
    ...
    const { approveZoltarForkRep, createChildUniverse, forkZoltar, hasLoadedZoltarQuestions, loadingZoltarForkAccess, loadingZoltarQuestionCount, loadingZoltarQuestion, loadingZoltarQuestions, loadingZoltarUniverse, loadZoltarQuestionPage, loadZoltarQuestion, loadZoltarQuestions, loadZoltarUniverse, migrateInternalRep, prepareRepForMigration, setZoltarForkQuestionId, setZoltarMigrationForm, zoltarChildUniverseError, zoltarChildUniversePendingOutcomeIndex, zoltarForkApproval, zoltarForkActiveAction, zoltarForkError, zoltarForkPending, zoltarForkQuestionId, zoltarForkRepBalanceAttoRep, zoltarMigrationChildRepBalancesAttoRep, zoltarMigrationActiveAction, zoltarMigrationError, zoltarMigrationForm, zoltarMigrationPending, zoltarMigrationPreparedRepBalanceAttoRep, zoltarQuestionCount, zoltarQuestionLookupError, zoltarQuestionLookupId, zoltarQuestionPage, zoltarQuestions, zoltarQuestionsError, zoltarUniverse, zoltarUniverseError, zoltarUniverseMissing, } = useZoltarOperations({ ...walletScopedHookConfig, activeUniverseId, activeZoltarView, autoLoadInitialData: walletBootstrapComplete && canReadOnchainData, deploymentStatuses, environmentRefreshKey: activeEnvironmentNonce });
    ...
    const openOracleRouteContentProps = {
        ...
        onOpenOracleFormChange: update => {
            ...
            if (update.reportId !== undefined)
                setOpenOracleReport(update.reportId);
        },
    ...
    if (route === 'zoltar') {
        routeSubNavigation = (_jsx(RouteSubNavigation, { ariaLabel: appCopy.zoltarViews, value: activeZoltarView, onChange: view => setZoltarView(view), options: [
