...
export const formatCheckingPoolRepMigratedToChildUniverse = (outcomeLabel) => `Checking whether pool-held REP has already been migrated for the ${outcomeLabel} child universe.`;
export const formatEthPerRepValue = (price) => `${price} ETH / REP`;
...
export const formatSelectPriceValueEthRepFromDepthChart = (price) => `Select price ${price} ETH / REP from depth chart`;
export const formatImportedForkDepositPageSummary = (startIndex, endIndex, totalCount, paginationSummary) => `Showing parent deposits ${startIndex}-${endIndex} of ${totalCount}. ${paginationSummary}`;
export const formatDepositSelectionRequired = (outcomeLabel) => `Select at least one ${outcomeLabel} fork-carried deposit to settle.`;
...
export const formatMissingOutcomePoolDetail = (outcomeLabel) => `Security Pool for ${outcomeLabel} universe does not exist.`;
...
export const formatFinalizedRefundOnlySettlementNotice = (capacityOwnershipLabel) => `Finalized refund-only settlement uses the child-pool settlement path to unlock ETH, and it does not assign REP backing units or ${capacityOwnershipLabel}.`;
export const formatWinningClaimCapacityOwnershipHeadline = (capacityOwnershipLabel) => `Winning claims add REP backing units and ${capacityOwnershipLabel}.`;
export const formatWinningClaimSettlementNotice = (capacityOwnershipLabel) => `Claiming a winning bid adds REP backing units and a pro-rata share of the ${capacityOwnershipLabel} to the bidder vault. That capacity ownership is the remaining capacity ownership being assigned during settlement. Refund-only bids just return locked ETH.`;
export const formatMixedSettlementPreviewDetail = (capacityOwnershipLabel) => `Winning rows receive estimated REP backing units plus estimated ${capacityOwnershipLabel}, while refund rows return locked ETH.`;
export const formatWinningSettlementPreviewDetail = (capacityOwnershipLabel) => `Winning rows receive estimated REP backing units plus estimated ${capacityOwnershipLabel}.`;
export const formatRefundSettlementPreviewDetail = (capacityOwnershipLabel) => `Refund-only settlement returns locked ETH and does not assign ${capacityOwnershipLabel}.`;
...
export const formatEstimatedValue = (label) => `Estimated ${label}`;
...
export const formatStartsInValue = (duration) => `Starts in ${duration}`;
export const pendingConfirmation = 'Pending confirmation';
export const settleSelectedBids = 'Settle selected bids';
export const formatWinningBidBatchSettlementDetail = (capacityOwnershipLabel) => `Select winning bids and settle them together. Winning claims add REP backing units plus ${capacityOwnershipLabel}.`;
export const formatFinalizedRefundBatchSettlementDetail = (capacityOwnershipLabel) => `Select finalized refund rows and settle them together. These rows return locked ETH without adding ${capacityOwnershipLabel}.`;
export const formatRefundableBidBatchSettlementDetail = (capacityOwnershipLabel) => `Select refundable bids and settle them together. Refund-only settlement returns locked ETH without adding ${capacityOwnershipLabel}.`;
export const formatMixedBidBatchSettlementDetail = (capacityOwnershipLabel) => `Select winning and refundable bids and settle them together. Winning selections add REP backing units plus ${capacityOwnershipLabel}, while refundable selections return locked ETH.`;
...
export const formatFinalizedSettlementDetail = (capacityOwnershipLabel) => `Bidding is closed and finalized settlement paths are now in effect. Winning claims receive REP backing units plus ${capacityOwnershipLabel}, while losing bids are refunded.`;
...
export const formatTruthAuctionStartDelay = (duration) => `Truth auction can be started in ${duration} once migration ends.`;
...
export const formatNoClaimableParentEscalationDeposits = (outcomeLabel) => `No ${outcomeLabel} parent escalation deposits are currently available for a direct claim by this wallet.`;
...
export const formatPoolRepAlreadyMigrated = (outcomeLabel) => `Pool-held REP has already been migrated to the ${outcomeLabel} universe.`;
export const formatPoolMigrationRequiredForVault = (outcomeLabel) => `Migrate pool to the ${outcomeLabel} universe before moving vault balances.`;
...
export const formatSettleSelectedValueForkCarriedDeposits = (outcomeLabel) => `Settle selected ${outcomeLabel} fork-carried deposits`;
...
export const formatEntitlementAlreadyMaterialized = (outcomeLabel) => `This wallet’s optional unresolved parent escalation-deposit accounting cleanup is already recorded for the ${outcomeLabel} child universe.`;
export const formatNoUnresolvedDeposits = (outcomeLabel) => `No ${outcomeLabel} unresolved deposits remain for this wallet.`;
export const formatMigrateUnresolvedEscalationToValue = (outcomeLabel) => `Clear unresolved parent escalation-deposit accounting for ${outcomeLabel}`;
...
export const formatClaimSelectedValueParentDeposits = (outcomeLabel) => `Claim selected ${outcomeLabel} deposits`;
...
export const formatMigratePoolToValueUniverse = (outcomeLabel) => `Migrate pool to ${outcomeLabel} universe`;
...
export const formatMigrateVaultToValue = (outcomeLabel) => `Migrate vault to ${outcomeLabel}`;
export const alreadyMigratedStatus = 'Already migrated';
export const truthAuctionStatus = 'Truth Auction Status';
export const formatStartTruthAuctionDetail = (capacityOwnershipLabel) => `Start the ETH-for-REP truth auction only after migration closes. Winning bids later claim REP backing units plus ${capacityOwnershipLabel}, while losing bids are refunded during settlement.`;
...
export const formatMissingChildUniverseDetail = (outcomeLabel) => `Child universe not created for the ${outcomeLabel} outcome.`;
...
export const formatLoadedBidCoverageSummary = (loadedBidCount, visibleBidCount) => `Showing ${loadedBidCount} of ${visibleBidCount} bids across visible levels`;
...
export const formatLoadingOutcomePoolDetail = (outcomeLabel) => `Loading the ${outcomeLabel} child pool…`;
...
export const formatEscalationDepositPageSummary = (startIndex, endIndex, totalCount, paginationSummary) => `Showing deposits ${startIndex}-${endIndex} of ${totalCount}. ${paginationSummary}`;
...
export const formatSubmissionsLabel = (count) => `${count} submissions`;
