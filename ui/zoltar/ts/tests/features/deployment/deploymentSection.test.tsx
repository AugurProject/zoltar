...
describe('DeploymentSection', () => {
	...
	test('shows waiting state while prerequisite step is missing', async () => {
		...
		const dependent = createDeploymentStep({ id: 'scalarOutcomes', deployed: false, dependencies: ['proxyDeployer'], label: 'ScalarOutcomes' })
		...
		expectTransactionButtonDisabled(document.body, 'Deploy ScalarOutcomes', 'Requires Proxy Deployer')
		const button = rendered.container.querySelector('button')
		const detailId = button?.getAttribute('aria-describedby')
		expect(detailId).toBe('deployment-scalarOutcomes-status-detail')
		...
		expect(rendered.container.textContent).toContain('ScalarOutcomes')
	})
