...
function assertTruthAuctionTickInRange(tick) {
    if (tick < TRUTH_AUCTION_MIN_SUPPORTED_TICK || tick > TRUTH_AUCTION_MAX_TICK)
        throw new Error('Truth auction tick is outside the supported range.');
}
export function getTruthAuctionPriceAtTick(tick) {
...
export function getTruthAuctionTickAtPrice(price) {
    if (price <= 0n || price < TRUTH_AUCTION_MIN_PRICE || price > TRUTH_AUCTION_MAX_PRICE)
        return undefined;
    if (price === TRUTH_AUCTION_PRICE_PRECISION)
        return 0n;
    if (price === TRUTH_AUCTION_MAX_PRICE)
...
//# sourceMappingURL=truthAuctionMath.js.map