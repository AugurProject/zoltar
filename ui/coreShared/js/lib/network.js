...
const COMMON_CHAIN_NAMES = new Map([
    [1n, 'Ethereum'],
    [10n, 'Optimism'],
    [25n, 'Cronos'],
    [56n, 'BNB Smart Chain'],
...
export function getChainIdDecimalLabel(chainId) {
    return parseChainId(chainId)?.toString();
}
export function getChainDisplayLabel(chainId) {
...
export function getKnownChainName(chainId) {
    if (chainId === undefined)
        return undefined;
    const numericChainId = parseChainId(chainId);
    if (numericChainId === undefined)
...
export function isSupportedAppChain(chainId) {
    const profile = getActiveNetworkProfile();
    return profile.isSupportedAppChain && sameChainId(chainId, profile.chainIdHex);
}
export function isActiveAppChain(chainId) {
    return isSupportedAppChain(chainId);
}
export function getWalletScopedAccountAddress(accountAddress, chainId) {
    if (accountAddress === undefined || chainId === undefined)
        return undefined;
