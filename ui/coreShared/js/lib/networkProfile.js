import { defineChain, getAddress } from '@zoltar/shared/ethereum';
...
const MAINNET_UNISWAP_V3_FACTORY_ADDRESS = '0x1F98431c8aD98523631AE4a59f267346ea31F984';
const MAINNET_UNISWAP_V3_QUOTER_ADDRESS = '0x61fFE014bA17989E743c5F6cB21bF9697530B21e';
const MAINNET_UNISWAP_V4_QUOTER_ADDRESS = '0x52f0e24d1c21c8a0cb1e5a5dd6198556bd9e1203';
...
export const MAINNET_NETWORK_PROFILE = {
    chain: mainnet,
    chainIdHex: '0x1',
...
export const SEPOLIA_NETWORK_PROFILE = {
    chain: sepoliaChain,
    chainIdHex: '0xaa36a7',
    displayName: 'Sepolia',
...
export function getPublicNetworkProfile(network) {
    const normalizedNetwork = network?.trim().toLowerCase();
    if (normalizedNetwork === undefined || normalizedNetwork === '' || normalizedNetwork === 'mainnet')
...
export function getPublicNetworkProfileForChainId(chainId) {
    if (sameChainId(chainId, MAINNET_NETWORK_PROFILE.chainIdHex))
        return MAINNET_NETWORK_PROFILE;
    if (sameChainId(chainId, SEPOLIA_NETWORK_PROFILE.chainIdHex))
        return SEPOLIA_NETWORK_PROFILE;
    return undefined;
}
...
export function buildTransactionExplorerUrl(profile, hash) {
    if (profile.transactionExplorerBaseUrl === undefined)
        return undefined;
    return `${profile.transactionExplorerBaseUrl}${hash}`;
}
export function formatTransactionNetworkLabel(profile) {
