...
export function parseScalarFormInputs({ scalarIncrement, scalarMax, scalarMin }) {
    ...
    if (displayValueMin < SCALAR_SIGNED_MIN || displayValueMin > SCALAR_SIGNED_MAX)
        throw new Error('Scalar min is outside the supported range');
    if (displayValueMax < SCALAR_SIGNED_MIN || displayValueMax > SCALAR_SIGNED_MAX)
        throw new Error('Scalar max is outside the supported range');
    if (increment <= 0n)
        throw new Error('Scalar increment must be greater than 0');
    if (displayValueMax <= displayValueMin)
        throw new Error('Scalar max must be greater than scalar min');
    ...
    if (numTicks <= 1n)
        throw new Error('Scalar inputs must produce more than 1 tick');
    if (numTicks > SCALAR_PART_MASK)
        throw new Error('Scalar range and increment produce too many ticks. Narrow the range or increase the increment.');
    if (numTicks > MAX_PRECISE_SCALAR_TICK_COUNT)
        throw new Error('Scalar range and increment produce too many ticks. Narrow the range or increase the increment.');
    return {
        displayValueMax,
        displayValueMin,
...
//# sourceMappingURL=scalarOutcome.js.map