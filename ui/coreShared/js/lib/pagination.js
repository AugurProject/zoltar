...
export function getPaginationPageCount(itemCount, pageSize) {
    if (itemCount === undefined)
        return undefined;
    if (itemCount < 0n)
        throw new RangeError('Pagination count must be non-negative');
    if (itemCount === 0n)
        return 0n;
    const pageSizeBigInt = getPageSizeBigInt(pageSize);
    return (itemCount + pageSizeBigInt - 1n) / pageSizeBigInt;
}
export function getHasNextPaginationPage(pageIndex, pageCount) {
    if (pageCount === undefined)
        return false;
    return BigInt(pageIndex) + 1n < pageCount;
}
export function resolvePaginationPageIndex(pageIndex, pageCount) {
    if (pageCount === undefined)
        return pageIndex;
    if (pageCount === 0n)
        return 0;
    const lastPageIndex = pageCount - 1n;
    if (BigInt(pageIndex) <= lastPageIndex)
        return pageIndex;
    if (lastPageIndex > BigInt(Number.MAX_SAFE_INTEGER))
        return pageIndex;
    return bigintToSafeNumber(lastPageIndex, 'Pagination page index');
}
export function formatPaginationSummary(pageIndex, pageCount) {
    if (pageCount === undefined)
        return undefined;
