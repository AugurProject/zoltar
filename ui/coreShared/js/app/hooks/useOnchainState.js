...
function buildReadBackendMismatchMessage(backend, actualChainId) {
...
function getReadBackendStatus(backend) {
    return (backend.getReadBackendStatus?.() ?? {
...
async function validateConfiguredReadBackend(backend) {
    try {
...
export async function loadWalletState({ chainIdPromise, connectedAddress, ethBalanceAttoEthPromise, fallbackChainId, getAccountState, isCurrent, setAccountState, setErrorMessage, setEthBalanceErrorMessage, setWethBalanceAttoEthErrorMessage, trackLoad, wethBalanceAttoEthPromise }) {
    if (connectedAddress === undefined || chainIdPromise === undefined || ethBalanceAttoEthPromise === undefined || wethBalanceAttoEthPromise === undefined)
        return;
    ...
    void trackLoad(async () => {
        try {
            ...
            if (!hasErrorCode(error) && !hasErrorMessage(error))
                throw error;
            if (!isCurrent())
                return;
...
async function loadBackendChainClock(backend) {
...
export function useOnchainState({ activeEnvironmentNonce = 0, enableChainClock = true, onSupportedNetworkChange } = {}, dependencies) {
    ...
    const deploymentStatuses = useSignal(dependencies.getDeploymentSteps().map(step => ({
        ...step,
        deployed: false,
    })));
    ...
    const augurStatoblastDeployed = useSignal(undefined);
    const currentTimestamp = useSignal(getActiveBackend().currentTimestamp);
    const currentBlockNumber = useSignal(undefined);
    const environmentBootstrapError = useSignal(undefined);
    ...
    const chainClockRefreshRef = useRef(undefined);
    ...
    const deploymentStatusError = useSignal(undefined);
    const ethBalanceAttoEthError = useSignal(undefined);
    const wethBalanceAttoEthError = useSignal(undefined);
    const chainClockError = useSignal(undefined);
    const readBackendMessage = useSignal(undefined);
    const readBackendValidated = useSignal(false);
    const readBackendStatus = useSignal(getReadBackendStatus(getActiveBackend()));
    const clearChainClock = () => {
        currentBlockNumber.value = undefined;
        currentTimestamp.value = undefined;
    };
    const updateReadBackendStatus = (backend, block) => {
        backend.setReadBackendBlock?.({
    ...
    const setDeploymentStatuses = (update) => {
        const updated = update(deploymentStatuses.value);
        deploymentStatuses.value = updated;
        if (updated.every(step => step.deployed))
            augurStatoblastDeployed.value = true;
    };
    ...
    const refreshChainClock = (backend) => {
        const activeRequest = chainClockRefreshRef.current;
        if (activeRequest !== undefined && activeRequest.activeEnvironmentNonce === activeEnvironmentNonce && activeRequest.backend === backend)
        ...
        void promise.finally(() => {
            if (chainClockRefreshRef.current?.promise === promise)
    ...
    const refreshState = async (options = {}) => {
        ...
        if (shouldLoadChainClock && isReadBackendReady())
    ...
    const runWalletManagementAction = async (action, fallbackMessage) => {
        if (isManagingWallet.value)
            return;
        const backend = getActiveBackend();
        manageWalletGenerationRef.current += 1;
    ...
    const changeWallet = async () => await runWalletManagementAction(async (backend) => {
        if (backend.requestAccountSelection === undefined)
            throw new Error('This wallet does not support account switching from the application. Open the wallet and choose another account.');
        await backend.requestAccountSelection();
    }, 'Wallet account change failed');
    const disconnectWallet = async () => await runWalletManagementAction(async (backend) => {
        if (backend.disconnectWallet === undefined)
            throw new Error('This wallet does not support disconnecting from the application. Disconnect this site in the wallet.');
        await backend.disconnectWallet();
    }, 'Wallet disconnect failed');
    const switchNetwork = async () => await runWalletManagementAction(async (backend) => {
        if (backend.switchNetwork === undefined)
            throw new Error(`This wallet does not support switching networks from the application. Switch to ${getNetworkSwitchTarget(backend.profile)} in the wallet.`);
    ...
    useEffect(() => {
        ...
        const unsubscribeState = backend.subscribe?.(() => {
            ...
            if (enableChainClock && isReadBackendReady())
                void refreshChainClock(backend);
        });
    ...
    return {
        ...
        errorMessages: [errorMessage.value, deploymentStatusError.value, ethBalanceAttoEthError.value, wethBalanceAttoEthError.value].filter((message) => message !== undefined),
...
//# sourceMappingURL=useOnchainState.js.map