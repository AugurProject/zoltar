...
export async function assertActiveWallet(accountAddress) {
    const backend = getActiveBackend();
    if (!backend.hasWallet())
        throw new Error('No wallet is available. Connect a wallet and try again.');
    ...
    if (!sameAddress(connectedAccount, accountAddress))
    ...
    if (!sameChainId(chainId, backend.profile.chainIdHex))
        throw new Error(`Wallet network changed. Switch to ${backend.profile.displayName} and try again.`);
    return {
        accountAddress: connectedAccount,
        chainId,
    };
}
//# sourceMappingURL=assertActiveWallet.js.map