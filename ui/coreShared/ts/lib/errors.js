...
export function isIgnorableLogDecodeError(error) {
...
export function isRecoverableContractReadError(error) {
    if (!(error instanceof Error))
        return false;
    if (recoverableContractReadErrorNames.includes(error.name))
        return true;
...
export function isRecoverableQuoteError(error) {
...
function collectErrorDetails(error, seen = new Set()) {
    ...
    if (Array.isArray(error))
        return readStringArrayValues(error);
    ...
    if (shortMessage !== undefined && shortMessage !== '')
        details.push(shortMessage);
    const nestedDetails = readStringValue(error['details']);
    if (nestedDetails !== undefined && nestedDetails !== '')
        details.push(nestedDetails);
...
function getKnownTransactionErrorDetail(details) {
    for (const detail of details) {
        if (detail.toLowerCase().includes('stale price'))
            return "The pool's oracle price expired. Request a new price in Price Oracle, then retry.";
    }
    return undefined;
}
function getContractNoDataDetail(value) {
    const normalized = value.toLowerCase();
    if (!normalized.includes('returned no data') && !normalized.includes('no data'))
        return undefined;
    if (!normalized.includes('contract function') && !normalized.includes('0x'))
        return undefined;
...
function shouldUseStandaloneWriteMessage(detail) {
    ...
    if (technicalWriteErrorPatterns.some(pattern => normalized.includes(pattern)))
        return false;
    return firstCharacter !== firstCharacter.toLowerCase();
}
export function sanitizeErrorDetail(detail, fallbackMessage) {
    if (detail === undefined)
        return undefined;
    ...
    if (sanitized === '' || isJsonOnlyValue(sanitized) || isGenericErrorDetail(sanitized))
        return undefined;
    if (fallbackMessage !== undefined && normalizeComparableMessage(sanitized) === normalizeComparableMessage(fallbackMessage))
        return undefined;
    const contractNoDataDetail = getContractNoDataDetail(sanitized);
    if (contractNoDataDetail !== undefined)
        return contractNoDataDetail;
    const maxLength = 160;
    return sanitized.length > maxLength ? `${sanitized.slice(0, maxLength - 3).trimEnd()}...` : sanitized;
}
export function getErrorDetail(error, fallbackMessage) {
    const details = collectErrorDetails(error);
    const knownTransactionErrorDetail = getKnownTransactionErrorDetail(details);
    if (knownTransactionErrorDetail !== undefined)
        return knownTransactionErrorDetail;
    for (const detail of details) {
...
export function formatWriteErrorMessage(error, fallbackMessage) {
    if (getCloseableMessage(error) !== undefined)
        return 'Action canceled in wallet.';
    const detail = getErrorDetail(error, fallbackMessage);
    if (detail !== undefined && shouldUseStandaloneWriteMessage(detail))
        return detail;
...
export function formatRefreshErrorMessage(error, fallbackMessage) {
    if (getCloseableMessage(error) !== undefined)
        return 'Action canceled in wallet.';
...
export function isCloseableErrorMessage(message) {
    if (message === undefined)
        return false;
    const normalizedMessage = message.toLowerCase();
    if (normalizedMessage.includes('"code":4001') || normalizedMessage.includes("'code':4001") || normalizedMessage.includes('code 4001'))
        return true;
