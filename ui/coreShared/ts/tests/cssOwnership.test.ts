...
test('persistent operational text and AugurScan disclosures keep accessible minimums', () => {
	...
	expect(controls).toMatch(/@media \(max-width: 64rem\) \{.*?\.header-toolbar-navigation \{[^}]*position: fixed;[^}]*bottom: 0;/s)
	expect(controls).toMatch(/\.account-menu-network dt,\s*\.account-menu-metrics \.metric-label \{[^}]*font-size: var\(--font-label\);/s)
	...
	expect(controls).toMatch(/\.account-menu-metrics \.metric-field-value \{[^}]*font-size: var\(--font-value\);/s)
