...
export function getAppBuildCommands(appIds: readonly UiAppId[]): readonly string[][] {
	...
	return [['run', 'ensure-contract-artifacts'], ...appIds.map(appId => ['./ui/coreShared/build/vendor.mts', appId]), ...packages.map(packageId => ['x', 'tsc', '--project', `ui/${packageId}/tsconfig.json`]), ...appIds.map(appId => ['./ui/coreShared/build/workers.mts', appId, '--artifacts-current'])]
}
