...
export function UniverseSelector({ options, selectedId, disabled, loading, onChange }: { options: readonly UniverseOption[]; selectedId: string | undefined; disabled: boolean; loading: boolean; onChange(selectedId: string): void }) {
	...
	return (
		<label class='universe-selector'>
			<select aria-label={copy.selectUniverse} title={selected?.accessibleLabel ?? selected?.label} value={selectedId ?? ''} disabled={disabled || options.length === 0} onChange={event => onChange(event.currentTarget.value)}>
				{options.length === 0 ? (
					<option value=''>{loading ? copy.loadingWithEllipsis : copy.unavailable}</option>
				) : (
					options.map(option => (
