	...
	for (const scenario of selectedScenarios) {
		...
		if (typeof result !== 'object' || result === null || !('value' in result) || result.value !== true) {
			const diagnostic = await command('Runtime.evaluate', {
				expression: `({ title: document.title, text: document.body.textContent, hash: location.hash, overflow: document.documentElement.scrollWidth - document.documentElement.clientWidth, hasSimulation: document.querySelector('.simulation-banner-details') !== null, hasOverview: document.querySelector('.top-shell .overview-panel') !== null, markets: document.querySelectorAll('.live-market-button').length })`,
				returnByValue: true,
			})
