...
export const SCALAR_PARITY_QUESTIONS = [
    {
...
export const SCALAR_PARITY_LABEL_FIXTURES = [
...
export const SCALAR_PARITY_ENCODING_FIXTURES = [
    {
...
export function getScalarParityQuestion(questionName) {
    const question = SCALAR_PARITY_QUESTIONS.find(candidate => candidate.name === questionName);
    if (question === undefined)
        throw new Error(`Unknown scalar parity question: ${questionName}`);
    return question;
}
export function combineScalarParityOutcomeIndex(invalid, firstPart, secondPart) {
...
export function getScalarParityOutcomeIndex(question, tickIndex) {
...
export function formatScalarParityLabel(question, tickIndex) {
    validateScalarParityTickIndex(question, tickIndex);
...
export function describeScalarParityOutcomeIndex(question, outcomeIndex) {
    if (question.numTicks <= 0n)
        return { kind: 'malformed' };
    if ((outcomeIndex & SCALAR_PARITY_RESERVED_BITS_MASK) !== 0n)
        return { kind: 'malformed' };
...
export function isScalarParityMalformedOutcomeIndex(question, outcomeIndex) {
    return describeScalarParityOutcomeIndex(question, outcomeIndex).kind === 'malformed';
}
export function formatScalarParityOutcomeName(question, outcomeIndex) {
...
function validateScalarParityTickIndex(question, tickIndex) {
    if (question.numTicks <= 0n)
...
function splitScalarParityOutcomeIndex(outcomeIndex) {
