export const ORACLE_PERCENTAGE_PRECISION = 10000000n;
...
export function calculateMaximumOracleInitialReportPriorityFeeAttoEthPerGas(parameters = DEFAULT_ORACLE_MINIMUM_WETH_REPORT_PARAMETERS, escalationHaltMultiplierBps = ORACLE_ESCALATION_HALT_MULTIPLIER_BPS) {
    if (parameters.gasUnitsForOneDispute <= 0n)
        throw new Error('Dispute gas units must be positive');
    if (escalationHaltMultiplierBps <= 0n)
        throw new Error('Escalation halt multiplier must be positive');
    ...
    if (correctionProfitNumerator <= 0n)
        throw new Error('Oracle fees must be below the target price error');
...
function ceilDivide(numerator, denominator) {
    if (denominator <= 0n)
        throw new Error('Cannot divide by zero or a negative denominator');
    return (numerator + denominator - 1n) / denominator;
}
export function calculateOracleMinimumWethReportAttoEth(parameters = DEFAULT_ORACLE_MINIMUM_WETH_REPORT_PARAMETERS) {
    const feeSum = BigInt(parameters.openOracleProtocolFee + parameters.openOracleReporterFee);
    const correctionProfitNumerator = parameters.targetPriceErrorForDispute - feeSum;
    const denominator = BPS_DENOMINATOR * correctionProfitNumerator;
    const calculateGasPriceReport = (gasPriceAttoEthPerGas) => {
        if (gasPriceAttoEthPerGas === 0n)
            return 0n;
