...
- Keep disabled controls legible and explain why the primary expected action is unavailable. Every disabled button uses the single shared disabled treatment regardless of tone, so disabled primary, secondary, and approval actions look identical and never resemble an enabled secondary button.
