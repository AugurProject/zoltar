...
export function VaultMetricGrid({ className = '', layout = 'grid', disputeStakedRepAttoRep, priceValidUntilTimestamp, vaultRepBackingAttoRep, coverageCommitmentAttoEth }: VaultMetricGridProps) {
	if (layout === 'preview')
		return (
			<div className={['vault-preview-strip', className].filter(Boolean).join(' ')}>
				...
				<div className='vault-preview-side-metrics'>
					<VaultPrimaryMetric label={commonCopy.poolHeldVaultRepBackingAttoRep} value={vaultRepBackingAttoRep} suffix={commonCopy.rep} />
				</div>
				<div className='vault-preview-meta'>
					{disputeStakedRepAttoRep === undefined ? null : (
						<MetricField label={commonCopy.disputeStakedRepAttoRep}>
							<CurrencyValue value={disputeStakedRepAttoRep} suffix={commonCopy.rep} />
						</MetricField>
					)}
	...
	return (
		<div className={['vault-detail-stage', className].filter(Boolean).join(' ')}>
			...
			<div className='vault-detail-meta'>
				...
				{disputeStakedRepAttoRep === undefined ? undefined : (
					<MetricField label={commonCopy.disputeStakedRepAttoRep}>
						<CurrencyValue value={disputeStakedRepAttoRep} suffix={commonCopy.rep} />
					</MetricField>
				)}
