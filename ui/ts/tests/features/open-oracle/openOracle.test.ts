...
import { getOpenOracleExtraData, getRequestPriceCostAttoEth, requestPriceWithValue, wrapWeth as wrapWethTestHelper } from '../../../../../solidity/ts/testSupport/simulator/utils/contracts/peripherals'
...
describe('Open Oracle helpers', () => {
	...
	test('posts and accepts a WETH operation bounty through the UI protocol client', async () => {
		...
		const postResult = await postOracleOperationBounty(uiWriteClient, managerAddress, {
			...
			amount: 1n,
			maximumInitialAttoWeth: 0n,
			minimumInitialAttoWeth: 0n,
	...
	test('rejects an operation bounty when the pending settlement queue is full without submitting a transaction', async () => {
		...
		for (let index = 0; index < 4; index += 1) await queueOracleManagerOperation(uiWriteClient, managerAddress, 'liquidation', addressString(TEST_ADDRESSES[1]), 1n, DEFAULT_SELF_OPERATION_TIMEOUT_SECONDS, index === 0 ? minimumToken1ReportAttoEth : undefined)
		const transactionCountBefore = await getUiAccountTransactionCount()
	...
	test('loads bounty one directly after newer bounties push it out of the preview', async () => {
		...
		for (let index = 0; index < 26; index += 1) {
			const hash = await client.writeContract({
				...
				args: [0, addressString(TEST_ADDRESSES[1]), 1n, DEFAULT_SELF_OPERATION_TIMEOUT_SECONDS, WETH_ADDRESS, 1n, currentTimestamp + DAY, 0n, 0n],
			})
