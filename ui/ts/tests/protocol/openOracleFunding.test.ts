...
describe('oracle manager queue funding helpers', () => {
	test('uses zero ETH when an existing report already owns the only pending slot', async () => {
		const readContract: Parameters<typeof loadOracleManagerQueueOperationEthValue>[0]['readContract'] = async request => {
			...
			if (request.address === MANAGER_ADDRESS && request.functionName === 'MAX_PENDING_SETTLEMENT_OPERATIONS') return 1n as never
			if (request.address === MANAGER_ADDRESS && request.functionName === 'pendingReportId') return 7n as never
			if (request.address === MANAGER_ADDRESS && request.functionName === 'candidateReportId') return 0n as never
			...
			if (request.address === MANAGER_ADDRESS && request.functionName === 'isPriceUsable') return false as never
