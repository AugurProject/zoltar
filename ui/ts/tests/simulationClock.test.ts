...
function createSimulationNode(overrides: {
    blockStateRootHex: Hex
    includeStateRoot: boolean
    txForHash?: { isFound: boolean; txValue?: unknown }
}) {
...
describe('simulation clock', () => {
    ...
    test('throws when mining a pending transaction that is missing from the tx pool', async () => {
        const { node, vm, canonicalBlock, blockBuilder } = createSimulationNode({
        ...
        await expect(minePendingSimulationTransactionAtTimestamp(memoryClient as never, txHash, SIMULATION_INITIAL_TIMESTAMP)).rejects.toThrow(
            `Simulation transaction ${txHash} was not found in the tx pool`,
        )
    })
