...
describe('documented protocol constants', () => {
	...
	test('keeps educational entry points and auction design in HTML docs', async () => {
		...
		expect(startHere).toContain('Placeholder is the market layer. Zoltar is the forkable oracle layer')
		expect(startHere).toContain('Only the hard cases leave that local path')
		expect(startHere).toContain('A normal market begins in one pool')
		expect(startHere).toContain('Keep one distinction in mind throughout')
		expect(startHere).toContain('The stack has four layers')
		expect(startHere).toContain('Reproduce parent-universe migration balance')
		expect(startHere.indexOf('Placeholder is the market layer. Zoltar is the forkable oracle layer')).toBeLessThan(startHere.indexOf('Keep one distinction in mind throughout'))
		...
		const childRepMint = startHere.search(/Calling <code>splitMigrationRep<\/code> mints\s+<em>child REP<\/em>/)
		expect(startHere.indexOf('<em>migration balance</em>')).toBeLessThan(childRepMint)
		expect(startHere).toContain('Calling <code>splitMigrationRep</code> mints <em>child REP</em> in')
		expect(startHere).toMatch(/each selected fork\s+branch/)
		expect(startHere).toContain('with each branch receiving at most the')
		expect(startHere).toContain('source balance.')
		expect(startHere).toMatch(/Reproduce parent-universe migration balance[\s\S]*each\s+child capped by the source balance/)
		expect(startHere).not.toContain('Reproduction mints <em>child REP</em> reproduced')
		...
		expect(auctionDesign).toContain('Truth Auction')
		expect(auctionDesign).toContain('Why This Auction Exists')
		expect(auctionDesign).toContain('A truth auction is held only after a fork')
		expect(auctionDesign).toContain('Placeholder migrates it into child pools')
		...
		expect(auctionDesign).not.toContain('forker only')
		expect(auctionDesign).not.toContain('The owner starts')
		expect(auctionDesign.indexOf('A truth auction is held only after a fork')).toBeLessThan(auctionDesign.indexOf('Fork migration preserves disagreement by splitting one parent market'))
		expect(auctionDesign.indexOf('Fork migration preserves disagreement by splitting one parent market')).toBeLessThan(auctionDesign.indexOf('Placeholder implements the truth auction with'))
		...
		expect(placeholder).toMatch(/The direct <code>startAuction<\/code> and <code>finalize<\/code>\s+calls are owner-only\./)
		expect(placeholder).toContain('<code>SecurityPoolForker.startTruthAuction</code>')
		expect(placeholder).toContain('<code>SecurityPoolForker.finalizeTruthAuction</code>')
		expect(placeholder).not.toContain('Only the owner, which is the <code>SecurityPoolForker</code>')
		expect(operatorReference).toContain('Implementation guardrails map to their contract sources')
		expect(operatorReference).toContain('while anyone can reach them through `startTruthAuction` and `finalizeTruthAuction`')
