...
export function useOpenOracleOperations({ accountAddress, onTransaction, refreshState }: UseOpenOracleOperationsParameters) {
	...
	const approveToken1 = async () =>
		await runOracleAction(async walletAddress => {
			...
			return await approveErc20(createWriteClient(ethereum, walletAddress), details.token1, details.openOracleAddress, parseBigIntInput(openOracleForm.value.amount1, 'Token1 amount'), 'approveToken1')
	...
	const approveToken2 = async () =>
		await runOracleAction(async walletAddress => {
			...
			return await approveErc20(createWriteClient(ethereum, walletAddress), details.token2, details.openOracleAddress, parseBigIntInput(openOracleForm.value.amount2, 'Token2 amount'), 'approveToken2')
