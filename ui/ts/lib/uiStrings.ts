...
export const CURATED_TSX_STRINGS = {
	actionSafety: {
		zoltarForkChecklist: ['Forking Zoltar changes the recovery path for the selected universe.', 'Users may need to split REP or migrate positions into child universes after this step.'],
		triggerZoltarForkChecklist: ['This pushes the pool out of local resolution and into fork recovery.', 'Follow-up migration and settlement work will continue in child universes, not this parent pool.'],
		childUniverseDeployChecklist: ['This deploys a child universe on-chain.', 'You may need to migrate positions or REP into the new universe before it becomes useful.'],
		migrateEscalationDepositsChecklist: ['The selected escalation deposits will move into the chosen child universe.', 'After migration, settlement continues in the child pool context rather than the parent pool.'],
		migrateRepToZoltarChecklist: ['This prepares pool REP for the selected child universe.', 'Vault migration and child-pool recovery depend on the selected outcome path staying consistent.'],
		migrateUnresolvedEscalationChecklist: ['All unresolved escalation locks on this wallet move together with the selected outcome path.', 'If you choose the wrong child universe, follow-up settlement becomes harder to reason about.'],
		migrateVaultChecklist: ['This moves the connected vault into the selected child universe.', 'Vault collateral, underwriting state, and later actions will continue there instead of the parent pool.'],
		startTruthAuctionChecklist: ['This starts the child-pool repair step or bypasses it when no auction is needed.', 'Once started, settlement and bidder behavior follow the truth-auction path for this child pool.'],
		claimAuctionProceedsChecklist: (value0: UiStringTemplateValue) => [
			`Claiming a winning bid adds child-pool REP and a pro-rata share of the ${value0} to the bidder vault.`,
			`That ${value0} is the remaining open-interest debt being assigned during auction settlement.`,
			`If the same settlement call includes losing bids, those rows are refunded while winning rows still add REP and ${value0}.`,
		],
		settleAuctionRefundsChecklist: (value0: UiStringTemplateValue) => [
			'This uses the finalized auction settlement path because the selected rows are already in the child-pool settlement phase.',
			`The selected rows should return locked ETH only and should not assign child-pool REP or ${value0}.`,
		...
		submitBidChecklist: (value0: UiStringTemplateValue) => [
			`Winning settlement does not only buy REP. It also assigns a pro-rata share of the ${value0} to the bidder vault.`,
			`That ${value0} is the remaining open-interest debt being assigned to auction participants.`,
		...
		marketCreateQuestionChecklist: ['This writes a new Zoltar question on-chain.', 'Only exact binary Yes / No origin questions are currently eligible for Placeholder origin security pools.'],
		openOracleCreateReportChecklist: ['This creates a standalone Open Oracle report instance on-chain.', 'Report lifecycle, approvals, disputes, and settlement will follow from this deployment.'],
		openOracleDisputeChecklist: ['This submits a new dispute state for the selected report.', 'You should confirm the new price and token amounts before continuing.'],
		openOracleSettleChecklist: ['This settles the currently active report state.', 'Settlement may finalize balances and end further dispute actions for this report.'],
		openOracleSubmitInitialReportChecklist: ['This submits the initial report state on-chain.', 'The selected price and token amounts become the starting point for later disputes and settlement.'],
		reportOutcomeChecklist: ['This can lock REP into the selected reporting side until local resolution or later settlement.', 'If the pool reaches non-decision or fork recovery, follow-up actions may move to child-universe workflows.'],
		withdrawEscalationChecklist: ['This settles the selected escalation deposits for the chosen side.', 'Claim value depends on the current finalized or migrated settlement path shown in the UI.'],
		createPoolChecklist: ['This deploys a new Placeholder security pool for the selected question and multiplier.', 'Origin pool addresses are deterministic for the question and multiplier pair.'],
		queueLiquidationChecklist: ['This can queue or execute a liquidation against the selected vault.', 'Oracle timing and settlement mode can determine whether execution is immediate or requires manual follow-up.'],
		depositRepChecklist: ['This deposits REP into the selected security vault.', 'Deposited REP backs underwriting capacity and may stay locked until later withdrawal or redemption conditions are met.'],
		queueSetSecurityBondAllowanceChecklist: ['This changes the vault underwriting allowance using the latest oracle-backed value path.', 'Increasing or decreasing allowance changes how much open interest this vault backs.'],
		queueWithdrawRepChecklist: ['This queues a REP withdrawal from the selected vault.', 'Execution may depend on oracle settlement timing and can require manual follow-up when auto-execution is unavailable.'],
		createCompleteSetChecklist: ['This locks ETH collateral into the pool and mints Invalid, Yes, and No shares together.', 'Placeholder is not a secondary-market venue, so this is a minting action rather than an exchange trade.'],
		migrateSharesChecklist: ['Share migration burns the entire selected parent balance for that outcome. Partial migration is not available.', 'The burned balance is reproduced into each valid child outcome you select, rather than being split pro rata.'],
		redeemCompleteSetChecklist: ['This burns matching Invalid, Yes, and No shares together.', 'The corresponding collateral amount is returned from the pool.'],
		redeemSharesChecklist: ['This burns finalized winning shares for collateral redemption.', 'Only the resolved winning path pays out here.'],
		prepareRepChecklist: ['This prepares REP for later child-universe splitting.', 'Prepared balances are part of the post-fork migration path, not normal trading flow.'],
		splitRepChecklist: ['This splits prepared REP across the selected child universes.', 'After splitting, follow-up actions continue with the resulting child-universe REP balances.'],
	},
	repPriceSource: {
...
export const TSX_STRINGS = {
	...
	componentsActionSafetyModal: {
		copy001: 'Continue To Wallet',
		copy002: 'High Risk',
		copy003: 'Review Needed',
		copy004: 'I reviewed the transaction consequences shown above.',
		copy005: SHARED_UI_STRINGS.cancel,
	},
	...
	libActionSafetyRuntime: {
		copy001: 'I understand this moves the selected universe into the fork path.',
		copy002: SHARED_UI_STRINGS.forkZoltar,
		copy003: 'Use this only when the universe should enter its fork process.',
		copy004: 'Review Zoltar Fork',
		copy005: 'I understand that triggering the fork moves this pool into fork recovery.',
		copy006: 'Trigger Fork',
		copy007: 'Use this only after non-decision when you are ready to move the pool into fork recovery.',
		copy008: SHARED_UI_STRINGS.triggerZoltarFork,
		copy009: 'Create a child universe only when you are intentionally continuing post-fork recovery.',
		copy010: 'Review Child Universe Deployment',
		copy011: 'Confirm that the selected child outcome is the one you want these deposits to follow.',
		copy012: 'Migrate Escalation Deposits',
		copy013: 'Move pool REP only when you are confident this is the correct recovery universe.',
		copy014: 'Migrate Pool REP',
		copy015: 'This moves unresolved parent escalation state into one child universe.',
		copy016: 'Migrate Unresolved Escalation',
		copy017: 'Migrate the vault only after confirming the correct child outcome and pool path.',
		copy018: SHARED_UI_STRINGS.migrateVault,
		copy019: 'Starting the truth auction changes the recovery phase for this child pool.',
		copy020: SHARED_UI_STRINGS.startTruthAuction,
		copy021: (value0: UiStringTemplateValue) => `I understand a winning claim assigns both REP and ${value0} to the bidder vault.`,
		copy022: 'Claim Auction Proceeds',
		copy023: (value0: UiStringTemplateValue) => `Review the selected winning bids before assigning their REP and ${value0} to the bidder vault.`,
		copy024: 'Review Auction Claim',
		copy025: (value0: UiStringTemplateValue) => `I understand this finalized refund-only settlement returns locked ETH and does not assign child-pool REP or ${value0}.`,
		copy026: SHARED_UI_STRINGS.settleFinalizedRefunds,
		copy027: 'Review the selected finalized refund rows before settling them through the child-pool settlement path.',
		copy028: 'Review Finalized Refund Settlement',
		copy029: (value0: UiStringTemplateValue) => `I understand a winning bid can later claim both REP and ${value0}.`,
		copy030: SHARED_UI_STRINGS.submitBid,
		copy031: 'Review the auction price, amount, and resulting underwriting exposure before submitting a truth-auction bid.',
		copy032: 'Review Truth Auction Bid',
		copy033: 'Confirm the market type and question wording before creating the question.',
		copy034: SHARED_UI_STRINGS.createQuestion,
		copy035: 'Create a standalone oracle game only when you are ready to operate its full report lifecycle.',
		copy036: 'Create Open Oracle Report',
		copy037: 'Disputing updates the active report path for this oracle game.',
		copy038: SHARED_UI_STRINGS.disputeReport,
		copy039: 'Settle only when the report state is ready to finalize.',
		copy040: SHARED_UI_STRINGS.settleReport,
		copy041: 'Confirm the fetched quote, source, and token approvals before submitting the initial report.',
		copy042: SHARED_UI_STRINGS.submitInitialReport,
		copy043: 'I understand this can lock REP into the selected escalation path.',
		copy044: SHARED_UI_STRINGS.reportOutcome,
		copy045: 'Review the selected outcome, contribution amount, and escalation state before reporting.',
		copy046: 'Review Outcome Report',
		copy047: 'Confirm which deposits you are settling and the value path they currently show.',
		copy048: SHARED_UI_STRINGS.settleEscalationDeposits,
		copy049: 'Create a pool only after confirming that the selected question is the intended binary origin market.',
		copy050: SHARED_UI_STRINGS.createSecurityPool,
		copy051: 'I understand this is a high-risk liquidation action against the selected vault.',
		copy052: SHARED_UI_STRINGS.queueLiquidation,
		copy053: 'Liquidation is a destructive operator action. Review the vault, oracle status, and amount carefully.',
		copy054: 'Review Vault Liquidation',
		copy055: 'Confirm the REP amount and vault before adding collateral.',
		copy056: SHARED_UI_STRINGS.depositRep,
		copy057: 'Review the new bond allowance and oracle context before queueing the change.',
		copy058: SHARED_UI_STRINGS.setBondAllowance,
		copy059: 'Confirm the withdrawal amount and staged-operation timing before continuing.',
		copy060: 'Queue REP Withdrawal',
		copy061: 'Confirm the complete-set mint amount and remaining pool capacity before minting.',
		copy062: SHARED_UI_STRINGS.mintCompleteSets,
		copy063: 'I understand the full selected parent share balance is burned and reproduced across the chosen child outcomes.',
		copy064: SHARED_UI_STRINGS.migrateShares,
		copy065: 'Review the source outcome and every selected child target before migrating shares.',
		copy066: 'Review Share Migration',
		copy067: 'Confirm the redeemable complete-set amount before burning shares.',
		copy068: SHARED_UI_STRINGS.redeemCompleteSets,
		copy069: 'Confirm that the pool has resolved and you are redeeming the intended winning shares.',
		copy070: SHARED_UI_STRINGS.redeemResolvedShares,
		copy071: 'Prepare REP only when you intend to continue into migration.',
		copy072: SHARED_UI_STRINGS.prepareRep,
		copy073: 'Confirm the selected child outcomes before splitting prepared REP.',
		copy074: SHARED_UI_STRINGS.splitRep,
	},
	libRepPriceSource: {
		copy001: 'MOCK',
