...
const getRouteHashForRoute = (route) => {
    if (route === 'deploy')
        return '#/deploy';
...
export function App() {
    ...
    const supportedNetworkChangeCoordinatorRef = useRef();
    const supportedNetworkChangeCoordinator = supportedNetworkChangeCoordinatorRef.current ??
    ...
    const { activeUniverseId, openOracleReportId: urlOpenOracleReportId, openOracleView, securityPoolsView, securityPoolAddress, securityPoolQuestionId, selectedPoolView, setActiveUniverseId, setOpenOracleReport, setOpenOracleView, setSecurityPoolsView, setSecurityPoolAddress, setSecurityPoolQuestionId, setSelectedPoolView, } = useUrlState();
    const onTransactionRequested = (intent) => {
        transactionState.value = markTransactionRequested(transactionState.value, intent);
    };
    const onTransactionPrepared = (preview) => {
        transactionState.value = markTransactionPrepared(transactionState.value, preview);
    };
    const onTransactionSubmitted = (hash) => {
        transactionState.value = markTransactionSubmitted(transactionState.value, hash);
    };
    const onTransactionFailed = (message) => {
    ...
    const onTransactionPresented = (presentation) => {
    ...
    const { approveRep, depositRepToVault, loadSecurityVault, loadingSecurityVault, redeemFees, redeemRepFromVault, securityVaultActiveAction, securityVaultDetails, securityVaultError, securityVaultForm, securityVaultMissing, securityVaultRepApproval, walletRepBalanceAttoRep, walletRepBalanceError, walletRepBalanceLoading, securityVaultResult, setSecurityVaultForm, withdrawRep, } = useSecurityVaultOperations({ ...walletScopedHookConfig, enabled: route === 'security-pools' && canReadOnchainData, selectedSecurityPoolAddress: securityPoolAddress });
    ...
    const { approveToken1, approveToken2, cancelWithdrawalBalanceCheck, createOpenOracleGame, disputeReport, loadOracleReport, loadingOpenOracleCreate, openOracleActiveAction, openOracleActiveWithdrawalBalance, openOracleCreateForm, openOracleCreateFieldErrors, openOracleDisputeSubmission, openOracleError, openOracleForm, openOracleReportLookupState, openOracleTokenAccessState, openOracleReportDetails, openOracleResult, openOracleWithdrawalBalanceChecking, openOracleWithdrawalReviewMessage, openOracleWithdrawableBalances, openOracleWithdrawableBalancesError, openOracleWithdrawableBalancesLoading, setOpenOracleCreateForm, setOpenOracleForm, settleReport, withdrawBalance, } = useOpenOracleOperations({
        ...walletScopedHookConfig,
        enabled: route === 'open-oracle' && canReadOnchainData,
        onReportSettled: async () => {
            if (poolOracleManagerDetails?.managerAddress !== undefined)
                await loadPoolOracleManager(poolOracleManagerDetails.managerAddress);
        },
    });
    ...
    const updateReportingForm = (update) => {
        setReportingForm((current) => applyReportingFormUpdate(current, update));
    };
    const { checkedSecurityPoolAddress, closeLiquidationModal, hasLoadedSecurityPoolPage, liquidationDebtEthAmount, maximumLiquidationDebtAttoEth, liquidationManagerAddress, liquidationFundingPreview, liquidationFundingPreviewError, liquidationModalOpen, liquidationSecurityPoolAddress, liquidationTargetVault, liquidationReceiverVault, liquidationApprovalId, liquidationApprovalDetails, liquidationApprovalError, liquidationReceiverVaultSummary, liquidationReceiverVaultSummaryError, liquidationReceiverVaultSummaryResolved, liquidationTimeoutMinutes, loadingSecurityPools, loadingLiquidationFundingPreview, loadingLiquidationApproval, loadingLiquidationReceiverVaultSummary, loadingSecurityPoolPage, loadBrowseSecurityPoolPage, loadSecurityPools, loadLiquidationFundingPreview, loadLiquidationApproval, loadLiquidationReceiverVaultSummary, openLiquidationModal, queueLiquidation, securityPoolOverviewActiveAction, securityPoolOverviewError, securityPoolLiquidationError, securityPoolOverviewResult, securityPoolBrowseCount, securityPoolPage, securityPools, setLiquidationAmount, setLiquidationReceiverVault, setLiquidationApprovalId, setLiquidationTimeoutMinutes, } = useSecurityPoolsOverview({ ...walletScopedHookConfig, environmentRefreshKey: activeEnvironmentNonce });
    ...
    const { claimAuctionProceeds, createChildUniverse, finalizeTruthAuction, forkAuctionActiveAction, forkAuctionDetails, forkAuctionError, forkAuctionForm, forkAuctionResult, forkUniverse, forkWithOwnEscalation, initiateFork, loadForkAuction, loadingForkAuctionDetails, claimParentEscalation, migrateUnresolvedEscalation, migrateRepToZoltar, migrateVault, refundLosingBids, setForkAuctionForm, settleForkedEscalation, startTruthAuction, submitBid, } = useForkAuctionOperations({ ...walletScopedHookConfig, selectedSecurityPoolAddress: securityPoolAddress });
    ...
    const lastSecurityVaultRepRefreshHash = useRef(undefined);
    const lastStagedVaultRepRefreshHash = useRef(undefined);
    const deploymentSections = getDeploymentSections(deploymentStatuses);
    const errorMessages = [deploymentErrorMessage, ...onchainErrorMessages.filter(message => message !== deploymentStatusError), chainClockError].filter((message) => message !== undefined);
    ...
    useEffect(() => {
        ...
        if (lastSecurityVaultRepRefreshHash.current === securityVaultRepRefreshHash)
            return;
    ...
    useEffect(() => {
        ...
        if (lastStagedVaultRepRefreshHash.current === stagedVaultRepRefreshHash)
            return;
    ...
    const securityPoolsRouteContentProps = {
        ...
        createPool: {
            ...
            onSecurityPoolFormChange: update => {
                ...
                if (update.marketId !== undefined)
                    setSecurityPoolQuestionId(update.marketId);
            },
            zoltarUniverseHasForked,
            securityPools,
        ...
        overview: {
            ...
            onLoadSecurityPoolPage: (pageIndex, pageSize, requestKey) => void loadBrowseSecurityPoolPage(pageIndex, pageSize, requestKey),
        ...
        workflow: {
            ...
            forkAuction: {
                ...
                onClaimParentEscalationDeposits: (outcome, depositIndexes) => void claimParentEscalation({
                    outcome,
            ...
            onLoadLiquidationFundingPreview: (managerAddress) => void loadLiquidationFundingPreview(managerAddress),
            onOpenLiquidationModal: (managerAddress, selectedSecurityPoolAddress, vaultAddress, maxAmount) => openLiquidationModal(managerAddress, selectedSecurityPoolAddress, vaultAddress, maxAmount),
            ...
            onQueueLiquidation: (managerAddress, selectedSecurityPoolAddress) => void queueLiquidation(managerAddress, selectedSecurityPoolAddress),
            onExecutePendingPoolOperation: (managerAddress, operationId, securityPoolAddress) => void executePendingPoolOperation(managerAddress, operationId, securityPoolAddress),
            loadingPoolOracleManager,
            loadingLiquidationFundingPreview,
            loadingSecurityPools,
            onLoadPoolOracleManager: (managerAddress) => void loadPoolOracleManager(managerAddress),
            onRequestPoolPrice: (managerAddress, securityPoolAddress, reviewedRequestValueAttoEth) => void requestPoolPrice(managerAddress, securityPoolAddress, reviewedRequestValueAttoEth),
    ...
    let routeSubNavigation = undefined;
    if (route === 'security-pools') {
        routeSubNavigation = (_jsx(RouteSubNavigation, { ariaLabel: appCopy.securityPoolsViews, value: activeSecurityPoolsView, onChange: view => setSecurityPoolsView(view), options: [
        ...
        routeSubNavigation = (_jsx(RouteSubNavigation, { ariaLabel: appCopy.oracleReportViews, value: activeOpenOracleView, onChange: view => setOpenOracleView(view), options: [
    ...
    const transactionRouteKey = (() => {
        if (route === 'security-pools')
            return `${route}:${activeSecurityPoolsView}`;
        if (route === 'open-oracle')
            return `${route}:${activeOpenOracleView}`;
        return route;
    })();
    return (_jsx(ChainBlockNumberContext.Provider, { value: currentBlockNumber, children: _jsx(ChainTimestampContext.Provider, { value: currentTimestamp, children: _jsxs("main", { children: [_jsx(AppPageHeading, { formatDocumentTitle: formatAppDocumentTitle, pageTitle: pageTitle }), _jsx(AppStatusNotices, { errorMessages: errorMessages, readBackendMessage: readBackendMessage, readBackendStatus: readBackendStatus, simulationBootstrapError: environmentBootstrapError, showAugurStatoblastDeploymentWarning: showAugurStatoblastDeploymentWarning, zoltarUniverseError: zoltarUniverseError }), _jsx(AppHeaderShell, { overview: overviewProps, simulationController: simulationController, subNavigation: routeSubNavigation, tabNavigation: tabNavigationProps, onEnvironmentChanged: refreshActiveEnvironment, onRefresh: refreshSimulationView }), _jsxs(GlobalTransactionPresentationProvider, { transaction: transactionState.value.active, children: [_jsx(GlobalTransactionTray, { routeKey: transactionRouteKey, transaction: transactionState.value.active }), _jsx("div", { id: 'app-content', tabIndex: -1, children: _jsx(TransactionActionButtonLockProvider, { disabledReason: getTransactionActionLockReason(transactionState.value), children: _jsx("fieldset", { className: 'route-shell', disabled: isRouteContentDisabled, children: _jsx(AppRouteContent, { deploy: deployRouteContentProps, openOracle: openOracleRouteContentProps, readBackendMessage: readBackendMessage, route: activeRoute, securityPools: securityPoolsRouteContentProps }) }) }) })] })] }) }) }));
}
