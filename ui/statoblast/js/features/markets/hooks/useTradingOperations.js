...
import { parseAddressInput, parseBigIntListInput, parseReportingOutcomeInput, tryParseAddressInput } from '@zoltar/ui-core-shared/lib/inputs.js';
import { parseTradingAmountInput } from '@zoltar/ui-core-shared/lib/formInputs.js';
...
const defaultUseTradingOperationsDependencies = {
...
export function useTradingOperations({ accountAddress, deploymentStatuses, enabled, onTransactionCanceled, onTransactionFailed, onTransactionFinished, onTransactionPresented, onTransactionPrepared, onTransactionRequested, onTransactionSubmitted, refreshState, selectedSecurityPoolAddress }, dependencies = defaultUseTradingOperationsDependencies) {
    const tradingDetailsLoad = useLoadController();
    const nextTradingDetailsLoad = useRequestGuard();
    const tradingDetails = useSignal(undefined);
    const tradingForkUniverse = useSignal(undefined);
    const tradingError = useSignal(undefined);
    const tradingForm = useSignal(getDefaultTradingFormState());
    const tradingActiveAction = useSignal(undefined);
    const tradingFeedback = useSignal(undefined);
    const tradingResult = useSignal(undefined);
    const targetOutcomeDefaultsKey = useRef(undefined);
    ...
    const resolveTradingPoolAddressInput = (value) => {
    ...
    const isTradingSelectionCurrent = (selectionKey) => currentTradingSelectionKeyRef.current === selectionKey;
    const refreshTradingDetails = async (securityPoolAddressInput, walletAddress, isCurrent) => {
        ...
        const loadOptions = {
            ...
            onSuccess: ({ details, forkUniverse }) => {
    ...
    const runTradingAction = async (actionName, action, errorFallback) => {
        ...
        try {
            ...
            await runWriteAction({
                ...buildWriteActionConfig({ accountAddress, onTransactionCanceled, onTransactionFailed, onTransactionFinished, onTransactionPresented, onTransactionPrepared, onTransactionRequested, refreshState }, tradingError, 'Connect a wallet before trading', createTradingTransactionIntent(actionName, transactionContext)),
                onRefreshError: (message, hash) => {
                    ...
                    if (result !== undefined)
                        onTransactionPresented(createTradingWarningPresentation(result, message));
                },
                onWriteCanceled: () => {
    ...
    const createCompleteSet = async () => await runTradingAction('createCompleteSet', async (walletAddress, securityPoolAddress, currentForm, isCurrentSelection) => {
    ...
    const redeemCompleteSet = async () => await runTradingAction('redeemCompleteSet', async (walletAddress, securityPoolAddress, currentForm, isCurrentSelection) => {
        const latestMintCapacity = await dependencies.loadSecurityPoolMintCapacity(securityPoolAddress);
        if (!isCurrentSelection())
            return undefined;
        ...
        if (redeemAmountAttoShares === undefined)
            throw new Error('Redeeming is unavailable because this pool has complete-set shares but no collateral.');
    ...
    const redeemShares = async () => await runTradingAction('redeemShares', async (walletAddress, securityPoolAddress, _currentForm, isCurrentSelection) => {
        if (!isCurrentSelection())
            return undefined;
    ...
    const migrateShares = async () => await runTradingAction('migrateShares', async (walletAddress, securityPoolAddress, currentForm, isCurrentSelection) => {
        if (!isCurrentSelection())
            return undefined;
    ...
    useEffect(() => {
        ...
        if (resolveTradingPoolAddressInput(effectiveSecurityPoolAddressInput) === undefined)
            return;
    ...
    useEffect(() => {
        ...
        if (securityPoolAddress === undefined || tradingForkUniverse.value === undefined)
            return;
        ...
        if (targetOutcomeDefaultsKey.current === defaultsKey)
            return;
        tradingForm.value = {
            ...tradingForm.value,
...
//# sourceMappingURL=useTradingOperations.js.map