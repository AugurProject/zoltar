...
function createQuestionData(form) {
    ...
    if (questionData.endTime <= questionData.startTime)
...
function getMissingRequiredCategoricalOutcomeLabels(form) {
    return [0, 1].filter(index => normalizeOutcomeLabel(form.categoricalOutcomes[index] ?? '') === '').map(index => `Outcome ${index + 1}`);
}
function getRequiredCategoricalOutcomeMessage(missingOutcomeLabels) {
    if (missingOutcomeLabels.length === 1)
        return `${missingOutcomeLabels[0]} is required`;
...
function getCategoricalOutcomeLabels(form) {
    ...
    if (missingRequiredOutcomeLabels.length > 0)
        throw new Error(getRequiredCategoricalOutcomeMessage(missingRequiredOutcomeLabels));
    const outcomeLabels = form.categoricalOutcomes.map(normalizeOutcomeLabel).filter(label => label !== '');
    if (new Set(outcomeLabels).size !== outcomeLabels.length)
...
export function getMarketCreationOutcomeLabels(form) {
    return getOutcomeLabels(form);
}
export function hasMarketEndTimePassed(form, currentTimestamp) {
    if (currentTimestamp === undefined || form.endTime.trim() === '')
        return false;
...
export function validateMarketForm(form) {
    const fieldErrors = {};
    const missingFields = [];
    ...
    if (missingFields.length > 0)
        noticeParts.push(`Missing required fields: ${formatFieldList(missingFields)}`);
    if (invalidMessages.length > 0)
        noticeParts.push(`Fix invalid fields: ${[...new Set(invalidMessages)].join(', ')}`);
    return {
        fieldErrors,
...
export function createMarketParameters(form) {
    return {
...
export function createSecurityPoolParameters(form) {
    ...
    if (statoblastSecurityMultiplierBps <= 10001n)
        throw new Error('Statoblast security multiplier must be at least 1.0002');
    ...
    if (initialReportPriorityFeeAttoEthPerGas <= 0n)
        throw new Error('Initial report priority fee must be greater than 0');
    return {
...
//# sourceMappingURL=marketCreation.js.map