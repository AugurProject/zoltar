...
async function loadTruthAuctionActiveTickPages(client, truthAuctionAddress, pageCount) {
    const tickSummaries = [];
    let tickCount = 0n;
    for (let pageIndex = 0; pageIndex < pageCount; pageIndex += 1) {
        ...
        if (BigInt(tickSummaries.length) >= tickCount || page.ticks.length === 0)
            break;
    }
    return {
        tickCount,
...
async function loadTruthAuctionTickBidPages(client, truthAuctionAddress, tick, pageCount) {
    const bids = [];
    let bidCount = 0n;
    for (let pageIndex = 0; pageIndex < pageCount; pageIndex += 1) {
        ...
        bids.push(...page.bids);
        if (BigInt(bids.length) >= bidCount || page.bids.length === 0)
            break;
    }
    return {
        bidCount,
...
async function loadTruthAuctionBidderBidPages(client, truthAuctionAddress, bidder, pageCount) {
    const bids = [];
    let bidCount = 0n;
    for (let pageIndex = 0; pageIndex < pageCount; pageIndex += 1) {
...
async function loadAggregatedTruthAuctionBidPages(client, truthAuctionAddress, tickSummaries, pageCount) {
...
export function useTruthAuctionBookData({ accountAddress, enteredBidTick, forkAuctionResultHash, selectedStage, shouldShowTruthAuctionVisualization, truthAuctionAddress, truthAuctionClearingTick, truthAuctionReadClient }) {
    const [truthAuctionBookData, setTruthAuctionBookData] = useState({
        tickSummaries: [],
        tickCount: 0n,
        viewerBids: [],
        viewerBidCount: 0n,
    });
    const [selectedBookTick, setSelectedBookTick] = useState(undefined);
    ...
    const [aggregatedAuctionBids, setAggregatedAuctionBids] = useState([]);
    const [aggregatedAuctionBidCountForLoadedTicks, setAggregatedAuctionBidCountForLoadedTicks] = useState(0n);
    const [tickDataKey, setTickDataKey] = useState(undefined);
    const [viewerDataKey, setViewerDataKey] = useState(undefined);
    const [aggregatedDataKey, setAggregatedDataKey] = useState(undefined);
    ...
    const [truthAuctionLevelsError, setTruthAuctionLevelsError] = useState(undefined);
    const [truthAuctionLevelsErrorKey, setTruthAuctionLevelsErrorKey] = useState(undefined);
    const [viewerTruthAuctionBidsError, setViewerTruthAuctionBidsError] = useState(undefined);
    const [viewerTruthAuctionBidsErrorKey, setViewerTruthAuctionBidsErrorKey] = useState(undefined);
    const [aggregatedAuctionBidsError, setAggregatedAuctionBidsError] = useState(undefined);
    const [aggregatedAuctionBidsErrorKey, setAggregatedAuctionBidsErrorKey] = useState(undefined);
    ...
    const scopedTruthAuctionBookData = {
    ...
    const selectTruthAuctionTick = (tick) => {
    ...
    useEffect(() => {
        ...
        void loadTruthAuctionActiveTickPages(client, truthAuctionAddress, loadedTickPageCount)
            ...
            setSelectedBookTick(currentSelection => {
                if (currentSelection !== undefined && sortedTickSummaries.some(tickSummary => tickSummary.tick === currentSelection))
                    return currentSelection;
                if (enteredBidTick !== undefined && sortedTickSummaries.some(tickSummary => tickSummary.tick === enteredBidTick))
                    return enteredBidTick;
                if (truthAuctionClearingTick !== undefined && sortedTickSummaries.some(tickSummary => tickSummary.tick === truthAuctionClearingTick))
                    return truthAuctionClearingTick;
...
//# sourceMappingURL=useTruthAuctionBookData.js.map