...
import { getCurrentSelectedPoolForkAuctionDetails, shouldReloadSelectedPoolDetails } from '../../security-pools/lib/securityPoolWorkflow.js';
export function useSelectedAuctionReadState({ accountAddress, currentSelectedOutcomePool, forkAuctionResultHash, forkMigrationReadClient, fullTruthAuctionReadClient, securityPoolAddress, selectedAuctionLabel, selectedOutcome, selectedOutcomeMigrationChildPool, selectedPoolRefreshNonce, selectedStage, universeId, }) {
    const [selectedAuctionDetails, setSelectedAuctionDetails] = useState(undefined);
    const [selectedAuctionError, setSelectedAuctionError] = useState(undefined);
    const [selectedAuctionErrorAddress, setSelectedAuctionErrorAddress] = useState(undefined);
    ...
    const [recoveredSelectedAuctionChildPool, setRecoveredSelectedAuctionChildPool] = useState(undefined);
    const [selectedAuctionChildPoolRecoveryCompletedKey, setSelectedAuctionChildPoolRecoveryCompletedKey] = useState(undefined);
    const [selectedAuctionChildPoolRecoveryError, setSelectedAuctionChildPoolRecoveryError] = useState(undefined);
    const [selectedAuctionChildPoolRecoveryErrorKey, setSelectedAuctionChildPoolRecoveryErrorKey] = useState(undefined);
    ...
    const [selectedOutcomeMigrationSeedStatus, setSelectedOutcomeMigrationSeedStatus] = useState(undefined);
    const [selectedOutcomeMigrationSeedStatusError, setSelectedOutcomeMigrationSeedStatusError] = useState(undefined);
    ...
    const currentRecoveredSelectedAuctionChildPool = recoveredSelectedAuctionChildPool !== undefined && securityPoolAddress !== undefined && sameAddress(recoveredSelectedAuctionChildPool.parent, securityPoolAddress) && recoveredSelectedAuctionChildPool.questionOutcome === selectedOutcome ? recoveredSelectedAuctionChildPool : undefined;
    ...
    const retrySelectedAuctionDetails = () => {
        if (selectedAuctionPoolAddress === undefined)
            return;
        ...
        void loadForkAuctionDetails(fullTruthAuctionReadClient ?? createConnectedReadClient(), selectedAuctionPoolAddress)
            .then(details => {
            if (requestGeneration !== selectedAuctionRequestGenerationRef.current)
                return;
    ...
    useEffect(() => {
        if (!retryingSelectedAuctionDetails || loadingSelectedAuctionDetails)
            return;
        setRetryingSelectedAuctionDetails(false);
    ...
    useEffect(() => {
        ...
        void loadForkAuctionDetails(client, selectedAuctionPoolAddress)
            .then(details => {
            if (cancelled || requestGeneration !== selectedAuctionRequestGenerationRef.current)
                return;
            setLoadingSelectedAuctionDetails(false);
            setRetryingSelectedAuctionDetails(false);
            setSelectedAuctionDetails(details);
    ...
    return {
        loadingSelectedAuctionChildPoolRecovery: (selectedStage === 'auction' || selectedStage === 'settlement') && selectedAuctionChildPoolRecoveryKey !== undefined && selectedAuctionChildPool === undefined && scopedSelectedAuctionChildPoolRecoveryError === undefined && selectedAuctionChildPoolRecoveryCompletedKey !== selectedAuctionChildPoolRecoveryKey,
...
//# sourceMappingURL=useSelectedAuctionReadState.js.map