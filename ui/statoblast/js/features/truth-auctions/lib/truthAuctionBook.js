...
export function estimateRepPurchased(bidAmountAttoEth, price) {
    if (bidAmountAttoEth <= 0n || price <= 0n)
        return 0n;
...
function findUnderfundedWinningAttoEth(tickSummaries, maxAttoRepBeingSold) {
    if (maxAttoRepBeingSold <= 0n)
        return 0n;
    ...
    for (const tickSummary of tickSummaries) {
        ...
        if (thresholdPrice > tickSummary.price)
            break;
        winningAttoEth = candidateWinningAttoEth;
    }
    return winningAttoEth;
}
function assertValidUnderfundedTruthAuctionMetrics(truthAuction) {
...
export function getTruthAuctionWinningThresholdPrice(truthAuction) {
    if (truthAuction === undefined || !truthAuction.finalized || !truthAuction.underfunded)
        return undefined;
    assertValidUnderfundedTruthAuctionMetrics(truthAuction);
    if (truthAuction.underfundedWinningAttoEth === 0n)
        return undefined;
    return truthAuction.underfundedThreshold;
}
function isFinalizedUnderfundedWithoutWinningPrefix(truthAuction) {
    return truthAuction.finalized && truthAuction.underfunded && truthAuction.underfundedWinningAttoEth === 0n;
}
function isUnderfundedWinningTick(tick, truthAuction) {
...
function getTruthAuctionTickDisposition(tickSummary, truthAuction) {
    if (tickSummary.currentTotalBidAttoEth === 0n)
    ...
    if (winningThresholdPrice !== undefined)
        return isUnderfundedWinningTick(tickSummary.tick, truthAuction) ? { label: 'Winning', tone: 'success' } : { label: 'Out', tone: 'danger' };
    if (isFinalizedUnderfundedWithoutWinningPrefix(truthAuction))
        return { label: 'Out', tone: 'danger' };
    if (!truthAuction.hitCap || truthAuction.clearingTick === undefined || truthAuction.clearingPrice === undefined)
        return truthAuction.finalized ? { label: 'Winning', tone: 'success' } : { label: 'In Book', tone: 'default' };
    if (tickSummary.tick > truthAuction.clearingTick)
        return { label: truthAuction.finalized ? 'Winning' : 'Above Clearing', tone: 'success' };
    if (tickSummary.tick < truthAuction.clearingTick)
        return { label: truthAuction.finalized ? 'Out' : 'Below Clearing', tone: 'danger' };
    return { label: truthAuction.finalized ? 'Clearing' : 'At Clearing', tone: 'warning' };
}
export function getTruthAuctionBidDisposition(bid, truthAuction) {
    if (bid.refunded)
    ...
    if (truthAuction === undefined) {
        if (bid.claimed)
            return { label: 'Claimed', tone: 'success', canPrefillRefund: false, canPrefillSettle: false, settlementKind: 'none', summaryKind: 'neutral' };
    ...
    if (winningThresholdPrice !== undefined) {
        if (isUnderfundedWinningTick(bid.tick, truthAuction)) {
            if (truthAuction.finalized) {
                if (bid.claimed)
                    return { label: 'Claimed', tone: 'success', canPrefillRefund: false, canPrefillSettle: false, settlementKind: 'repClaim', summaryKind: 'neutral' };
...
export function getTruthAuctionBidSettlementEstimate(bid, truthAuction) {
    if (truthAuction === undefined) {
        return {
...
export function getTruthAuctionDispositionClassName(tone) {
    switch (tone) {
        case 'danger':
            return 'is-danger';
        case 'success':
...
export function sortTruthAuctionTickSummariesDescending(tickSummaries) {
...
export function buildTruthAuctionDepthPoints({ enteredBidTick, selectedBookTick, tickSummaries, truthAuction }) {
...
export function getTruthAuctionOverviewProgress(truthAuction, tickSummaries) {
    if (truthAuction === undefined)
        return undefined;
    if (truthAuction.finalized) {
        return {
    ...
    if (!truthAuction.hitCap || truthAuction.clearingTick === undefined || truthAuction.clearingPrice === undefined) {
        ...
        for (const tickSummary of activeTickSummaries) {
            ...
            if (tickSummary.tick > truthAuction.clearingTick)
                acceptedAttoEth = tickSummary.currentTotalBidAttoEth;
            else if (tickSummary.tick === truthAuction.clearingTick)
                acceptedAttoEth = truthAuction.bidAtClearingTickAttoEth < tickSummary.currentTotalBidAttoEth ? truthAuction.bidAtClearingTickAttoEth : tickSummary.currentTotalBidAttoEth;
            if (acceptedAttoEth <= 0n)
                continue;
            if (acceptedAttoEth > remainingCap)
                acceptedAttoEth = remainingCap;
...
export function sortTruthAuctionBidsByPriority(bids) {
    return [...bids].sort((left, right) => {
        if (left.tick !== right.tick)
            return left.tick > right.tick ? -1 : 1;
        if (left.bidIndex !== right.bidIndex)
            return left.bidIndex < right.bidIndex ? -1 : 1;
        return 0;
    });
}
function normalizeTruthAuctionPriceInput(value) {
    if (value.startsWith('.'))
        return `0${value}`;
    if (value.endsWith('.'))
        return `${value}0`;
...
function formatTruthAuctionValidationPrice(price) {
...
function isTruthAuctionPriceInputDefinitelyOutOfRange(input) {
    ...
    if (fractionalPart.length > 18)
        return false;
    if (wholePart.length !== TRUTH_AUCTION_MAX_PRICE_WHOLE.length)
        return wholePart.length > TRUTH_AUCTION_MAX_PRICE_WHOLE.length;
    if (wholePart !== TRUTH_AUCTION_MAX_PRICE_WHOLE)
        return wholePart > TRUTH_AUCTION_MAX_PRICE_WHOLE;
    return fractionalPart.padEnd(18, '0') > TRUTH_AUCTION_MAX_PRICE_FRACTION;
}
export function getTruthAuctionBidPreview(submitBidPriceInput) {
    if (submitBidPriceInput.trim() === '')
        return undefined;
    if (isTruthAuctionPriceInputDefinitelyOutOfRange(submitBidPriceInput))
        return undefined;
    const enteredBidPrice = tryParseTruthAuctionPriceInput(submitBidPriceInput);
    if (enteredBidPrice === undefined || enteredBidPrice <= 0n)
        return undefined;
    const enteredBidTick = getTruthAuctionTickAtPrice(enteredBidPrice);
    if (enteredBidTick === undefined)
        return undefined;
    return {
...
export function getTruthAuctionBidPriceValidationMessage(submitBidPriceInput) {
    if (submitBidPriceInput.trim() === '')
        return 'Enter a bid price greater than zero.';
    if (isTruthAuctionPriceInputDefinitelyOutOfRange(submitBidPriceInput))
        return 'Bid price is outside the supported auction range.';
    const enteredBidPrice = tryParseTruthAuctionPriceInput(submitBidPriceInput);
    if (enteredBidPrice === undefined)
        return 'Enter a valid bid price.';
    if (enteredBidPrice <= 0n)
        return 'Enter a bid price greater than zero.';
    if (getTruthAuctionTickAtPrice(enteredBidPrice) === undefined)
        return 'Bid price is outside the supported auction range.';
    return undefined;
}
export function getTruthAuctionBidGuardMessage({ accountAddress, currentTimestamp, isOnActiveAppChain, submitBidAmountInput, truthAuction, walletBalanceAttoEth, }) {
    ...
    if (truthAuction.finalized)
        return 'Truth auction is already finalized.';
    ...
    if (auctionHasEndedByTimestamp || truthAuction.timeRemaining === 0n)
        return 'Truth auction has ended.';
    ...
    if (bidAmount <= 0n)
        return 'Enter a bid amount greater than zero.';
    if (bidAmount < truthAuction.minBidSizeAttoEth)
        return `Bid must be at least ${formatCurrencyBalance(truthAuction.minBidSizeAttoEth)} ETH.`;
    if (walletBalanceAttoEth === undefined)
        return 'Loading wallet ETH balance.';
    if (bidAmount > walletBalanceAttoEth)
        return `Need ${formatCurrencyBalance(bidAmount - walletBalanceAttoEth)} more ETH in this wallet to bid the selected amount.`;
    return undefined;
}
//# sourceMappingURL=truthAuctionBook.js.map