...
import { loadAllSecurityPools, loadCoordinatorInitialReportFundingRequirement, loadLiquidationApproval as loadProtocolLiquidationApproval, loadOracleManagerDetails, loadOracleManagerQueueOperationEthValue, loadSecurityPoolPage, loadSecurityPoolVaultSummary as loadProtocolSecurityPoolVaultSummary, queueSecurityPoolLiquidation, } from '../../../protocol/index.js';
...
export function shouldFallbackToAllSecurityPoolsPage(error) {
...
export function createSecurityPoolPageFromLoadedPools(pools, pageIndex, pageSize) {
...
function getLiquidationFundingPreviewRequestKey(managerAddress, walletAddress, environmentRefreshKey) {
...
const defaultUseSecurityPoolsOverviewDependencies = {
...
function useSecurityPoolsOverviewWithDependencies({ accountAddress, environmentRefreshKey, onTransactionCanceled, onTransactionFailed, onTransactionFinished, onTransactionPresented, onTransactionPrepared, onTransactionRequested, onTransactionSubmitted, refreshState }, dependencies) {
    ...
    const maximumLiquidationDebtAttoEth = useSignal(undefined);
    ...
    const liquidationApprovalDetails = useSignal(undefined);
    const liquidationApprovalError = useSignal(undefined);
    const liquidationApprovalLoadingKey = useSignal(undefined);
    const liquidationReceiverVaultSummary = useSignal(undefined);
    const liquidationReceiverVaultSummaryError = useSignal(undefined);
    const liquidationReceiverVaultSummaryResolvedKey = useSignal(undefined);
    const liquidationReceiverVaultSummaryLoadingKey = useSignal(undefined);
    const liquidationTimeoutMinutes = useSignal(DEFAULT_STAGED_OPERATION_TIMEOUT_MINUTES.toString());
    const liquidationManagerAddress = useSignal(undefined);
    const liquidationFundingPreview = useSignal(undefined);
    const liquidationFundingPreviewError = useSignal(undefined);
    const liquidationFundingPreviewErrorKey = useSignal(undefined);
    const liquidationFundingPreviewLoadingKey = useSignal(undefined);
    const liquidationFundingPreviewResolvedKey = useSignal(undefined);
    const liquidationSecurityPoolAddress = useSignal(undefined);
    const liquidationModalOpen = useSignal(false);
    const securityPoolBrowseCount = useSignal(undefined);
    const securityPoolPage = useSignal(undefined);
    ...
    const securityPoolsLoadedEnvironmentRefreshKey = useSignal(undefined);
    const hasLoadedSecurityPoolPage = useSignal(false);
    const checkedSecurityPoolAddress = useSignal(undefined);
    const securityPoolOverviewActiveAction = useSignal(undefined);
    const securityPoolOverviewFeedback = useSignal(undefined);
    const securityPoolOverviewError = useSignal(undefined);
    const securityPoolsLoadError = useSignal(undefined);
    const securityPoolsLoadErrorEnvironmentRefreshKey = useSignal(undefined);
    const securityPoolLiquidationError = useSignal(undefined);
    const securityPoolOverviewResult = useSignal(undefined);
    ...
    const loadSecurityPools = async (securityPoolAddress) => {
        ...
        const result = await securityPoolsLoad.run({
            ...
            load: async () => {
                ...
                const loadOptions = nextCheckedAddress === undefined
                    ? {
    ...
    const loadBrowseSecurityPoolPage = async (pageIndex, pageSize, requestKey) => {
        ...
        await securityPoolPageLoad.run({
            ...
            load: async () => {
                ...
                try {
                    ...
                    if (!shouldFallbackToAllSecurityPoolsPage(error))
                        throw error;
    ...
    const resolveLiquidationFundingPreview = async (managerAddress, walletAddress) => {
    ...
    const loadLiquidationFundingPreview = async (managerAddress) => {
        ...
        const result = await liquidationFundingPreviewLoad.run({
            ...
            onSuccess: preview => {
                if (getCurrentLiquidationFundingPreviewRequestKey() !== requestKey)
                    return;
        ...
        if (liquidationFundingPreviewLoadingKey.value === requestKey)
    ...
    const getCurrentLiquidationApprovalRequestKey = () => {
        ...
        if (!/^0x[0-9a-fA-F]{64}$/.test(approvalId))
            return undefined;
    ...
    const loadLiquidationApproval = async () => {
        ...
        const result = await liquidationApprovalLoad.run({
            ...
            onSuccess: approval => {
                if (getCurrentLiquidationApprovalRequestKey() !== requestKey)
                    return;
        ...
        if (liquidationApprovalLoadingKey.value === requestKey)
    ...
    const loadLiquidationReceiverVaultSummary = async () => {
        ...
        const result = await liquidationReceiverVaultSummaryLoad.run({
            ...
            onSuccess: summary => {
                if (getCurrentLiquidationReceiverVaultSummaryRequestKey() !== requestKey)
                    return;
        ...
        if (liquidationReceiverVaultSummaryLoadingKey.value === requestKey)
    ...
    const openLiquidationModal = (managerAddress, securityPoolAddress, vaultAddress, maxAmount) => {
    ...
    const getLiquidationSubmittedFeedback = (hash) => createSuccessActionFeedback('queueLiquidation', 'Liquidation submitted', hash, 'Waiting for refreshed pool state.');
    const getLiquidationFeedbackFromResult = (result) => {
        if (result.stagedExecution?.success === false)
            return createErrorActionFeedback('queueLiquidation', 'Liquidation failed', getLiquidationExecutionFailureDetail(result.stagedExecution.errorMessage) ?? 'The liquidation execution failed.');
    ...
    const isLiquidationSnapshotCurrent = (snapshot) => liquidationApprovalId.value === snapshot.approvalId &&
    ...
    const queueLiquidation = async (managerAddress, securityPoolAddress) => {
        securityPoolLiquidationError.value = undefined;
        securityPoolOverviewResult.value = undefined;
        ...
        try {
            ...
            await runWriteAction({
                ...buildWriteActionConfig({ accountAddress, onTransactionCanceled, onTransactionFailed, onTransactionFinished, onTransactionPresented, onTransactionPrepared, onTransactionRequested, refreshState }, securityPoolOverviewError, 'Connect a wallet before queueing liquidation', createLiquidationTransactionIntent(transactionContext)),
                onRefreshError: (message, hash) => {
                    if (completedResult?.stagedExecution?.success === false)
                        return;
                    ...
                    if (completedResult !== undefined)
                        onTransactionPresented(createLiquidationWarningPresentation(completedResult, message, transactionContext));
                },
                ...
                if (fundingPreview.currentRepBalanceAttoRep < fundingPreview.initialReportRepRequiredAttoRep)
                    throw new Error(`Need ${formatCurrencyBalance(fundingPreview.initialReportRepRequiredAttoRep - fundingPreview.currentRepBalanceAttoRep)} more REP in this wallet to fund the initial report.`);
                ...
                if (walletBalanceAttoEth !== undefined && walletBalanceAttoEth < fundingPreview.totalWalletEthRequiredAttoEth)
                    throw new Error(`Need ${formatCurrencyBalance(fundingPreview.totalWalletEthRequiredAttoEth - walletBalanceAttoEth)} more ETH in this wallet to fund the initial report and queue this liquidation.`);
                const timeoutMinutes = parseBigIntInput(submittedLiquidation.timeoutMinutes, 'Liquidation timeout');
                if (timeoutMinutes < MIN_STAGED_OPERATION_TIMEOUT_MINUTES)
                    throw new Error('Liquidation timeout must be at least 1 minute');
                if (timeoutMinutes > MAX_STAGED_OPERATION_TIMEOUT_MINUTES)
                    throw new Error('Liquidation timeout must be 5 minutes or less');
            ...
            }, 'Failed to queue liquidation', async (result) => {
                const nextResult = {
    ...
    return {
        ...
        setLiquidationTimeoutMinutes: (value) => {
            liquidationTimeoutMinutes.value = value;
        },
        setLiquidationTargetVault: (value) => {
            liquidationTargetVault.value = value;
        },
        setLiquidationReceiverVault: (value) => {
            nextLiquidationReceiverVaultSummaryLoad();
        ...
        setLiquidationApprovalId: (value) => {
            nextLiquidationApprovalLoad();
...
export function useSecurityPoolsOverview(parameters, dependencies) {
    if (dependencies === undefined)
        return useSecurityPoolsOverviewWithDependencies(parameters, defaultUseSecurityPoolsOverviewDependencies);
    return useSecurityPoolsOverviewWithDependencies(parameters, dependencies);
}
//# sourceMappingURL=useSecurityPoolsOverview.js.map