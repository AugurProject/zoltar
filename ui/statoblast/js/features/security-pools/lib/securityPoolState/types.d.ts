...
export type SecurityPoolActionId = 'approveRep' | 'depositRepToVault' | 'queueWithdrawRep' | 'redeemRepFromVault' | 'redeemFees' | 'createCompleteSet' | 'redeemCompleteSet' | 'migrateShares' | 'redeemShares' | 'reportOutcome' | 'withdrawEscalation' | 'requestPrice' | 'executeStagedOperation' | 'queueLiquidation' | 'forkWithOwnEscalation' | 'initiateFork' | 'forkUniverse' | 'createChildUniverse' | 'migrateRepToZoltar' | 'migrateVault' | 'claimParentEscalationDeposits' | 'migrateUnresolvedEscalation' | 'startTruthAuction' | 'submitBid' | 'finalizeTruthAuction' | 'refundLosingBids' | 'claimAuctionProceeds' | 'settleForkedEscalation';
