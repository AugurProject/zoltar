...
export function parseTargetHealthFactorBps(value) {
    const trimmed = value.trim();
    if (!/^\d+(?:\.\d{1,4})?$/.test(trimmed))
        throw new Error('Target health factor must be a number with at most four decimal places');
    const [whole = '', fraction = ''] = trimmed.split('.');
    const factorBps = BigInt(whole) * BPS_DENOMINATOR + BigInt(fraction.padEnd(4, '0'));
    if (factorBps < BPS_DENOMINATOR)
        throw new Error('Target health factor must be at least 1.00×');
    return factorBps;
}
function getMigrationSecurityMultiplierBps(poolSecurityMultiplierBps) {
    return BPS_DENOMINATOR + (poolSecurityMultiplierBps - BPS_DENOMINATOR) / 2n;
}
export function getSelectedVaultOwner(selectedVaultOwner, accountAddress) {
    const trimmedSelectedVaultOwner = selectedVaultOwner?.trim() ?? '';
    if (trimmedSelectedVaultOwner !== '')
        return trimmedSelectedVaultOwner;
    return accountAddress?.toString();
}
export function isSelectedVaultOwnedByAccount(selectedVaultOwner, accountAddress) {
    const trimmedSelectedVaultOwner = selectedVaultOwner?.trim() ?? '';
    if (trimmedSelectedVaultOwner === '' || accountAddress === undefined)
        return false;
    return sameAddress(trimmedSelectedVaultOwner, accountAddress);
}
export function doesLoadedSecurityVaultMatchSelection({ accountAddress, securityPoolAddress, securityVaultDetails, selectedVaultOwner }) {
    if (securityVaultDetails === undefined)
        return false;
    const effectiveSelectedVaultOwner = getSelectedVaultOwner(selectedVaultOwner, accountAddress);
    if (effectiveSelectedVaultOwner === undefined)
        return false;
...
export function isSecurityVaultDepositBelowMinimum(currentVaultRepBackingAttoRep, depositAmount, minimumVaultRepDepositAttoRep = MIN_SECURITY_VAULT_REP_DEPOSIT_ATTO_REP) {
...
export function doesSecurityVaultExistOnchain(securityVaultDetails) {
    if (securityVaultDetails === undefined)
        return false;
...
function divideBigintRoundUp(value, divisor) {
    if (divisor <= 0n)
        throw new Error('Divisor must be greater than zero');
    return (value + divisor - 1n) / divisor;
}
function getCapacityOwnershipBackedRepFloor(capacityOwnershipAttoRep, repPerEthPrice, statoblastSecurityMultiplierBps) {
    if (capacityOwnershipAttoRep === undefined || capacityOwnershipAttoRep <= 0n)
        return 0n;
    if (repPerEthPrice === undefined || repPerEthPrice <= 0n)
        return undefined;
    if (statoblastSecurityMultiplierBps === undefined || statoblastSecurityMultiplierBps <= 0n)
        return undefined;
...
function getStrictCapacityOwnershipBackedRepMinimum(capacityOwnershipAttoRep, repPerEthPrice, multiplierBps) {
    if (capacityOwnershipAttoRep === undefined || capacityOwnershipAttoRep <= 0n)
        return 0n;
    if (repPerEthPrice === undefined || repPerEthPrice <= 0n)
        return undefined;
    if (multiplierBps === undefined || multiplierBps <= 0n)
        return undefined;
    return (capacityOwnershipAttoRep * repPerEthPrice * multiplierBps) / (PRICE_PRECISION * BPS_DENOMINATOR) + 1n;
}
function getBackedCapacityOwnershipCeiling(attoRepAmount, repPerEthPrice, statoblastSecurityMultiplierBps) {
    if (attoRepAmount === undefined || attoRepAmount <= 0n)
        return 0n;
    if (repPerEthPrice === undefined || repPerEthPrice <= 0n)
        return 0n;
    if (statoblastSecurityMultiplierBps === undefined || statoblastSecurityMultiplierBps <= 0n)
...
function getStrictlyBackedCapacityOwnershipCeiling(attoRepAmount, repPerEthPrice, multiplierBps) {
    if (attoRepAmount === undefined || attoRepAmount <= 0n || repPerEthPrice === undefined || repPerEthPrice <= 0n || multiplierBps === undefined || multiplierBps <= 0n)
        return 0n;
...
export function getSecurityVaultWithdrawableRepAmount({ disputeStakedAttoRep = 0n, vaultAttoRepBacking, repPerEthPrice, capacityOwnershipAttoRep, statoblastSecurityMultiplierBps, totalPoolHeldAttoRep, totalCapacityOwnershipAttoRep, }) {
    if (vaultAttoRepBacking === undefined)
        return undefined;
    if (disputeStakedAttoRep > 0n)
        return 0n;
    ...
    if (requiredVaultAttoRep === undefined)
        return undefined;
    ...
    if (migrationRequiredRep === undefined)
        return undefined;
    ...
    if (migrationHeadroom < maxWithdrawableAttoRep)
        maxWithdrawableAttoRep = migrationHeadroom;
...
export function getSecurityVaultMaxCapacityOwnershipAttoRepAmount({ currentCapacityOwnershipAttoRep, disputeStakedAttoRep = 0n, vaultAttoRepBacking, repPerEthPrice, statoblastSecurityMultiplierBps, totalPoolHeldAttoRep, totalCapacityOwnershipAttoRep, }) {
    ...
    if (migrationCapacityOwnershipCeilingAttoRep < maxCapacityOwnershipAttoRepAmount)
        maxCapacityOwnershipAttoRepAmount = migrationCapacityOwnershipCeilingAttoRep;
...
export function getStagedOperationTimeoutSeconds(timeoutMinutes) {
    if (timeoutMinutes === undefined || timeoutMinutes < MIN_STAGED_OPERATION_TIMEOUT_MINUTES)
        return undefined;
    return timeoutMinutes * 60n;
}
export function hasValidSecurityVaultOraclePrice(managerAddress, oracleManagerDetails, currentTimestamp) {
    if (managerAddress === undefined || oracleManagerDetails === undefined)
        return false;
    if (!sameAddress(managerAddress, oracleManagerDetails.managerAddress))
        return false;
    return isOracleManagerPriceUsable(oracleManagerDetails, currentTimestamp);
}
export function isOracleManagerPriceUsable(oracleManagerDetails, currentTimestamp) {
    if (oracleManagerDetails?.isPriceValid !== true)
        return false;
