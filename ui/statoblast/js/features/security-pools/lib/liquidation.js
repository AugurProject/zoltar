...
function getVaultOpenInterestAttoEth(vault) {
    return vault.openInterestAttoEth;
}
function requireVaultOpenInterestAttoEth(vault) {
    const openInterestAttoEth = getVaultOpenInterestAttoEth(vault);
    if (openInterestAttoEth === undefined)
        throw new Error('Vault live open interest is still loading');
    return openInterestAttoEth;
}
function requireVaultBadDebtAttoEth(vault) {
    if (vault.badDebtAttoEth === undefined)
        throw new Error('Vault bad debt is still loading');
    return vault.badDebtAttoEth;
}
function calculateVaultOpenInterestAttoEth(settlementCollateralAttoEth, capacityOwnershipAttoRep, totalCapacityOwnershipAttoRep) {
    if (capacityOwnershipAttoRep === 0n || totalCapacityOwnershipAttoRep === 0n)
        return 0n;
...
function calculateLiveVaultOpenInterestAfterOwnershipChange({ capacityOwnershipAfterAttoRep, totalCapacityOwnershipAttoRep, settlementCollateralAttoEth, vaultBadDebtAttoEth, vaultBadDebtIncreaseAttoEth = 0n, }) {
...
function mulDivCeil(value, multiplier, denominator) {
    const product = value * multiplier;
    return product === 0n ? 0n : (product - 1n) / denominator + 1n;
}
export function isVaultHealthyAtFactor({ disputeStakedAttoRep = 0n, healthFactorBps, openInterestAttoEth, poolHeldVaultRepBackingAttoRep, poolSecurityMultiplierBps, repPerEthPrice, }) {
    if (healthFactorBps < LIQUIDATION_BPS_DENOMINATOR)
        return false;
    if (openInterestAttoEth === 0n)
        return true;
    ...
    if (poolHeldVaultRepBackingAttoRep + disputeStakedAttoRep < associatedRequiredAttoRep)
        return false;
...
function isVaultLiquidatable(lastPrice, openInterestAttoEth, vaultAttoRepBacking, disputeStakedAttoRep, statoblastSecurityMultiplierBps) {
    if (lastPrice === undefined || openInterestAttoEth === undefined || vaultAttoRepBacking === undefined || statoblastSecurityMultiplierBps === undefined)
        return false;
...
function getPartialLiquidationTransfer(debtMovedAttoEth, targetVaultSummary, repPerEthPrice) {
    ...
    if (repBackingUnits === undefined || totalRepBackingUnits === undefined || totalPoolHeldRepBalanceAttoRep === undefined) {
        if (quotedRep >= targetVaultSummary.vaultAttoRepBacking)
            return { remainingAttoRep: 0n, vaultAttoRepBackingToTransfer: targetVaultSummary.vaultAttoRepBacking };
    ...
    if (backingUnitsToTransfer >= repBackingUnits)
        return { remainingAttoRep: 0n, vaultAttoRepBackingToTransfer: targetVaultSummary.vaultAttoRepBacking };
...
function getAwardableLiquidationRep(targetVaultSummary, reserveMinimumRep, minimumVaultRepDepositAttoRep) {
    const { repBackingUnits, totalRepBackingUnits, totalPoolHeldRepBalanceAttoRep } = targetVaultSummary;
    if (repBackingUnits === undefined || totalRepBackingUnits === undefined || totalPoolHeldRepBalanceAttoRep === undefined) {
        if (!reserveMinimumRep)
            return targetVaultSummary.vaultAttoRepBacking;
    ...
    if (reserveBackingUnits >= repBackingUnits)
        return 0n;
...
function getFundedLiquidationAmounts(requestedDebtAttoEth, targetVaultSummary, receiverVaultSummary, repPerEthPrice, minimumVaultRepDepositAttoRep, settlementCollateralAttoEth, totalCapacityOwnershipAttoRep) {
    ...
    if (targetOpenInterestAttoEth === 0n)
        return { badDebtAttoEth: 0n, capacityOwnershipMovedAttoRep: 0n, debtMovedAttoEth: 0n };
    ...
    if (capacityOwnershipMovedAttoRep === 0n)
        return { badDebtAttoEth: 0n, capacityOwnershipMovedAttoRep: 0n, debtMovedAttoEth: 0n };
    ...
    if (debtMovedAttoEth === 0n || debtMovedAttoEth > nominalDebtToMoveAttoEth)
        return { badDebtAttoEth: 0n, capacityOwnershipMovedAttoRep: 0n, debtMovedAttoEth: 0n };
...
export function getLiquidationExecutionFailureDetail(errorMessage) {
    switch (errorMessage) {
        case 'Target safe':
...
export function getMaxLiquidationAmount({ repPerEthPrice, statoblastSecurityMultiplierBps, targetVaultSummary }) {
    if (repPerEthPrice === undefined || statoblastSecurityMultiplierBps === undefined || targetVaultSummary === undefined)
        return undefined;
    if (repPerEthPrice <= 0n || statoblastSecurityMultiplierBps <= 0n)
        return 0n;
    ...
    if (targetOpenInterestAttoEth === undefined)
        return undefined;
    if (targetOpenInterestAttoEth === 0n)
        return 0n;
    if (!isVaultLiquidatable(repPerEthPrice, targetOpenInterestAttoEth, targetRepDeposit, targetVaultSummary.disputeStakedAttoRep, statoblastSecurityMultiplierBps))
        return 0n;
    return targetOpenInterestAttoEth;
}
export function simulateLiquidation({ callerVaultSummary, requestedDebtAttoEth, totalCapacityOwnershipAttoRep, minimumVaultRepDepositAttoRep = DEFAULT_MINIMUM_VAULT_REP_DEPOSIT_ATTO_REP, repPerEthPrice, settlementCollateralAttoEth, statoblastSecurityMultiplierBps, targetVaultSummary, }) {
    ...
    const targetCapacityOwnershipAttoRep = targetVaultSummary.capacityOwnershipAttoRep;
    const targetOpenInterestAttoEth = requireVaultOpenInterestAttoEth(targetVaultSummary);
    const maxLiquidationDebtAttoEth = getMaxLiquidationAmount({
        repPerEthPrice,
        statoblastSecurityMultiplierBps,
        targetVaultSummary,
    }) ?? targetOpenInterestAttoEth;
    const { badDebtAttoEth, capacityOwnershipMovedAttoRep, debtMovedAttoEth } = getFundedLiquidationAmounts(requestedDebtAttoEth < maxLiquidationDebtAttoEth ? requestedDebtAttoEth : maxLiquidationDebtAttoEth, targetVaultSummary, callerVaultSummary, repPerEthPrice, minimumVaultRepDepositAttoRep, settlementCollateralAttoEth, totalCapacityOwnershipAttoRep);
...
export function getDeterministicLiquidationFailureReason({ callerVaultSummary, requestedDebtAttoEth, totalCapacityOwnershipAttoRep, maxLiquidationDebtAttoEth, minimumSecurityBondDebtAttoEth = DEFAULT_MINIMUM_SECURITY_BOND_DEBT_ATTO_ETH, minimumVaultRepDepositAttoRep = DEFAULT_MINIMUM_VAULT_REP_DEPOSIT_ATTO_REP, repPerEthPrice, settlementCollateralAttoEth, statoblastSecurityMultiplierBps, targetVaultSummary, }) {
    if (requestedDebtAttoEth === undefined)
        return 'Enter a valid liquidation amount.';
    if (requestedDebtAttoEth <= 0n)
        return 'Enter a liquidation amount greater than zero.';
    if (targetVaultSummary === undefined)
        return 'Target vault details are still loading.';
    const targetOpenInterestAttoEth = getVaultOpenInterestAttoEth(targetVaultSummary);
    if (targetOpenInterestAttoEth === undefined)
        return 'Target vault live open interest is still loading.';
    if (targetOpenInterestAttoEth === 0n)
        return 'This vault has no open interest to liquidate.';
    ...
    if (repPerEthPrice === undefined || settlementCollateralAttoEth === undefined || totalCapacityOwnershipAttoRep === undefined)
        return undefined;
    ...
    if ((debtMovedAttoEth <= 0n || capacityOwnershipMovedAttoRep === 0n) && badDebtAttoEth <= 0n)
        return liquidationCopy.executableCapacityOwnershipUnavailable;
    const vaultAttoRepBackingToTransfer = getPartialLiquidationTransfer(debtMovedAttoEth, targetVaultSummary, repPerEthPrice).vaultAttoRepBackingToTransfer;
    ...
    if (callerOpenInterestAttoEth === undefined)
        return 'Receiver vault live open interest is still loading.';
    const resultingReceiverDebtAttoEth = callerOpenInterestAttoEth + debtMovedAttoEth;
    if (remainingTargetDebtAttoEth !== 0n && targetAfterRepDeposit !== undefined && targetAfterRepDeposit < minimumVaultRepDepositAttoRep)
        return 'The target vault would fall below the minimum REP backing after liquidation.';
    if (remainingTargetDebtAttoEth !== 0n && remainingTargetDebtAttoEth < minimumSecurityBondDebtAttoEth)
        return 'The target vault would fall below the minimum security-bond debt after liquidation.';
    if (debtMovedAttoEth !== 0n && callerAfterRepDeposit < minimumVaultRepDepositAttoRep)
        return 'The receiver vault would remain below the minimum REP backing after liquidation.';
    if (debtMovedAttoEth !== 0n && resultingReceiverDebtAttoEth < minimumSecurityBondDebtAttoEth)
        return 'The selected receiver would remain below the minimum security-bond debt after liquidation.';
    if (debtMovedAttoEth !== 0n && resultingCallerCapacityOwnershipAttoRep === 0n)
        return 'No capacity ownership would move with the liquidation debt.';
    return undefined;
}
export function getLiquidationFailureReason({ callerVaultSummary, requestedDebtAttoEth, totalCapacityOwnershipAttoRep, minimumReceiverHealthFactorBps = LIQUIDATION_BPS_DENOMINATOR, minimumSecurityBondDebtAttoEth = DEFAULT_MINIMUM_SECURITY_BOND_DEBT_ATTO_ETH, minimumVaultRepDepositAttoRep = DEFAULT_MINIMUM_VAULT_REP_DEPOSIT_ATTO_REP, repPerEthPrice, settlementCollateralAttoEth, statoblastSecurityMultiplierBps, targetVaultSummary, }) {
    const targetOpenInterestAttoEth = targetVaultSummary === undefined ? undefined : getVaultOpenInterestAttoEth(targetVaultSummary);
    if (repPerEthPrice !== undefined &&
    ...
    if (deterministicFailureReason !== undefined)
        return deterministicFailureReason;
    if (requestedDebtAttoEth === undefined)
        return 'Enter a valid liquidation amount.';
    if (repPerEthPrice === undefined || statoblastSecurityMultiplierBps === undefined)
        return 'Refresh the Open Oracle before executing liquidation.';
    if (targetVaultSummary === undefined)
        return 'Target vault details are still loading.';
    ...
    if (!receiverHealthyAtRequiredFactor) {
        if (minimumReceiverHealthFactorBps > LIQUIDATION_BPS_DENOMINATOR)
            return liquidationCopy.receiverBelowApprovedHealthFactor;
        if (isVaultLiquidatable(repPerEthPrice, callerOpenInterestBeforeAttoEth, simulation.callerBefore.vaultAttoRepBacking, callerVaultSummary?.disputeStakedAttoRep, statoblastSecurityMultiplierBps))
            return 'The receiver vault would remain liquidatable after this liquidation.';
