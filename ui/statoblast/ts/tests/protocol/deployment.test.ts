...
describe('statoblast deployment steps', () => {
	test('pins every directly deployed Statoblast artifact to its expected runtime code hash', () => {
		expect(Object.keys(STATIC_STATOBLAST_DEPLOYMENT_ARTIFACT_RUNTIME_CODE_BY_STEP_ID).sort()).toEqual(['escalationGameClaimDelegate', 'openOracle', 'securityPoolUtils', 'uniformPriceDualCapBatchAuctionFactory'])
