...
function getTruthAuctionTickPricePower(index) {
    const power = TRUTH_AUCTION_TICK_PRICE_POWERS[index];
    if (power === undefined)
        throw new Error('Auction tick price power index is out of bounds');
    return power;
}
export function assertTruthAuctionTickInContractDomain(tick) {
    if (tick < TRUTH_AUCTION_MIN_TICK || tick > TRUTH_AUCTION_MAX_TICK)
        throw new Error('Truth auction tick is outside the supported range.');
}
export function tickToPrice(tick) {
    ...
    for (let bitIndex = 0; bitIndex < TRUTH_AUCTION_TICK_PRICE_POWERS.length; bitIndex += 1) {
        ...
        if ((absoluteTick & bitMask) !== 0n)
            price = (price * getTruthAuctionTickPricePower(bitIndex)) / TRUTH_AUCTION_PRICE_PRECISION;
    }
...
export function priceToClosestTick(price) {
    ...
    if (lowerBoundTick > TRUTH_AUCTION_MAX_TICK)
        return TRUTH_AUCTION_MAX_TICK;
    if (upperBoundTick < TRUTH_AUCTION_MIN_TICK)
        return TRUTH_AUCTION_MIN_TICK;
