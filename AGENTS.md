...
- When changing observable behavior or an API contract, search existing tests for assertions of both the old and intended behavior. Inspect every discovered test file that imports or exercises the changed API, and record whether it is affected; give a concrete scope-based reason for each excluded file. Update obsolete expectations while preserving their safety checks, and run the affected existing tests alongside new regression tests. Passing new tests alone does not establish compatibility with existing consumers.
