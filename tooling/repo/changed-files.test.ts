...
test('changed-files combines committed, staged, unstaged, and untracked paths', () => {
	...
	expect(changedFiles).toEqual(['scripts/NewScript.mts', 'shared/core/ts/Shared.ts', 'ui/zoltar/ts/components/Committed.tsx', 'ui/zoltar/ts/components/Staged.tsx', 'ui/zoltar/ts/components/Unstaged.tsx'])
})
...
test('test planning preserves deletions and rename source paths', () => {
	const changes = getChangedFileEntries(args => {
		...
		if (command === 'diff --name-status -z --find-renames --diff-filter=ACMRTUXBD baseline') return 'D\0shared/core/ts/deleted.ts\0R100\0ui/zoltar/ts/tests/old.test.ts\0ui/zoltar/ts/tests/new.test.ts\0'
	...
	expect(changes).toEqual([
		...
		{ path: 'shared/core/ts/deleted.ts', status: 'deleted' },
