...
function assertAuditFindingRemediations(): void {
	...
	assert.match(
		securityPoolForker,
		/function _getTruthAuctionCap\([\s\S]*Math\.ceilDiv\(data\.migratedAttoRep, SecurityPoolUtils\.MAX_AUCTION_VAULT_HAIRCUT_DIVISOR\)[\s\S]*Math\.mulDiv\(migratedPoolRepRetentionAttoRep, combinedAuctionableAttoRep, poolAuctionableRepAtForkAttoRep, Math\.Rounding\.Ceil\)[\s\S]*function _finalizeBackingUnitsAfterAuction\([\s\S]*uint256 incumbentRepAfterAttoRep = poolRepAfterAttoRep - repPurchasedAttoRep;[\s\S]*if \(incumbentRepAfterAttoRep == 0\)[\s\S]*auctionRepBackingUnits = Math\.mulDiv\(poolRepAfterAttoRep, SecurityPoolUtils\.PRICE_PRECISION, 1\);[\s\S]*Math\.mulDiv\(poolRepBeforeAttoRep, poolRepAfterAttoRep, incumbentRepAfterAttoRep, Math\.Rounding\.Ceil\)[\s\S]*auctionRepBackingUnits = totalRepBackingUnitsAtFinalization - poolRepBeforeAttoRep;/,
