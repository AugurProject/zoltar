...
const generatedPages = await generateReferencePages()
const renderedPages: Array<{ html: string; outputPath: string }> = []
for (const page of generatedPages) {
	assert.match(page.content, /^<!-- Generated by tooling\/docs\/generate-contract-interaction-reference\.mts\. Do not edit directly\. -->/, `${page.outputPath} must identify its canonical generator`)
	renderedPages.push({ html: await renderReferencePage(page.title, page.content, page.outputPath), outputPath: page.outputPath })
}
const expectedContractPageNames = new Set(renderedPages.map(page => path.basename(page.outputPath)).filter(name => name !== path.basename(outputPath)))
const existingContractPageNames = (await readdir(contractPagesDirectory).catch(() => [])).filter(name => name.endsWith('.html'))

if (Bun.argv.includes('--check')) {
	for (const page of renderedPages) {
		const outputFile = Bun.file(page.outputPath)
		assert.equal(await outputFile.exists(), true, `${page.outputPath} is missing; run bun run docs:generate-contract-reference`)
		assert.equal(await outputFile.text(), page.html, `${page.outputPath} is stale; run bun run docs:generate-contract-reference`)
	}
	const unexpectedContractPages = existingContractPageNames.filter(name => !expectedContractPageNames.has(name))
	assert.deepEqual(unexpectedContractPages, [], `${contractPagesDirectory} contains pages without a generating contract reference; run bun run docs:generate-contract-reference`)
} else {
	await mkdir(contractPagesDirectory, { recursive: true })
	for (const page of renderedPages) await writeFile(page.outputPath, page.html)
	for (const name of existingContractPageNames) {
		if (!expectedContractPageNames.has(name)) await unlink(path.join(contractPagesDirectory, name))
	}
}

type GeneratedPage = { content: string; outputPath: string; title: string }

async function generateReferencePages(): Promise<GeneratedPage[]> {
	...
	<header>
		<h1>${escapeHtml(contractReference.name)}</h1>
		<p class="lede">${renderRichText(contractReference.purpose)} <a href="${escapeHtml(repositorySourceUrl(contractReference.sourcePath))}">${escapeHtml(contractReference.name)} source contract</a></p>
	</header>
	<h2 id="read-surface">Read surface</h2>
	...
	<table>
		<thead>
			<tr>
				<th>Contract</th>
				<th>Purpose</th>
				<th>State-changing entrypoints</th>
			</tr>
		</thead>
		<tbody>
${indexRows}
